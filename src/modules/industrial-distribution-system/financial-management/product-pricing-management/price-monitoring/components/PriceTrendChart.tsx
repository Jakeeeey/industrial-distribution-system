...
function CustomTooltip({ active, payload }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-lg border bg-background px-3 py-2.5 shadow-xl text-xs min-w-[220px] flex flex-col gap-1.5">
      ...
      <div className="flex flex-col gap-1.5">
        {payload.map((entry) => {
          ...
          return (
            <div key={entry.name} className="flex flex-col gap-0.5 pb-1.5 last:pb-0 border-b last:border-0 border-muted/50">
              ...
              {changed && event && (
                <div className="pl-3.5 mt-0.5 text-[10px] text-muted-foreground flex flex-col gap-0.5 bg-muted/40 p-1.5 rounded border border-muted/20">
                  <div className="flex justify-between font-semibold text-foreground/90">
                    <span>Change:</span>
                    <span>
                      {formatCurrency(event.oldPrice)} → {formatCurrency(event.newPrice)}
                    </span>
                  </div>
                  {event.priceChangePercentage !== null && (
                    <div className="flex justify-between">
                      <span>Percentage:</span>
                      <span className="font-mono">{formatPct(event.priceChangePercentage)}</span>
                    </div>
                  )}
                  {event.approvedByName && (
                    <div className="flex justify-between text-[9px] text-muted-foreground/80 mt-0.5">
                      <span>Approved By:</span>
                      <span className="truncate max-w-[100px]" title={event.approvedByName}>
                        {event.approvedByName}
                      </span>
                    </div>
                  )}
...
function getActivePriceForGroup(
  ...
  const seedPrice = priorRows.length > 0 ? priorRows[priorRows.length - 1].newPrice : null;
  ...
  const changedOnThisDate = activeRow
    ? Math.abs(new Date(activeRow.priceChangeDatetime ?? activeRow.approvedAt ?? 0).getTime() - date.getTime()) < 60000
    : false;
...
export function PriceTrendChart({
  ...
  const priceTypeGroups = React.useMemo(() => groupByPriceType(allRows), [allRows]);
  ...
  const chartData = React.useMemo(() => {
    ...
    // 1. YEARLY MODE: Multi-year trend across all years
    if (granularity === "yearly") {
      ...
      return sortedYears.map((y) => {
        ...
        const point: Record<string, string | number | boolean | null | ViewPriceMonitoringRow | undefined> = {
          dateLabel: String(y),
          displayDate,
          timestamp: new Date(y, 11, 31).getTime(),
        };

        const activeDate = y === currentYear ? now : new Date(y, 11, 31, 23, 59, 59, 999);
        const startOfChart = new Date(y, 0, 1, 0, 0, 0, 0);

        for (const group of priceTypeGroups) {
          const { activePrice, activeEvent } = getActivePriceForGroup(group, activeDate, allRows, startOfChart);
    ...
    const startOfChart = dateFrom ? new Date(dateFrom) : new Date(selectedYear, 0, 1, 0, 0, 0, 0);
    let endOfChart = dateTo ? new Date(dateTo) : new Date(selectedYear, 11, 31, 23, 59, 59, 999);
    if (!dateTo && selectedYear === currentYear) {
      endOfChart = now;
    }

    // 2. MONTHLY MODE: month-by-month within the date range
    if (granularity === "monthly") {
      const points = [];
      let currentDate = new Date(startOfChart.getFullYear(), startOfChart.getMonth(), 1, 0, 0, 0, 0);
      ...
      while (currentDate <= endOfChart) {
        ...
        const displayDate = currentDate.toLocaleDateString("en-PH", { month: "long", year: "numeric" });
        ...
        for (const group of priceTypeGroups) {
          ...
          point[group.priceTypeName] = activePrice;
          point[`${group.priceTypeName}_event`] = monthEvents.length > 0 ? monthEvents[monthEvents.length - 1] : activeEvent;
    ...
    // 3. WEEKLY MODE: Week-by-week within the date range
    if (granularity === "weekly") {
      ...
      while (currentWeekDate <= endOfChart) {
        ...
        const displayDate = `Week ${weekNum} (${currentWeekDate.toLocaleDateString("en-PH", { month: "short", day: "numeric" })})`;
        
        const activeDate = new Date(currentWeekDate.getTime() + 7 * 24 * 60 * 60 * 1000 - 1);
        ...
        for (const group of priceTypeGroups) {
          const { activePrice, activeEvent } = getActivePriceForGroup(group, activeCapped, allRows, startOfChart);
          
          const weekEvents = group.rows.filter(r => {
            const dt = r.priceChangeDatetime ?? r.approvedAt;
            if (!dt) return false;
            const t = new Date(dt).getTime();
            return t >= currentWeekDate.getTime() && t <= activeCapped.getTime();
          });

          point[group.priceTypeName] = activePrice;
          point[`${group.priceTypeName}_event`] = weekEvents.length > 0 ? weekEvents[weekEvents.length - 1] : activeEvent;
    ...
    // 4. DAILY MODE: All days of the year / date range
    const points = [];
    ...
    while (currentDate <= endOfChart) {
      ...
      const activeCapped = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), 23, 59, 59, 999);
      ...
      for (const group of priceTypeGroups) {
        ...
        point[group.priceTypeName] = activePrice;
        point[`${group.priceTypeName}_event`] = dayEvents.length > 0 ? dayEvents[dayEvents.length - 1] : activeEvent;
  ...
  if (priceTypeGroups.length === 0) {
    return (
      <div className="flex h-[350px] items-center justify-center rounded-lg border border-dashed text-muted-foreground gap-2 text-sm">

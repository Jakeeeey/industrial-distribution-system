...
function buildFiltersText(args: {
  ...
  const { filters, categoriesById, brandsById, unitsById, suppliersById } =
    args;
...
export default function PricingMatrixView() {
  ...
  const [lookupFilters, setLookupFilters] =
    React.useState<PricingFilters>(EMPTY_FILTERS);
  ...
  const scopedCategories = React.useMemo(() => {
    ...
    return lookups.categories.filter((item) =>
  ...
  const scopedBrands = React.useMemo(() => {
    ...
    return lookups.brands.filter((item) =>
      allowedBrandIds.has(Number(item.brand_id)),
    );
  ...
  const scopedUnits = React.useMemo(() => {
    ...
    return lookups.units.filter((item) =>
      allowedUnitIds.has(Number(item.unit_id)),
    );
  ...
  React.useEffect(() => {
    ...
    const nextCat = currentCat.filter((id) =>
      allowedCategoryIds.has(Number(id)),
    );
    const nextBrand = currentBrand.filter((id) =>
      allowedBrandIds.has(Number(id)),
    );
    ...
    matrix.setFilters(
      (prev: PricingFilters): PricingFilters => ({
        ...prev,
  ...
  const [printUsedUnitIds, setPrintUsedUnitIds] = React.useState<Set<number>>(
    new Set(),
  );
    ...
    try {
      ...
      const params: PrintProductsParams = {
        ...
        category_ids: filters.category_ids.length
          ...
          : undefined,
        brand_ids: filters.brand_ids.length
          ? filters.brand_ids.join(",")
          : undefined,
        unit_ids: filters.unit_ids.length
          ? filters.unit_ids.join(",")
          : undefined,
        supplier_ids: filters.supplier_ids.length
          ? filters.supplier_ids.join(",")
          : undefined,
      ...
      for (const p of allProducts) {
        ...
        if (p.unit_of_measurement)
          usedUnitIds.add(Number(p.unit_of_measurement));
      }
      ...
      for (const [groupId, variants] of groups.entries()) {
        const display =
          variants.find((v) => v.product_id === groupId) || variants[0];
        ...
        assembled.push({
          ...
          display,
          variantsByUnitId,
          category_name: display.product_category
            ? (lookupMaps.categoriesById.get(
                Number(display.product_category),
              ) ?? null)
            : null,
          brand_name: display.product_brand
            ? (lookupMaps.brandsById.get(Number(display.product_brand)) ?? null)
            : null,
        });
      }

      assembled.sort((a, b) =>
        (a.display.product_name || "").localeCompare(
          b.display.product_name || "",
        ),
      );
      ...
      const message =
        error instanceof Error ? error.message : "Failed to open print editor";
      toast.error(message);
    } finally {
      setIsPrinting(false);
    }
  ...
  const isInitialLoad =
    (lookups.loading || pt.loading) && currentRows.length === 0;
    ...
    <div className="h-[calc(100dvh-64px)] min-h-0 overflow-hidden px-0">
      <div className="flex h-full min-h-0 flex-col gap-3">
        ...
        <PrintPricingDialog
          ...
          unitName={(id) =>
            id ? (lookupMaps.unitsById.get(Number(id)) ?? "") : ""
          }
          units={lookups.units}
          priceTypes={pt.priceTypes}

...
const TIER_STYLES = [
  {
    ...
    cell: "bg-slate-50/50 hover:bg-slate-100/80 dark:bg-slate-950/20",
    border: "border-slate-200 dark:border-slate-800",
  }, // LIST
  {
    head: "bg-indigo-50 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-200",
    cell: "bg-indigo-50/30 hover:bg-indigo-100/50 dark:bg-indigo-950/20",
    border: "border-indigo-100 dark:border-indigo-900",
  }, // A
  {
    head: "bg-sky-50 text-sky-800 dark:bg-sky-950 dark:text-sky-200",
    cell: "bg-sky-50/30 hover:bg-sky-100/50 dark:bg-sky-950/20",
    border: "border-sky-100 dark:border-sky-900",
  },
  {
    head: "bg-emerald-50 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200",
    cell: "bg-emerald-50/30 hover:bg-emerald-100/50 dark:bg-emerald-950/20",
    border: "border-emerald-100 dark:border-emerald-900",
  },
  {
    head: "bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-200",
    cell: "bg-amber-50/30 hover:bg-amber-100/50 dark:bg-amber-950/20",
    border: "border-amber-100 dark:border-amber-900",
  },
  {
    head: "bg-rose-50 text-rose-800 dark:bg-rose-950 dark:text-rose-200",
    cell: "bg-rose-50/30 hover:bg-rose-100/50 dark:bg-rose-950/20",
    border: "border-rose-100 dark:border-rose-900",
  },
  {
    head: "bg-fuchsia-50 text-fuchsia-800 dark:bg-fuchsia-950 dark:text-fuchsia-200",
    cell: "bg-fuchsia-50/30 hover:bg-fuchsia-100/50 dark:bg-fuchsia-950/20",
    ...
    <>
      {Array.from({ length: rowCount }).map((_, i) => (
        <PTableRow key={`lsk-${i}`} className="hover:bg-transparent">
          ...
          <PTableCell className="h-[80px] w-[150px]">
      ...
      {Array.from({ length: rowCount }).map((_, i) => (
        <PTableRow key={`rsk-${i}`} className="hover:bg-transparent">
          {Array.from({ length: priceCols }).map((__, j) => (
            <PTableCell
              key={`rskc-${i}-${j}`}
              className="h-[80px] border-l align-top"
            >
...
export default function PricingTable({ matrix }: Props) {
  const usedUnits: Unit[] = Array.isArray(matrix.usedUnits)
    ? matrix.usedUnits
    : [];
  ...
  const totalPages =
    toNum(meta.totalPages ?? 0, 0) ||
    (totalGroups > 0 ? Math.ceil(totalGroups / pageSize) : 1);

  const startIndex = totalGroups === 0 ? 0 : (page - 1) * pageSize;
  const endIndex =
    totalGroups === 0 ? 0 : Math.min(totalGroups, startIndex + rows.length);

  const canPrev = !loading && page > 1 && totalGroups > 0;
  const canNext = !loading && page < totalPages && totalGroups > 0;

  const headCellBase =
    "whitespace-nowrap border-b text-[12px] font-semibold text-foreground/80 p-0";
  const subHeadCellBase =
    "whitespace-nowrap border-b text-[11px] font-medium text-foreground/70 p-0";
  ...
  const thumbX =
    maxScrollLeft <= 0
      ? 0
      : Math.floor((metrics.scrollLeft / maxScrollLeft) * thumbMaxX);
  ...
  const onThumbMouseDown = React.useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setScrollLeft(
          draggingRef.current.startScrollLeft + ratio * maxScrollLeft,
        );
      };
        ...
        <div className="flex min-w-0">
          ...
          >
            <div className="sticky top-0 z-10 overflow-hidden bg-background">
              <PTable
                className="pmx-table table-fixed"
                style={{ width: LEFT_TABLE_WIDTH }}
              >
                <colgroup>
            ...
            >
              ...
              <PTableBody
                className={cn(loading && rows.length > 0 && "pmx-loading-row")}
              >
                {loading && rows.length === 0 ? (
                  <LoadingLeftBody rowCount={10} />
                ) : null}
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <PTableRow
                      key={`L-${groupKey}`}
                      className={cn(
                        hovered ? "bg-muted/30" : "hover:bg-muted/30",
                      )}
                    ...
                    >
                      <PTableCell
                        ...
                        style={{
                          width: LEFT_COL_WIDTHS[0],
                          height: BODY_ROW_H,
                        }}
                      >
                        <div
                          style={{ height: BODY_ROW_H }}
                          className="flex flex-col justify-center gap-1.5 px-3"
                        >
                      ...
                      <PTableCell
                        className="p-0 border-b overflow-hidden"
                        style={{
                          width: LEFT_COL_WIDTHS[1],
                          height: BODY_ROW_H,
                        }}
                      >
                        <div
                          style={{ height: BODY_ROW_H }}
                          className="flex items-center px-3 truncate text-[13px] text-muted-foreground"
                        >
                          {r.category_name ?? "—"}
                        </div>
                      </PTableCell>
            ...
            >
              ...
              <div className="w-max min-w-full">
                <PTable className="pmx-table w-max table-fixed">
                  ...
                  >
                    {loading && rows.length === 0 ? (
                      <LoadingRightBody rowCount={10} priceCols={priceCols} />
                    ) : null}
                        ...
                        >
                          {tiers.map((tier, ti) => {
                            ...
                            if (!usedUnits.length) {
                              return (
                                <PTableCell
                                  ...
                                  className={cn(
                                    "p-0 text-center border-b text-muted-foreground",
                                    st.cell,
                                    st.border,
                                    ti === 0 ? "" : "border-l",
                                  )}
                            ...
                            return usedUnits.map((u, uIndex) => {
                              ...
                              const variantProductId = Number(
                                variant.product.product_id,
                              );
                              if (
                                !Number.isFinite(variantProductId) ||
                                variantProductId <= 0
                              ) {
                                return (
                                  <PTableCell
                              ...
                              const base = toNullableNumber(
                                variant.tiers?.[tier],
                              );
                              const val = matrix.getCellValue(
                                variantProductId,
                                tier,
                                base,
                              );
                              const dirty = matrix.isDirty(
                                variantProductId,
                                tier,
                              );
                              const err = toErrorString(
                                matrix.getError(variantProductId, tier),
                              );
        ...
        <div className="flex items-center justify-between gap-2 sm:justify-end">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              size="sm"
              className="h-8 px-2"
              onClick={() => matrix.setPage(Math.max(1, page - 1))}
              disabled={!canPrev}
              title="Previous page"
            >
            ...
            <Button
              ...
              className="h-8 px-2"
              onClick={() =>
                matrix.setPage(Math.min(Math.max(1, totalPages), page + 1))
              }
              disabled={!canNext}
              title="Next page"
            >
            ...
            <Button
              ...
              size="sm"
              className="h-8 px-2"
              onClick={() => matrix.setPage(Math.max(1, totalPages))}
              disabled={!canNext}
              title="Last page"
            >

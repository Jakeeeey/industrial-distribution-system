...
export function generatePricingMatrixPdf(
  ...
  for (const tier of TIERS) {
    if (usedUnits.length === 0) {
      headRow3.push({
        content: "Price",
        styles: { halign: "center", fillColor: groupColors[tier] },
      });
    } else {
  ...
  const body: (string | number)[][] = rows.map((row) => {
    ...
    const cells: (string | number)[] = [display.product_code || "—"];
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      if (
        data.section === "body" &&
        data.column.index >= (includeBarcode ? 5 : 4)
      ) {
        const tierIdx = Math.floor(
          (data.column.index - (includeBarcode ? 5 : 4)) / uomCount,
        );

...
export async function updatePriceType(id: number, data: Partial<PriceType>) {
    return http<{ data: PriceType }>(`/api/ids/fm/product-pricing/price-types/${id}`, {
        method: "PATCH",
        body: JSON.stringify(data),
    });
}
...
export async function getPricesForProducts(productIds: number[]) {
    ...
    return http<{ data: PriceRow[] }>(`/api/ids/fm/product-pricing/prices?${sp.toString()}`);
}

export async function upsertPrices(lines: UpsertLine[]) {
    return http<{ ok: boolean; affected: number }>(`/api/ids/fm/product-pricing/prices-upsert`, {
        method: "POST",
...
export async function bulkUpdateProducts(items: { product_id: number; cost_per_unit: number | null }[]) {
    return http<{ ok: boolean; affected: number }>(`/api/ids/fm/product-pricing/products/bulk-patch`, {
        method: "POST",
        body: JSON.stringify({ items }),
    });
}
...
export async function getPendingPriceRequests() {
    return http<{ data: PriceChangeRequest[] }>("/api/ids/fm/product-pricing/price-change-requests?status=PENDING&limit=-1");
}

export async function getPendingCostRequests() {
    return http<{ data: CostChangeRequest[] }>("/api/ids/fm/product-pricing/cost-change-requests?status=PENDING&limit=-1");
}

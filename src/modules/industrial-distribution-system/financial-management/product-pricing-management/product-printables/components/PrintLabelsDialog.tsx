// src/modules/industrial-distribution-system/financial-management/printables-management/product-printables/components/PrintLabelsDialog.tsx
"use client";
...
export default function PrintLabelsDialog({
  ...
  const [companyData, setCompanyData] = React.useState<CompanyData | null>(
    null,
  );
      ...
      const init = async () => {
        ...
        try {
          ...
          if (compRes.ok) {
            ...
            const company =
              result.data?.[0] ||
              (Array.isArray(result.data) ? null : result.data);
            ...
            (() => {
              ...
              const rows2: { label: string; value: string }[] = [
                { label: "Paper Size", value: cfg.paperSize },
                {
                  label: "Orientation",
                  value:
                    cfg.orientation.charAt(0).toUpperCase() +
                    cfg.orientation.slice(1),
                },
                {
                  label: "Margins",
                  value: `T:${cfg.margins.top} R:${cfg.margins.right} B:${cfg.margins.bottom} L:${cfg.margins.left} mm`,
                },

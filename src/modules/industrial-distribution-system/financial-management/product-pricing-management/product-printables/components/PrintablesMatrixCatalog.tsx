// src/modules/industrial-distribution-system/financial-management/product-pricing-management/product-printables/components/PrintablesMatrixCatalog.tsx
"use client";
      ...
      {rows.map((row) => {
        ...
        const hasAnyPrices = visibleUnits.some((u) => {
          ...
          return activePriceTypes.some((pt, i) => {
            ...
            return (
              (variant.tiers as Record<string, number | null>)?.[ptSuffix] !=
              null
            );
          });
        });
        ...
        return (
          <Card
            key={row.group_id}
            className="overflow-hidden hover:shadow-md transition-all duration-200 border-[#D1D5DB] flex flex-col group"
          >
            ...
            <CardHeader className="p-4 pb-2 space-y-2">
              <div className="flex flex-wrap gap-1.5 mb-1">
                {row.brand_name && row.brand_name !== "—" && (
                  <Badge
                    variant="secondary"
                    className="text-[10px] uppercase font-medium bg-secondary/50 text-secondary-foreground flex items-center gap-1 px-1.5"
                  >
                ...
                {row.category_name && row.category_name !== "—" && (
                  <Badge
                    variant="outline"
                    className="text-[10px] uppercase font-medium text-muted-foreground border-border/50 px-1.5"
                  >
                    {row.category_name}
                  </Badge>
                )}
              ...
              <CardTitle
                ...
                title={row.display.product_name || ""}
              >
              ...
              <div className="mt-2 space-y-3">
                {visibleUnits.map((u) => {
                  ...
                  return (
                    <div
                      key={u.unit_id}
                      className="rounded-lg border border-border/50 bg-card overflow-hidden text-sm"
                    >
                              ...
                              <div className="flex items-center gap-2">
                                <div
                                  className={cn(
                                    ...
                                    groupColors[i % groupColors.length].split(
                                      " ",
                                    )[0],
                                  )}
                                />

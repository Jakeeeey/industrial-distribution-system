// src/modules/industrial-distribution-system/financial-management/printables-management/product-printables/components/PrintablesFiltersBar.tsx
"use client";
...
const FilterSelector = ({
  ...
  <Popover>
    <PopoverTrigger asChild>
      <Button
        variant="outline"
        className="w-full justify-between rounded-xl border-border/50 bg-background/50 text-left font-normal h-9 px-3"
      >
    ...
    <div className="flex flex-col gap-4 bg-background/60 backdrop-blur-md p-6 rounded-2xl border border-border/50 shadow-sm">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <FilterSelector
          ...
          onClear={() =>
            setFilters((prev) => ({ ...prev, supplier_ids: [], page: 1 }))
          }
        />
        ...
        <FilterSelector
          ...
          onClear={() =>
            setFilters((prev) => ({ ...prev, brand_ids: [], page: 1 }))
          }
        />
        ...
        <FilterSelector
          ...
          onClear={() =>
            setFilters((prev) => ({ ...prev, category_ids: [], page: 1 }))
          }
        />
        ...
        <FilterSelector
          ...
          options={priceTypes.map((pt) => ({
            ...
            value: String(pt.price_type_id),
          }))}
          onToggle={(id) => toggleFilter("price_type_ids", id)}
          onClear={() =>
            setFilters((prev) => ({ ...prev, price_type_ids: [] }))
          }
        />
      </div>
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            onCheckedChange={(checked) =>
              setFilters((prev) => ({ ...prev, active_only: checked, page: 1 }))
            }
          />
          <Label
            htmlFor="print-active-only"
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-dashed border-border/50">
          ...
          {filters.q && (
            <Badge
              variant="secondary"
              className="gap-1 rounded-lg px-2 py-0.5 bg-primary/5 text-primary border-primary/20"
            >
              Search: {filters.q}
              <X
          ...
          {(filters.category_ids || []).map((id) => {
            const name = categories.find(
              (c) => String(c.category_id) === id,
            )?.category_name;
            return (
              <Badge
                key={id}
                variant="secondary"
                className="gap-1 rounded-lg px-2 py-0.5"
              >
                Cat: {name || id}
                <X
                  className="w-3 h-3 cursor-pointer hover:text-destructive"
                  onClick={() => removeFilter("category_ids", id)}
                />
              </Badge>
            );
          })}
          {(filters.brand_ids || []).map((id) => {
            ...
            return (
              ...
              >
                ...
                <X
                  className="w-3 h-3 cursor-pointer hover:text-destructive"
                  onClick={() => removeFilter("brand_ids", id)}
                />
              </Badge>
            );
          })}
          {(filters.unit_ids || []).map((id) => {
            const name = units.find(
              (u) => String(u.unit_id) === id,
            )?.unit_shortcut;
            return (
              <Badge
                key={id}
              ...
              >
                ...
                <X
                  className="w-3 h-3 cursor-pointer hover:text-destructive"
                  onClick={() => removeFilter("unit_ids", id)}
                />
              </Badge>
            );
          })}
          {(filters.supplier_ids || []).map((id) => {
            ...
            return (
              <Badge
                key={id}
                variant="secondary"
                className="gap-1 rounded-lg px-2 py-0.5 bg-orange-50 text-orange-700 border-orange-200"
              >
                Supplier: {name || id}
                <X
                  className="w-3 h-3 cursor-pointer hover:text-destructive"
                  onClick={() => removeFilter("supplier_ids", id)}
                />
              </Badge>
            );
          })}
          {(filters.price_type_ids || []).map((id) => {
            const name = priceTypes.find(
              (pt) => String(pt.price_type_id) === id,
            )?.price_type_name;
            return (
              <Badge
                key={id}
                variant="secondary"
                className="gap-1 rounded-lg px-2 py-0.5 bg-blue-50 text-blue-700 border-blue-200"
              >
                {getTierLabel(name || id)}
                <X
                  className="w-3 h-3 cursor-pointer hover:text-destructive"
                  onClick={() => removeFilter("price_type_ids", id)}
                />
              </Badge>
            );
          })}
          {filters.active_only && (
            <Badge
              variant="secondary"
              className="gap-1 rounded-lg px-2 py-0.5 bg-green-50 text-green-700 border-green-200"
            >
              Active only
              <X
                className="w-3 h-3 cursor-pointer hover:text-destructive"

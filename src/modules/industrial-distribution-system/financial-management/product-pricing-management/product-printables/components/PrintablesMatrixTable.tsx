// src/modules/industrial-distribution-system/financial-management/printables-management/product-printables/components/PrintablesMatrixTable.tsx
"use client";
...
export default function PrintablesMatrixTable({
  ...
  const activePriceTypes = priceTypes.filter((pt) => {
    if (selectedPriceTypeIds.length === 0)
      return pt.sort != null && pt.sort <= 5; // Default to first 5
      ...
      <Table className="border-collapse border-hidden">
        <TableHeader className="bg-[#F9FAFB]">
          ...
          <TableRow className="border-b border-[#D1D5DB]">
            <TableHead
              colSpan={3}
              className="border-r border-[#D1D5DB] sticky left-0 z-30 bg-[#F9FAFB]"
            ></TableHead>
            <TableHead
          ...
          <TableRow className="border-b border-[#D1D5DB]">
            <TableHead className="font-bold sticky left-0 z-30 bg-[#F9FAFB] border-r border-[#D1D5DB] min-w-[120px] text-[10px] uppercase text-[#374151]">
              Brand
            </TableHead>
            <TableHead className="font-bold sticky left-[120px] z-30 bg-[#F9FAFB] border-r border-[#D1D5DB] min-w-[120px] text-[10px] uppercase text-[#374151]">
              Category
            </TableHead>
            <TableHead className="font-bold sticky left-[240px] z-30 bg-[#F9FAFB] border-r border-[#D1D5DB] min-w-[180px] text-[10px] uppercase text-[#374151]">
              Product Name
            </TableHead>
              ...
              {activePriceTypes.map((pt) => (
                <React.Fragment key={pt.price_type_id}>
                  {visibleUnits.length > 0 ? (
                    ...
                    <TableHead className="min-w-[70px] border-r border-[#E5E7EB]">
                      —
                    </TableHead>
                  )}
                </React.Fragment>
        ...
        <TableBody>
          {rows.map((row) => (
            <TableRow
              key={row.group_id}
              className="hover:bg-[#F3F4F6] transition-colors border-b border-[#E5E7EB]"
            >
              ...
              {activePriceTypes.map((pt, i) => {
                ...
                const nonSyntheticIndex = activePriceTypes
                  .slice(0, i)
                  .filter((p) => p.price_type_id !== -1).length;
                const ptSuffix =
                  pt.price_type_id === -1
                    ? "ListPrice"
                    : ["A", "B", "C", "D", "E"][nonSyntheticIndex] || "A";
                  ...
                  <React.Fragment key={pt.price_type_id}>
                    {visibleUnits.length > 0 ? (
                      visibleUnits.map((u) => {
                        ...
                        const price = (
                          variant?.tiers as Record<string, number | null>
                        )?.[ptSuffix];
                      ...
                      <TableCell className="border-r border-[#E5E7EB] bg-[#F9FAFB]/50">
                        —
                      </TableCell>
                    )}
                  </React.Fragment>

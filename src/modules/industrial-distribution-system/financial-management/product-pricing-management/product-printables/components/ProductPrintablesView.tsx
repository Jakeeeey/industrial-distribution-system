// src/modules/industrial-distribution-system/financial-management/printables-management/product-printables/components/ProductPrintablesView.tsx
"use client";
...
import {
  ...
  defaultFilters,
} from "../hooks/useProductPrintables";
...
function pickId(
  v: string | number | null | undefined | Record<string, unknown>,
): number | null {
...
export default function ProductPrintablesView({
  ...
  const {
    ...
    resetFilters,
  } = useProductPrintables(filters, setFilters, categories, brands);
  ...
  const [allUsedUnitIds, setAllUsedUnitIds] = React.useState<Set<number>>(
    new Set(),
  );
  ...
  const filterSummary = React.useMemo(() => {
    ...
    if (filters.category_ids.length) {
      const names = filters.category_ids
        .map(
          (id) =>
            categories.find((c) => String(c.category_id) === String(id))
              ?.category_name,
        )
        .filter(Boolean);
    ...
    if (filters.brand_ids.length) {
      const names = filters.brand_ids
        .map(
          (id) =>
            brands.find((b) => String(b.brand_id) === String(id))?.brand_name,
        )
        .filter(Boolean);
      if (names.length) parts.push(`Brands: ${names.join(", ")}`);
    }
    if (filters.supplier_ids.length) {
      const names = filters.supplier_ids
        .map(
          (id) =>
            suppliers.find((s) => String(s.id) === String(id))?.supplier_name,
        )
        .filter(Boolean);
  ...
  const handlePrintAll = async () => {
    ...
    try {
      ...
      if (filters.category_ids.length)
        sp.set("category_ids", filters.category_ids.join(","));
      if (filters.brand_ids.length)
        sp.set("brand_ids", filters.brand_ids.join(","));
      if (filters.unit_ids.length)
        sp.set("unit_ids", filters.unit_ids.join(","));
      if (filters.supplier_ids.length)
        sp.set("supplier_ids", filters.supplier_ids.join(","));
      ...
      const catMap = new Map(
        categories.map((c) => [Number(c.category_id), c.category_name]),
      );
      const brandMap = new Map(
        brands.map((b) => [Number(b.brand_id), b.brand_name]),
      );
      ...
      for (const [groupId, variants] of groups.entries()) {
        const display =
          variants.find((v) => Number(v.product_id) === groupId) || variants[0];
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to prepare print data",
      );
      console.error(error);
    } finally {
      setIsPrinting(false);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <p className="text-sm text-muted-foreground">
            Filter and generate spreadsheet-style matrix reports for your
            products.
          </p>
        </div>
        <div className="flex gap-2">
          <ToggleGroup
            type="single"
            value={viewMode}
            onValueChange={(v) => v && setViewMode(v as "list" | "catalog")}
            className="bg-background border border-border/50 rounded-xl p-0.5"
          >
            <ToggleGroupItem
              value="list"
              aria-label="List View"
              className="rounded-lg px-3 data-[state=on]:bg-primary/10 data-[state=on]:text-primary"
            >
      ...
      {filters.total_pages > 1 && (
        <div className="flex items-center justify-between px-2 py-4 bg-muted/20 rounded-2xl border border-border/50">
          <div className="text-sm text-muted-foreground">
            Page{" "}
            <span className="font-bold text-foreground">{filters.page}</span> of{" "}
      ...
      <PrintLabelsDialog
        ...
        supplier={
          filters.supplier_ids.length === 1
            ? suppliers.find((s) => String(s.id) === filters.supplier_ids[0])
            : null
        }
        selectedPriceTypeIds={filters.price_type_ids}

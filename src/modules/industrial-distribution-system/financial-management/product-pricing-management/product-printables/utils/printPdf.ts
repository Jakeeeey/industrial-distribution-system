// src/modules/industrial-distribution-system/financial-management/printables-management/product-printables/utils/printPdf.ts
...
export async function generateProductMatrixPdf(
  rows: MatrixRow[],
  ...
  const { drawPageNumbers } =
    await import("@/components/pdf-layout-design/PdfGenerator");
  const {
    fontSize = 7,
  ...
  const activeTiers =
    allPriceTypes.length > 0
      ? allPriceTypes
          .filter(
            (pt) =>
              ...
              selectedPriceTypeIds.includes(String(pt.price_type_id)),
          )
          .map((pt) => {
            if (pt.price_type_id === -1) {
              return {
                key: "ListPrice" as const,
                label: getTierLabel(pt.price_type_name),
              };
            }
            const nonSynthetic = allPriceTypes.filter(
              (p) => p.price_type_id !== -1,
            );
            const idx = nonSynthetic.findIndex(
              (p) => p.price_type_id === pt.price_type_id,
            );
            return {
      ...
      : TIERS.map((key, i) => ({
          key,
          label: getTierLabel(
            allPriceTypes?.[i]?.price_type_name || `PRICE TYPE ${key}`,
          ),
        })).slice(0, 5);
  ...
  const finalPaper =
    tplConfig?.paperSize.toLowerCase() || options.paper || "a4";
  const finalOrientation =
    tplConfig?.orientation || options.orientation || "landscape";

  // Margins: prefer template config, fallback to default
  const finalMargins = tplConfig?.margins || {
    top: 10,
    left: 10,
    right: 10,
    bottom: 10,
  ...
  for (const tier of activeTiers) {
    if (usedUnits.length === 0) {
      headRow2.push({
        content: "Price",
        styles: { halign: "center", fillColor: groupColors[tier.key] },
      });
    } else {
  ...
  if (selectedTemplate) {
    let templateHeaderEnd = await PdfEngine.applyTemplate(
      doc,
    ...
    doc.line(
      currentX,
      metadataTop + 2,
      doc.internal.pageSize.getWidth() - finalMargins.right,
      metadataTop + 2,
    );
    ...
    if (filterSummary) {
      ...
      doc.setFont("helvetica", "normal");
      const filterX =
        finalMargins.left + doc.getTextWidth("Active Criteria: ") + 1.5;
      const splitFilters = doc.splitTextToSize(
        filterSummary,
        doc.internal.pageSize.getWidth() - filterX - finalMargins.right,
      );
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      "OFFICIAL PRODUCT PRICING MATRIX REPORT",
      finalMargins.left,
      finalMargins.top + 11,
    );
    ...
    doc.roundedRect(
      finalMargins.left,
      y - 5,
      doc.internal.pageSize.getWidth() - finalMargins.left - finalMargins.right,
      metaHeight,
      1,
      1,
      "FD",
    ...
    if (filterSummary) {
      ...
      doc.setFont("helvetica", "normal");
      const filterX =
        finalMargins.left + 5 + doc.getTextWidth("FILTER CRITERIA: ") + 2;
      const splitFilters = doc.splitTextToSize(
        filterSummary,
        doc.internal.pageSize.getWidth() - filterX - finalMargins.right - 5,
      );
    ...
    if (supplier) {
      ...
      if (supplier.contact_person)
        details.push(`Contact: ${supplier.contact_person}`);
      if (supplier.phone_number)
        details.push(`Phone: ${supplier.phone_number}`);

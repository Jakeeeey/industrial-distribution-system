// src/modules/industrial-distribution-system/financial-management/treasury/bulk-approval/components/ActivityFeed.tsx
"use client";
...
export function ActivityFeed({ logs, loading }: Props) {
  ...
  const [details, setDetails] = React.useState<
    Record<number, ActivityLogDetail[]>
  >({});
      ...
      <div className="flex-1 overflow-auto pr-1 pb-4 space-y-3 rounded-xl">
        {loading ? (
          ...
          filtered.map((draft) => {
            ...
            const latestMeta = getOutcomeMeta(
              latestRound?.outcome ?? "IN_PROGRESS",
            );
              ...
              >
                ...
                >
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    {/* Avatar */}
                    <div
                      className={`shrink-0 h-10 w-10 rounded-full border-2 font-black text-sm flex items-center justify-center shadow-sm ${latestMeta.pill}`}
                    >
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    <span
                      className={`flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${draftStatus.cls}`}
                    >
                      {draftStatus.icon}
                      {draftStatus.label}
                    </span>
                        ...
                        [...draft.rounds]
                          ...
                          .map((round) => (
                            <RoundSection
                              ...
                              round={round}
                              defaultOpen={
                                round.is_current || draft.rounds.length === 1
                              }
                            />
                          ))
                      )}

// src/modules/industrial-distribution-system/financial-management/treasury/bulk-approval/components/VoteModal.tsx
"use client";
...
export default function VoteModal({
  ...
  const currentTotalAmount = React.useMemo(() => {
    ...
    return payables.reduce((acc, p) => {
      ...
      return (
        acc + (val !== undefined && val !== "" ? Number(val) : Number(p.amount))
      );
    }, 0);
  ...
  const { draft, payables, approvers_by_level, my_level, my_vote, can_vote } =
    detail;
  const currentTier = draft.current_tier ?? ParseTierLabel(draft.status);
  const maxLevel = draft.max_level ?? 1;

  const isRejectionSubmittable =
    confirmAction === "REJECTED" && remarks.trim().length >= 10;
  ...
  async function handleVote() {
    ...
    try {
      ...
      const adjustmentSummary = payloadEditedPayables
        .map((ep) => {
      ...
      const result = await api.submitVote({
        ...
        edited_payables:
          payloadEditedPayables.length > 0 ? payloadEditedPayables : undefined,
      });

      if (result.result === "APPROVED") {
        toast.success(
          `Draft fully approved! Posted as ${result.doc_no ?? "live disbursement"}.`,
          {
        ...
        toast.success(
          `Level ${currentTier} complete! Advanced to Level ${result.next_tier}.`,
          {
            description: result.message,
          },
        );
      } else if (result.result === "VOTE_RECORDED") {
        toast.info("Your approval has been recorded.", {
          description: result.message,
        });
      } else if (result.result === "REJECTED") {
        toast.error("Draft has been rejected.", {
          description: result.message,
        });
      }
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[1250px] w-[98vw] max-h-[92vh] flex flex-col gap-0 p-0 overflow-hidden border-none shadow-2xl rounded-xl">
        <DialogTitle className="sr-only">
          Verification & Approval - {draft.doc_no}
        </DialogTitle>
        <DialogDescription className="sr-only">
          Review details and vote on the treasury disbursement draft for{" "}
          {draft.payee_name}.
        </DialogDescription>
        {/* Header */}
        ...
        <div className="px-8 py-5 bg-background border-b relative overflow-hidden shrink-0">
          <div className="flex items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex flex-col items-end gap-0.5">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 leading-none mb-0.5 text-right w-full">
                Current Total
              </p>
              ...
              <div className="flex-[3] flex flex-col min-w-0 min-h-0 bg-background relative border-r overflow-hidden shadow-sm">
                <div className="px-6 py-3 bg-muted/10 border-b flex items-center justify-between sticky top-0 z-10 backdrop-blur-md">
                  <div className="flex items-center gap-2 text-primary font-black uppercase tracking-[0.2em] text-[10px]">
                    ...
                    <Badge
                      variant="secondary"
                      className="ml-2 h-4 px-1.5 bg-primary/10 text-primary border-none pointer-events-none"
                    >
                    ...
                    <table className="w-full text-left border-collapse table-fixed">
                      ...
                      <thead className="sticky top-0 bg-background/95 backdrop-blur-md z-10">
                        <tr className="border-b bg-muted/20">
                          <th className="py-2.5 px-3 text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            #
                          </th>
                          <th className="py-2.5 px-3 text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            Account / COA
                          </th>
                          <th className="py-2.5 px-3 text-right text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            Amount
                          </th>
                          <th className="py-2.5 px-3 text-center text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            Trans Date
                          </th>
                          <th className="py-2.5 px-3 text-center text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            Docs
                          </th>
                          <th className="py-2.5 px-3 text-left text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">
                            Audit Remarks
                          </th>
                        </tr>
                      </thead>
                        ...
                        {payables.length === 0 ? (
                          ...
                          payables.map((p, idx) => {
                            const originalAmount =
                              detail.expense_logs?.find(
                                (l) =>
                                  l.expense_id === p.id &&
                                  l.action === "Edited",
                              )?.amount || p.amount;
                            const isEdited =
                              Number(editedAmounts[p.id] || p.amount) !==
                            ...
                            return (
                              ...
                              >
                                <td className="py-2.5 px-3 text-[10px] text-muted-foreground/40 font-mono font-bold">
                                  {idx + 1}
                                </td>
                                <td className="py-2.5 px-3">
                                  <div className="flex flex-col gap-0">
                                    <span
                                      className="text-xs font-black text-foreground group-hover:text-primary transition-colors truncate"
                                      title={p.coa_name}
                                    >
                                      {p.coa_name || "Uncategorized"}
                                    </span>
                                    <div className="flex items-center gap-2">
                                      <span className="text-[9px] text-muted-foreground/60 font-mono uppercase tracking-tighter">
                                        #{p.coa_id}
                                      </span>
                                      {p.reference_no && (
                                        <Badge
                                          variant="outline"
                                          className="h-3.5 px-1 text-[7px] text-muted-foreground tracking-tighter font-mono rounded-sm opacity-60"
                                        >
                                          REF: {p.reference_no}
                                        </Badge>
                                      )}
                                ...
                                <td className="py-2.5 px-3 text-center">
                                  {p.attachment_url ? (
                                    ...
                                    <span className="text-[8px] font-bold text-muted-foreground/20 italic tracking-widest">
                                      —
                                    </span>
                                  )}
                                </td>
                ...
                <div className="flex-[1.5] min-w-[320px] max-w-[400px] flex flex-col min-h-0 bg-muted/10 relative border-l animate-in slide-in-from-right-2 duration-300 overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin">
                    ...
                    {(() => {
                      ...
                      // 1. Add Tiers (Votes)
                      Array.from({ length: maxLevel }, (_, i) => i + 1).forEach(
                        (level) => {
                          ...
                          approvers.forEach((a) => {
                            if (a.vote) {
                              timeline.push({
                                ...
                                node: (
                                  <div
                                    key={`vote-${a.approver_id}`}
                                    className="relative pl-6 border-l border-emerald-500/20 pb-2"
                                  >
                                    ...
                                    <div className="flex flex-col gap-1">
                                      <div className="flex items-center justify-between">
                                        <p className="text-[10px] font-black text-emerald-700/80 uppercase tracking-tighter">
                                          Level {level} Verified
                                        </p>
                                        <p className="text-[8px] font-bold text-muted-foreground/40">
                                          {formatDateTime(a.vote.created_at)}
                                        </p>
                                      </div>
                                      <p className="text-xs font-black text-foreground/90 leading-none">
                                        {a.name}
                                      </p>
                                      {a.vote.remarks && (
                                        <p className="text-[10px] font-medium italic text-muted-foreground/60">
                                          &ldquo;{a.vote.remarks}&rdquo;
                                        </p>
                                      )}
                                    </div>
                                  </div>
                      ...
                      detail.logs?.forEach((log) => {
                        ...
                        timeline.push({
                          ...
                          node: (
                            <div
                              key={`log-${log.id}`}
                              className="relative pl-6 border-l border-primary/20 pb-2"
                            >
                              ...
                              <div className="flex flex-col gap-1">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-[10px] font-black tabular-nums ${variance > 0 ? "text-rose-500" : "text-emerald-500"}`}
                                  >
                                    ({variance > 0 ? "+" : ""}
                                    {formatCurrency(variance)})
                                  </span>
                                </div>
                                <p className="text-[10px] font-medium italic text-muted-foreground/60 bg-muted/30 p-1.5 rounded-md mt-1">
                                  &ldquo;{log.edit_reason}&rdquo;
                                </p>
                              </div>
                            </div>
                          ),
                          ...
                          node: (
                            ...
                            >
                              ...
                              <div className="flex flex-col gap-1">
                                ...
                                <p className="text-[10px] font-black text-foreground/80 leading-none">
                                  {log.particulars}
                                </p>
                                {log.remarks && (
                                  <p className="text-[9px] font-medium italic text-muted-foreground/60 leading-snug">
            ...
            <div className="px-6 py-4 bg-background border-t shadow-[0_-12px_32px_-12px_rgba(0,0,0,0.12)] shrink-0 relative z-30">
              ...
              {my_vote && (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {my_vote.remarks && (
                      <p className="text-xs font-medium italic mt-0.5 opacity-70">
                        &ldquo;{my_vote.remarks}&rdquo;
                      </p>
                    )}
                  </div>
                </div>
              ...
              {!can_vote && !my_vote && (
                <div className="flex items-center gap-3 p-3 rounded-xl mb-3 bg-muted/40 border-2 border-muted border-dashed text-xs text-muted-foreground font-bold shadow-inner">
                  ...
                  <div>
                    <p className="uppercase tracking-wider text-[10px] opacity-60">
                      Status: Locked
                    </p>
                    <p className="text-foreground/60">
                      This draft is at{" "}
                      <span className="text-primary">Level {currentTier}</span>.
                      You can vote at Level {my_level}.
                    </p>
                  </div>
                </div>
              )}
                          ...
                          <span>
                            {confirmAction === "REJECTED" ? (
                              <>
                                ...
                                <span className="text-red-600 ml-1 opacity-100">
                                  *
                                </span>
                              </>
                            ) : (
                        ...
                        {confirmAction === "REJECTED" &&
                          ...
                          remarks.trim().length < 10 && (
                            <p className="text-[11px] text-red-600 font-black animate-in slide-in-from-left-2 px-1">
                              {10 - remarks.trim().length} more character
                              {10 - remarks.trim().length !== 1 ? "s" : ""}{" "}
                              needed to confirm rejection.
                            </p>
                          )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={
                            submitting ||
                            (confirmAction === "REJECTED" &&
                              !isRejectionSubmittable) ||
                            (confirmAction === "APPROVED" &&
                              !isApprovalSubmittable)
                          }
                        >
                          {submitting ? (
      ...
      >
        ...
        >
          ...
          <DialogDescription className="sr-only">
            Full-screen view of the encoded attachment document.
          </DialogDescription>

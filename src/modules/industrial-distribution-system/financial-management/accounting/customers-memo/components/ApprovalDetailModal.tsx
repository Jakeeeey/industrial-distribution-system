// src/modules/industrial-distribution-system/financial-management/accounting/customers-memo/components/ApprovalDetailModal.tsx
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[95vw] sm:max-w-5xl p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl"
      >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 space-y-2">
                  ...
                  <p className="text-xl font-black text-slate-900 tabular-nums">
                    {formatCurrency(details.header.amount)}
                  </p>
                </div>
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 space-y-2">
                  <div className="flex items-center gap-2 text-blue-600">
                  ...
                  <p className="text-xl font-black text-slate-900 tabular-nums">
                    {formatCurrency(details.header.applied_amount || 0)}
                  </p>
                </div>
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200/60 space-y-2">
              ...
              <div className="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm overflow-hidden">
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    <DetailItem
                      label="Customer"
                      value={details.header.customer_id.customer_name}
                    />
                    <DetailItem
                      label="Supplier"
                      value={details.header.supplier_id.supplier_name}
                    />
                    <DetailItem
                      label="Salesman"
                      value={`${details.header.salesman_id.salesman_code} - ${details.header.salesman_id.salesman_name}`}
                    />
                    <DetailItem
                      label="Memo Type"
                      value={
                        details.header.type === 1
                          ? "Credit Memo"
                          : details.header.type === 2
                            ? "Debit Memo"
                            : "Unknown"
                      }
                    />
                  ...
                  <div className="space-y-4">
                    <DetailItem
                      label="GL Account"
                      value={details.header.chart_of_account.account_title}
                    />
                    <DetailItem
                      label="Reason"
                      value={details.header.reason || "No reason provided"}
                    />
                    <DetailItem
                      label="Date Created"
                      value={new Date(
                        details.header.created_at,
                      ).toLocaleString()}
                    />
                  </div>
                </div>

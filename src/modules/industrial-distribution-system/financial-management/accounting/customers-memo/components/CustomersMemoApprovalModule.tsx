// src/modules/industrial-distribution-system/financial-management/accounting/customers-memo/components/CustomersMemoApprovalModule.tsx
...
export default function CustomersMemoApprovalModule() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [memoData, supplierData, customerData, salesmanData] =
        await Promise.all([
  ...
  const filteredMemos = memos.filter((m) => {
    const matchesSearch =
      ...
      m.customer_id.customer_name
        .toLowerCase()
        .includes(searchQuery.toLowerCase());

    const matchesSupplier =
      !filterSupplier || String(m.supplier_id?.id || "") === filterSupplier;
    const matchesCustomer =
      !filterCustomer || String(m.customer_id?.id || "") === filterCustomer;
    const matchesSalesman =
      !filterSalesman || String(m.salesman_id?.id || "") === filterSalesman;
    const matchesTab =
      activeTab === "all" ||
      (activeTab === "credit" && m.type === 1) ||
      (activeTab === "debit" && m.type === 2);
  ...
  const toggleSelectAll = () => {
    if (
      selectedIds.size === paginatedMemos.length &&
    ...
    <div className="flex flex-col gap-6 p-6 lg:p-10 max-w-[1700px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 min-h-screen pb-32">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pb-4 border-b border-muted/50">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 ring-4 ring-slate-50">
          ...
          >
            <RefreshCcw
              className={`h-5 w-5 ${loading ? "animate-spin text-amber-500" : ""}`}
            />
          </Button>
        </div>
      </div>
      ...
      <div className="flex bg-slate-200/50 p-1.5 rounded-[1.25rem] w-fit border border-slate-200 shadow-inner">
        <Button
          ...
          className={cn(
            "h-10 px-8 rounded-xl font-black text-xs transition-all",
            activeTab === "all"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          ...
          color="amber"
          trend={
            searchQuery || filterCustomer || filterSupplier || filterSalesman
              ? "Filtered Set"
              : "Memos Pending"
          }
        />
        <StatsCard
          ...
          value={formatCurrency(
            filteredMemos.reduce((s, m) => s + m.amount, 0),
          )}
          icon={CreditCard}
          color="blue"
          trend="Gross Credits"
        ...
        <StatsCard
          ...
          value={formatCurrency(
            filteredMemos.length > 0
              ? Math.max(...filteredMemos.map((m) => m.amount))
              : 0,
          )}
          icon={ArrowUpRight}
          color="amber"
            ...
            <Table>
              <TableHeader className="bg-slate-50/50">
                <TableRow className="hover:bg-transparent border-b border-slate-100">
                  ...
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.3em] py-8 text-slate-400/80 min-w-[130px]">
                    Memo #
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400/80 min-w-[220px]">
                    Customer & Salesman Representative
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400/80 min-w-[200px]">
                    COA & Remarks
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400/80 min-w-[160px]">
                    Memo Amount
                  </TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400/80 text-right pr-12 min-w-[160px]">
                    Decision
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginatedMemos.map((memo) => (
                  <TableRow
                    key={memo.id}
                    className={`group hover:bg-amber-50/30 transition-all duration-500 border-b border-slate-50 ${selectedIds.has(memo.id) ? "bg-amber-50/50" : ""}`}
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            variant="outline"
                            className="text-[9px] font-black uppercase tracking-widest border-slate-200 text-slate-500 bg-white"
                          >
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1.5">
                        ...
                        <p className="text-[10px] font-black text-slate-400 ml-9 uppercase tracking-widest opacity-70 italic leading-none">
                          Rep: {memo.salesman_id.salesman_code} -{" "}
                          {memo.salesman_id.salesman_name}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          <Badge
                            variant="outline"
                            className="bg-blue-50 text-blue-700 border-blue-100 text-[9px] font-black px-2 py-0.5 rounded-lg border shadow-sm shadow-blue-500/5"
                          >
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-10 duration-500">
          <div className="bg-slate-900 text-white rounded-3xl p-3 pl-8 shadow-2xl shadow-slate-900/40 border border-slate-800 flex items-center gap-8 ring-8 ring-slate-950/5 backdrop-blur-xl">
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl font-black tabular-nums text-amber-500">
                  {selectedIds.size}
                </span>
      ...
      <Dialog open={showBulkConfirm} onOpenChange={setShowBulkConfirm}>
        <DialogContent
          ...
          className="w-[95vw] sm:max-w-2xl p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl"
        >
...
function StatsCard({ label, value, icon: Icon, color, trend }: StatsCardProps) {
  const colorStyles = {
    amber:
      "from-amber-400 to-orange-500 text-amber-600 bg-amber-50 shadow-amber-500/10 ring-amber-100",
    blue: "from-blue-500 to-indigo-600 text-blue-600 bg-blue-50 shadow-blue-200/50 ring-blue-100",
    green:
      "from-emerald-400 to-green-500 text-emerald-600 bg-emerald-50 shadow-emerald-500/10 ring-emerald-100",
  };
  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-slate-200/80 border border-slate-100 relative overflow-hidden group hover:scale-[1.02] transition-all duration-700">
      ...
      <div className="relative z-10 space-y-8">
        <div
          className={`h-14 w-14 rounded-2xl flex items-center justify-center bg-gradient-to-br shadow-2xl ${colorStyles[color].split(" text")[0]} text-white`}
        >
          <Icon className="h-7 w-7" />
        </div>

        <div className="space-y-1.5 pt-2">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 opacity-80">
            {label}
          </p>
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-slate-50">
          <div
            className={`h-2.5 w-2.5 rounded-full animate-pulse shadow-sm transition-colors ${color === "amber" ? "bg-amber-500 shadow-amber-200" : color === "blue" ? "bg-blue-500 shadow-blue-200" : "bg-emerald-500 shadow-emerald-200"}`}
          />
          <span className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400 italic">
            {trend}
          </span>
        </div>
      </div>

// src/modules/industrial-distribution-system/financial-management/accounting/customers-memo/components/CollectionAllocationTable.tsx
...
const CollectionRow = React.memo(function CollectionRow({
  ...
  return (
    <React.Fragment>
      <TableRow
        className={`${isAllocated ? "bg-blue-50/50" : "hover:bg-muted/30"} transition-colors cursor-pointer group`}
        onClick={() => onToggleExpand(col.collectionId)}
      >
        ...
        <TableCell className="font-black text-sm tracking-tight">
          {col.collectionNo}
        </TableCell>
        <TableCell className="font-mono text-xs font-bold">
          ₱
          {col.totalAmount.toLocaleString(undefined, {
            ...
            <div className="p-6 pl-14 border-l-[6px] border-l-blue-600/30 space-y-4 bg-gradient-to-r from-muted/5 to-transparent">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-black uppercase tracking-widest text-muted-foreground">
                  Invoice Availability for{" "}
                  <span className="text-blue-600">#{col.collectionNo}</span>
                </h4>
                {isAllocated && (
                  <Badge
                    variant="outline"
                    className="text-[10px] font-bold border-blue-200 text-blue-700 bg-blue-50/50"
                  >
                    Total Linked: ₱
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {col.invoices.map((inv) => {
                  const invAlloc = allocation?.invoices.find(
                    (i) => i.invoiceId === inv.collectionDetailId,
                  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-muted-foreground font-bold italic">
                            ₱
                            {inv.netAmount.toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                            })}
                          </span>
                        </div>
                        <div
                          className={`p-1 rounded-full ${isActive ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground opacity-30"}`}
                        >
                          {isActive ? (
                            <svg
                      ...
                      {isActive && (
                        <div
                          className="mt-4 pt-4 border-t border-blue-50/80 animate-in zoom-in-95 duration-200"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <Label className="text-[10px] font-black uppercase tracking-tighter text-blue-600">
                              Applied Amount
                            </Label>
                          ...
                          <div className="relative">
                            <span className="absolute left-2.5 top-2 text-xs font-black text-blue-600 opacity-50">
                              ₱
                            </span>
                            <Input
                              type="number"
...
export const CollectionAllocationTable = React.memo(
  function CollectionAllocationTable({
    groupedData,
    allocations,
    loadingLookup,
    hasSearched,
                  ...
                  {groupedData.map((col) => (
                    <CollectionRow
                      ...
                      allocation={allocations.find(
                        (a) => a.collectionId === col.collectionId,
                      )}

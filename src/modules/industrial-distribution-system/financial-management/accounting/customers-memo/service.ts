// src/modules/industrial-distribution-system/financial-management/accounting/customers-memo/service.ts
...
export async function fetchNextMemoNumber(shortcut: string): Promise<string> {
  ...
  const res = await fetch(
    `${API_BASE}?action=next-memo-number&shortcut=${shortcut}`,
  );
  const json = await res.json();
...
export async function fetchMemosByStatus(
  status: string,
): Promise<MemoApprovalRow[]> {

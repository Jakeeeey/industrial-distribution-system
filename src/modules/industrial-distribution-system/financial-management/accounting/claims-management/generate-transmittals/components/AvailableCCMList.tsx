// src/modules/industrial-distribution-system/financial-management/claims-management/generate-transmittals/components/AvailableCCMList.tsx
"use client";
...
export function AvailableCCMList({
  ...
  const [customerFilter, setCustomerFilter] = React.useState<number | "all">(
    "all",
  );
  const visibleRows = React.useMemo(() => {
    return rows.filter((r) => {
      if (coaFilter !== "all" && Number(r.coa_id) !== coaFilter) return false;
      if (customerFilter !== "all" && Number(r.customer_id) !== customerFilter)
        return false;
      return true;
    });
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setCoaFilter(v === "all" ? "all" : Number(v))
              }
        ...
        <div className="flex flex-col gap-1">
          ...
          <Select
            ...
            onValueChange={(v) =>
              setCustomerFilter(v === "all" ? "all" : Number(v))
            }
            disabled={
              loading || rows.length === 0 || customerOptions.length === 0
            }
          >
            <SelectTrigger className="h-9 w-full sm:w-[420px]">

...
export async function searchSuppliers(q: string, signal?: AbortSignal) {
    const url = `/api/ids/fm/accounting/claims/ccm/suppliers?q=${encodeURIComponent(q)}`;
    const json = await getJson<{ data: LookupItem[] }>(url, signal);
    return json.data ?? [];
}

export async function searchCustomers(q: string, signal?: AbortSignal) {
    const url = `/api/ids/fm/accounting/claims/ccm/customers?q=${encodeURIComponent(q)}`;
    const json = await getJson<{ data: LookupItem[] }>(url, signal);
    return json.data ?? [];
}
export async function getChartOfAccount(coaId: number, signal?: AbortSignal) {
    const url = `/api/ids/fm/accounting/claims/ccm/chart-of-accounts?id=${encodeURIComponent(String(coaId))}`;
...
export async function getBalanceType(typeId: number, signal?: AbortSignal) {
    const url = `/api/ids/fm/accounting/claims/ccm/balance-types?id=${encodeURIComponent(String(typeId))}`;

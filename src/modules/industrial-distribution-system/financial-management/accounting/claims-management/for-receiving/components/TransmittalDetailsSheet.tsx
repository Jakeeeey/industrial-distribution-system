// src/modules/industrial-distribution-system/financial-management/claims-management/for-receiving/components/TransmittalDetailsSheet.tsx
"use client";
...
function safeMemoNo(memo_number: unknown, customer_memo_id: unknown): string {
  const raw =
    typeof memo_number === "string" ? memo_number.trim() : memo_number;
  if (!raw || raw === "null" || raw === "undefined")
    return `CCM #${String(customer_memo_id ?? "")}`;
  return String(raw);
}
...
export function TransmittalDetailsSheet({
  ...
  const { rows, loading, error, reload } = useTransmittalDetails(
    id ?? 0,
    open && !!id,
  );
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm">
                      {transmittal.created_at
                        ? formatDateTime(transmittal.created_at)
                        : "—"}
                    </div>
                  </div>
                      ...
                      <TableBody>
                        {loading ? (
                          ...
                          rows.map((raw) => {
                            ...
                            const rowIdSafe = Number.isFinite(rowId)
                              ? rowId
                              : 0;

                            const memoNo = safeMemoNo(
                              r.memo_number,
                              r.customer_memo_id,
                            );
                            const isRemoving = busyRemove === rowIdSafe;

                            const creditDate = (r.credit_date ??
                              r.created_at ??
                              null) as string | null;

                            const supplier =
                              safeText(r.supplier_name).trim() || "—";
                            const customer =
                              safeText(r.customer_name).trim() || "—";
                            const remarks =

// src/modules/industrial-distribution-system/financial-management/accounting/supplier-credit-memo/components/MemoTable.tsx
...
function StatusPill({ status }: { status: string }) {
  ...
  const style =
    STATUS_STYLES[key] ??
    STATUS_STYLES[key.charAt(0).toUpperCase() + key.slice(1).toLowerCase()];
...
export function MemoTable({
  ...
  accounts.forEach((a) => {
    const key = String(
      "id" in a && typeof (a as { id?: number }).id === "number"
        ? (a as { id: number }).id
        : (a.coa_id ?? a.coa_id),
    );
  ...
  const filtered = q
    ? memos.filter(
        (m) =>
          ...
          (supplierMap[String(m.supplier_id)] ?? "")
            .toLowerCase()
            .includes(q) ||
          (accountMap[String(m.chart_of_account)] ?? "")
            .toLowerCase()
            .includes(q) ||
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              <TableHead className="text-xs font-bold py-3 pl-6 whitespace-nowrap">
                Memo No.
              </TableHead>
              <TableHead className="text-xs font-bold py-3 whitespace-nowrap">
                Date
              </TableHead>
              <TableHead className="text-xs font-bold py-3 whitespace-nowrap">
                Supplier Name
              ...
              <TableHead className="text-xs font-bold py-3 text-right whitespace-nowrap">
                Amount
              </TableHead>
              <TableHead className="text-xs font-bold py-3 pr-6 whitespace-nowrap">
                Status
              </TableHead>
            </TableRow>
          </TableHeader>
            ...
            {loading ? (
              ...
              paged.map((m, i) => (
                ...
                >
                  ...
                  <TableCell
                    className="text-xs font-medium py-3 max-w-[180px] truncate"
                    title={supplierMap[String(m.supplier_id)]}
                  >
                  ...
                  <TableCell
                    className="text-xs text-muted-foreground py-3 max-w-[200px] truncate"
                    title={m.reason ?? ""}
                  >
                    {m.reason || (

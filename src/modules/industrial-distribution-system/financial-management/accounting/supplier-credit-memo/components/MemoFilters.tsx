// src/modules/industrial-distribution-system/financial-management/accounting/supplier-credit-memo/components/MemoFilters.tsx
...
interface MemoFiltersProps {
  ...
  hasFilters: boolean;
  onChange: <K extends keyof FiltersType>(
    key: K,
    value: FiltersType[K],
  ) => void;
  onClear: () => void;
...
export function MemoFiltersBar({
  ...
  const filteredAccounts = accounts.filter(
    (a) =>
      a.account_title?.toLowerCase().includes(accountSearch.toLowerCase()) ??
      false,
  );
    ...
    <div className="flex items-center justify-between gap-2 w-full flex-wrap">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Select
          ...
          onValueChange={(val) =>
            onChange("supplier_id", val === "all" ? "" : val)
          }
        >
        ...
        <Select
          ...
          onValueChange={(val) =>
            onChange("chart_of_account", val === "all" ? "" : val)
          }
        >
          <SelectTrigger className="h-9 w-[180px] text-xs">

// src/modules/industrial-distribution-system/financial-management/accounting/supplier-debit-memo/hooks/useSupplierDebitMemo.ts
...
import type {
  SupplierDebitMemo as SupplierMemo,
  Supplier,
  ChartOfAccount,
  CreateDebitMemoPayload,
  MemoFilters,
} from "../types";

const API_PATH = "/api/ids/fm/accounting/supplier-debit-memo";
...
export function useSupplierDebitMemo() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setAllMemos(data);
      toast.success("Supplier debit memos loaded successfully", {
        id: toastId,
      });
    } catch (e) {
  ...
  const memos = useMemo(() => {
    return allMemos.filter((m) => {
      ...
      if (filters.search) {
        ...
        const matches =
          m.memo_number?.toLowerCase().includes(search) ||
      ...
      // Supplier ID filter
      if (filters.supplier_id && String(m.supplier_id) !== filters.supplier_id)
        return false;

      // Chart of Account filter
      if (
        filters.chart_of_account &&
        String(m.chart_of_account) !== filters.chart_of_account
      )
        return false;
  ...
  const updateFilter = <K extends keyof MemoFilters>(
    key: K,
    value: MemoFilters[K],
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const allSuppliers = extractList<Supplier & { supplier_type?: string }>(
          json,
        );
        const tradeSuppliers = allSuppliers.filter(
          (s) => s.supplier_type === "TRADE",
        );

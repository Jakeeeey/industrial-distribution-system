// src/modules/industrial-distribution-system/financial-management/accounting/accounts-receivable/components/InvoiceTable.tsx
    ...
    <Card className="dark:bg-zinc-950 border-border overflow-hidden w-full">
      <CardHeader className="bg-muted/30 border-b border-border/50 flex flex-row items-center justify-between gap-4">
        ...
        <span className="text-xs text-muted-foreground shrink-0">
          {filtered.length} result{filtered.length !== 1 ? "s" : ""} &mdash;
        ...
        <Table className="w-full table-fixed">
          <TableHeader>
            <TableRow className="bg-muted/50">
              <TableHead className="text-xs font-bold py-3 pl-4 w-[9%] whitespace-nowrap">
                Invoice #
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[7%] whitespace-nowrap">
                Order No.
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[14%] whitespace-nowrap">
                Customer
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[10%] whitespace-nowrap">
                Salesman
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[9%] whitespace-nowrap">
                Branch
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[8%] whitespace-nowrap">
                Invoice Date
              </TableHead>
              <TableHead className="text-xs font-bold py-3 w-[8%] whitespace-nowrap">
                Due Date
              </TableHead>
              <TableHead className="text-xs font-bold py-3 text-right w-[9%] whitespace-nowrap">
                Net Recv.
              </TableHead>
              <TableHead className="text-xs font-bold py-3 text-right w-[8%] whitespace-nowrap">
                Paid
              </TableHead>
              <TableHead className="text-xs font-bold py-3 text-right w-[9%] whitespace-nowrap">
                Outstanding
              </TableHead>
              <TableHead className="text-xs font-bold py-3 text-center w-[6%] whitespace-nowrap">
                Overdue
              </TableHead>
              <TableHead className="text-xs font-bold py-3 pr-4 w-[6%] whitespace-nowrap">
                Status
              </TableHead>
            </TableRow>
          </TableHeader>
              ...
              paged.map((inv, i) => (
                ...
                >
                  ...
                  <TableCell className="py-3 pl-4">
                    <span
                      className="font-bold text-primary text-xs truncate block w-full"
                      title={inv.invoiceNo}
                    >
                      {inv.invoiceNo}
                    </span>
                  ...
                  <TableCell className="py-3">
                    <span
                      className="text-[11px] text-muted-foreground truncate block w-full"
                      title={inv.salesman}
                    >
                      {inv.salesman || (
                        <span className="text-muted-foreground/40">—</span>

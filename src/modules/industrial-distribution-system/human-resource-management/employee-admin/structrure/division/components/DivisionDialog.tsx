...
import { SingleDatePicker } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/structrure/department/components/SingleDatePicker";
...
export function DivisionDialog({
  ...
  const availableDepartments = departments.filter(
    (d) => !assignments.some((a) => a.department_id === d.department_id),
  );
  ...
  const updateBankAssignment = (deptId: number, bankId: string) => {
    ...
    setAssignments((prev) =>
      prev.map((a) =>
        a.department_id === deptId ? { ...a, bank_id: parsedBankId } : a,
      ),
    );
  };
              ...
              <div className="space-y-3 mt-4">
                {assignments.length === 0 ? (
                  <div className="text-center py-8 border rounded-md border-dashed text-muted-foreground text-sm bg-muted/20">
                    No departments selected. Use the search above to add
                    departments.
                  </div>
                ) : (
                        ...
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center justify-between">
                            <h5 className="font-medium text-sm">
                              {getDeptName(assignment.department_id)}
                            </h5>
                            <Button
                              variant="ghost"
                              size="icon-xs"
                              ...
                              onClick={() =>
                                removeDepartment(assignment.department_id)
                              }
                              type="button"
                            >
                              <X className="h-4 w-4" />

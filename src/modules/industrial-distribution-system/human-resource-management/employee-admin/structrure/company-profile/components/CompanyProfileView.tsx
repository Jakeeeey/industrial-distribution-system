...
import { getDirectusAssetUrl } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/administrator/utils/utils";
...
export function CompanyProfileView({ onEdit }: CompanyProfileViewProps) {
  ...
  const tags = data.company_tags
    ? data.company_tags.split(",").map((t) => t.trim())
    : [];
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 pt-2">
                {tags.map((tag, idx) => (
                  <Badge
                    key={idx}
                    variant="secondary"
                    className="px-3 py-1 bg-slate-100 text-slate-600 border-none hover:bg-slate-200"
                  >
        ...
        <Card className="border-none shadow-sm hover:shadow-md transition-shadow bg-white">
          ...
          <CardContent className="space-y-4 pt-4">
            <InfoItem
              ...
              value={data.company_registrationNumber}
            />
            <InfoItem
            ...
            <InfoItem
              label="Date Admitted"
              value={data.company_dateAdmitted}
              icon={<Calendar className="w-4 h-4 mt-0.5 text-slate-400" />}
            />
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm hover:shadow-md transition-shadow bg-white">
          ...
          <CardContent className="space-y-4 pt-4">
            <InfoItem
              ...
              icon={<Phone className="w-4 h-4 mt-0.5 text-slate-400" />}
            />
            <InfoItem
              label="Primary Email"
              value={data.company_email}
              icon={<Mail className="w-4 h-4 mt-0.5 text-slate-400" />}
            />
            <InfoItem
              label="Outlook Email"
              value={data.company_outlook}
              icon={<Mail className="w-4 h-4 mt-0.5 text-slate-400" />}
            />

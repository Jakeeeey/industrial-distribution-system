...
import { UndertimeTable } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/undertime-request/components/UndertimeTable";
import { UndertimeRequestFilters } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/undertime-request/components/UndertimeRequestFilters";
import {
  fetchUndertimeRequests,
  approveOrRejectUndertimeRequest,
} from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/undertime-request/providers/fetchProvider";
import type { UndertimeRequestWithUser } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/undertime-request/type";
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Failed to load undertime requests";
  ...
  const handleApprove = async (undertimeId: number, remarks: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Failed to approve undertime request";
      toast.error(errorMessage);
      throw err;
    }
  };
  ...
  const handleReject = async (undertimeId: number, remarks: string) => {
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Failed to reject undertime request";
      toast.error(errorMessage);
      throw err;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          ...
          <span className="font-semibold">{requests.length}</span> undertime{" "}

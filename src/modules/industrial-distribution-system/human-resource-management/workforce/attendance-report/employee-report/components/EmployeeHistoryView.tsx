// src/modules/industrial-distribution-system/human-resource-management/workforce/attendance-report/employee-report/components/EmployeeHistoryView.tsx
"use client";
...
function StatusBadge({ status }: { status: string }) {
  if (PRESENT_STATUSES.has(status))
    return (
      <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold border border-green-500 text-green-600 bg-green-50">
        Present
      </span>
    );
  if (status === "Rest Day")
  ...
  if (status === "Holiday" || status === "Leave")
    return (
      <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-600 border border-blue-200">
        {status}
      </span>
    );
  if (status === "Incomplete")
    return (
      <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-700 border border-yellow-300">
        Incomplete
      </span>
    );
  return (
    <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-50 text-red-600 border border-red-200">
      {status || "Absent"}
    </span>
  );
}
...
export function EmployeeHistoryView({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 ml-auto flex-wrap">
          ...
          <Button
            ...
            onClick={async () => {
              const { exportEmployeeHistoryToPDF } =
                await import("../utils/exportEmployeePDF");
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <label className="flex items-center gap-1.5 text-xs cursor-pointer select-none mb-1">
          <input
            ...
            checked={showRestDay}
            onChange={(e) => {
              setShowRestDay(e.target.checked);
              setPage(1);
            }}
      ...
      <div
        className={`grid grid-cols-2 md:grid-cols-5 gap-4 transition-opacity duration-300 ${refreshing ? "opacity-60" : "opacity-100"}`}
      >
        <MetricCard
          title="Attended"
          value={summary.attended}
          sub={`${summary.attended} days`}
          icon={<Check className="h-5 w-5" />}
        />
        <MetricCard
          title="Absent"
          value={summary.absent}
          sub={`${summary.absent} days`}
          icon={<X className="h-5 w-5" />}
        />
        <MetricCard
          title="Work Hours"
          value={minsToHM(summary.workMins)}
          sub={`${summary.workMins}m`}
          icon={<Clock3 className="h-5 w-5" />}
        />
        <MetricCard
          title="Overtime"
          value={minsToHM(summary.otMins)}
          sub={`${summary.otMins}m`}
          icon={<Zap className="h-5 w-5" />}
        />
        <MetricCard
          title="Late"
          value={minsToHM(summary.lateMins)}
          sub={`${summary.lateMins}m`}
      ...
      <div
        className={`grid grid-cols-1 md:grid-cols-2 gap-4 transition-opacity duration-300 ${refreshing ? "opacity-60" : "opacity-100"}`}
      >
      ...
      <div
        className={`rounded-xl border border-border overflow-hidden bg-background transition-opacity duration-300 ${refreshing ? "opacity-60" : "opacity-100"}`}
      >
          ...
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {paginatedLogs.map((log) => {
                ...
                const isExpanded =
                  directusId !== null && expandedLog === directusId;
                const hasGeotag =
                  directusId !== null &&
                ...
                return (
                  <React.Fragment key={log.log_date}>
                    <tr
                      className={`border-b border-border transition-colors ${isExpanded ? "bg-muted/40" : "hover:bg-muted/30"}`}
                    >
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="font-semibold">
                          {fmtDate(log.log_date, "MMM d, yyyy")}
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {fmtDate(log.log_date, "EEEE")}
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-muted-foreground">
                        {!NEUTRAL_STATUSES.has(log.status)
                          ? (schedule ?? "—")
                          : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {fmtTime(log.lunch_start)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        {fmtTime(log.lunch_end)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        {fmtTime(log.break_start)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        {fmtTime(log.break_end)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {hasGeotag ? (
                          <button
                            onClick={() =>
                              setExpandedLog(isExpanded ? null : directusId)
                            }

...
export function useUserConfiguration() {
  ...
  const [activeUser, setActiveUser] = useState<UserSubsystemAccess | null>(
    null,
  );
  const [activeSubsystem, setActiveSubsystem] =
    useState<SubsystemRegistration | null>(null);
  ...
  const handleToggleAccess = useCallback(
    async (
      ...
      const subRegistry = subsystems.find(
        (s) => Number(s.id) === Number(subsystemId),
      );
      ...
      const newSlugs = authorized
        ...
        : user.authorized_subsystems.filter(
            (slug) => !slugsInSubsystem.includes(slug),
          );
      ...
      const newModuleIds = authorized
        ...
        : (user.authorized_module_ids || []).filter(
            (id) => !allModuleIds.includes(id),
          );

      const updates = {
        subsystemsToAdd: authorized ? [subPk] : [],
        subsystemsToRemove:
          !authorized && user.subsystemAccessIds[subPk]
            ? [user.subsystemAccessIds[subPk]]
            : [],
        modulesToAdd: [] as number[],
        modulesToRemove: !authorized
          ? allModuleIds
              .filter((id) => user.moduleAccessIds[id])
              .map((id) => user.moduleAccessIds[id])
          : [],
      };
      ...
      updateUserPermissions(
        ...
        newSubIds,
        newModuleIds,
        user.subsystemAccessIds,
        user.moduleAccessIds,
      );

      const success = await UserService.updatePermissions(
        userId,
        null,
        updates,
      );
  ...
  const handleConfigure = useCallback(
    (user: UserSubsystemAccess, subsystem: SubsystemRegistration) => {
      setActiveUser(user);
  ...
  const handleUpdatePermissions = useCallback(
    async (userId: string, newSubIds: number[], newModuleIds: number[]) => {
      const user = users.find((u) => u.user_id === userId);
      ...
      const modAdded = newModuleIds.filter(
        (id) => !currentModuleIds.includes(id),
      );
      const modRemoved = currentModuleIds.filter(
        (id) => !newModuleIds.includes(id),
      );

      const updates = {
        subsystemsToAdd: subAdded,
        subsystemsToRemove: subRemoved
          .map((id) => user.subsystemAccessIds[id])
          .filter(Boolean) as number[],
        modulesToAdd: modAdded,
        modulesToRemove: modRemoved
          .map((id) => user.moduleAccessIds[id])
          .filter(Boolean) as number[],
      };
      ...
      const activeSubSlug = newSubIds.includes(activeSubPk)
        ? [activeSubsystem.slug]
        : [];

      const newSlugs = Array.from(
        new Set([
          ...user.authorized_subsystems.filter(
            (s) => !allSlugsInActiveSub.includes(s),
          ),
          ...activeSubSlug,
          ...activeModSlugs,
        ]),

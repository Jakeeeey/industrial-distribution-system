...
export const createColumns = (
  ...
  ...subsystems.map(
    (sys): ColumnDef<UserSubsystemAccess> => ({
      id: sys.slug,
      header: () => (
      ...
      cell: ({ row }) => {
        const user = row.original;
        const isAuthorized = (user.authorized_subsystem_ids || []).includes(
          Number(sys.id),
        );
              ...
              <TooltipTrigger asChild>
                ...
                >
                  <Shield
                    className={cn(
                      "h-4 w-4",
                      isAuthorized ? "text-primary" : "text-muted-foreground",
                    )}
                  />
                </Button>
              </TooltipTrigger>
              {isAuthorized && (
                <TooltipContent side="right">
                  <p className="text-[10px] font-bold">
                    Configure {sys.title} Permissions
                  </p>
                </TooltipContent>
              )}
            </Tooltip>

...
import {
  ...
  ModuleRegistration,
} from "@/modules/industrial-distribution-system/human-resource-management/subsystem-registration/types";
...
import { extractAllIds } from "@/modules/industrial-distribution-system/human-resource-management/user-configuration/utils/permissionUtils";
...
export function PermissionsDialog({
  ...
  const toggleItem = (
    id: number,
    checked: boolean,
    item?: SubsystemRegistration | ModuleRegistration,
    parentIds: number[] = [],
  ) => {
    ...
    if (checked) {
      ...
      setLocalModules((prev) =>
        Array.from(new Set([...prev, ...idsToToggle, ...parentIds])),
      );
    } else {
  ...
  const handleSubsystemToggle = (checked: boolean) => {
    ...
    if (checked) {
      ...
      setLocalModules((prev) =>
        prev.filter((id) => !allModuleIds.includes(id)),
      );
    }
  };
  ...
  const handleSelectAllModules = () => {
    ...
    setLocalSubsystems((prev) => Array.from(new Set([...prev, subId])));
    setLocalModules((prev) =>
      Array.from(new Set([...prev, ...moduleIdsToSelect])),
    );
  };
  ...
  const handleClearAllModules = () => {
    ...
    setLocalModules((prev) =>
      prev.filter((id) => !moduleIdsToClear.includes(id)),
    );
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="p-0 sm:max-w-[650px] h-[75vh] sm:h-[85vh] flex flex-col overflow-hidden rounded-2xl border shadow-2xl bg-background"
      >
        <DialogHeader className="p-5 pb-3 border-b bg-muted/20 shrink-0 flex-none">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5 font-medium uppercase tracking-wider">
            Configuring{" "}
            <span className="font-bold text-primary">{subsystem.title}</span>{" "}
            Access
          </p>
        </DialogHeader>
              ...
              {!isSubsystemChecked && (
                <div className="text-center py-10 bg-muted/5 rounded-2xl border border-dashed border-muted-foreground/20">
                  ...
                  <p className="text-[11px] text-muted-foreground font-medium">
                    Enable access to configure specific modules.
                  </p>
                </div>
              )}
        ...
        <DialogFooter className="p-5 py-4 border-t bg-muted/10 shrink-0 flex-none flex items-center justify-end gap-2">
          <Button
            ...
            className="h-9 px-4 rounded-xl text-xs font-bold opacity-60 hover:opacity-100"
          >
            CANCEL
          </Button>
          <Button
            onClick={handleSave}
            className="h-9 px-8 rounded-xl font-black shadow-xl shadow-primary/20 bg-primary text-xs tracking-widest"
          >
            APPLY
          </Button>
        </DialogFooter>
...
function ModulePermissionItem({
  ...
  const allChildrenChecked =
    hasChildren &&
    module.subModules?.every((s) => authorizedModuleIds.includes(Number(s.id)));
      ...
      <div className="flex items-center space-x-3 group">
        <Switch
          ...
          onCheckedChange={(checked) =>
            onToggle(moduleId, !!checked, module, parentIds)
          }
        />
        <label

...
import { SubsystemRegistration } from "@/modules/industrial-distribution-system/human-resource-management/subsystem-registration/types";
...
export function UserConfigurationTable({
  ...
  const columns = useMemo(
    () => createColumns(subsystems, onToggleAccess, onConfigure),
    [subsystems, onToggleAccess, onConfigure],
  );
                    ...
                    {headerGroup.headers.map((header, index) => (
                      <TableHead
                        ...
                        className={cn(
                          ...
                          index === 0 &&
                            "sticky left-0 bg-muted/95 backdrop-blur-md z-30 border-r-2 border-primary/10 shadow-[2px_0_10px_-4px_rgba(0,0,0,0.1)]",
                        )}
                      >
              ...
              <TableBody>
                {table.getRowModel().rows?.length ? (
                  table.getRowModel().rows.map((row) => (
                    <TableRow
                      key={row.id}
                      className="hover:bg-primary/[0.01] transition-colors border-b-muted-foreground/5 last:border-b-0"
                    >
                      {row.getVisibleCells().map((cell, index) => (
                        <TableCell
                          ...
                          className={cn(
                            ...
                            index === 0 &&
                              "sticky left-0 bg-card z-20 shadow-[4px_0_10px_-5px_rgba(0,0,0,0.1)] border-r-2 border-primary/5",
                          )}
                        >
                          {flexRender(

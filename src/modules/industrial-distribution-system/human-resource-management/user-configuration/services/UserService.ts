...
export class UserService {
  ...
  static async getUsers(
    ...
    offset = 0,
    search = "",
  ): Promise<{ users: UserSubsystemAccess[]; total: number }> {
  ...
  /**
     ...
     */
  static async getPermissionsForUsers(
    userIds: string[],
  ): Promise<
    Record<
      string,
  ...
  > {
    ...
    try {
      ...
      const [subResult, modResult] = await Promise.all([
        subRes.json(),
        modRes.json(),
      ]);
      ...
      subData.forEach(
        (row: {
          ...
          subsystem_id?: { slug?: string; id?: number };
        }) => {
      ...
      modData.forEach(
        (row: {
          id: number;
          user_id: string | number;
          module_id?: { slug?: string; id?: number };
        }) => {
          const uid = String(row.user_id);
          if (!mapping[uid])
            mapping[uid] = {
      ...
      if (updates.subsystemsToAdd.length > 0) {
        promises.push(
          fetch(`/api/ids/hrm/user-configuration/user-access-subsystems`, {
            ...
            body: JSON.stringify(
              updates.subsystemsToAdd.map((id) => ({
                user_id: userId,
                subsystem_id: id,
  ...
  static async getSubsystemsRegistry(): Promise<SubsystemRegistration[]> {
    ...
    try {
      ...
      const finalSubsystems = (subsystems || []).map(
        (sub: SubsystemRegistration) => {
          ...
          subModulesForThisSub.forEach((m) => {
            const parentId = m.parent_module_id
              ? Number(m.parent_module_id)
              : null;
            if (parentId && modulesById[parentId]) {
              if (!modulesById[parentId].subModules)
                modulesById[parentId].subModules = [];

              // Prevent duplicates
              const alreadyExists = modulesById[parentId].subModules?.some(
                (child) => child.id === m.id,
              );
              if (!alreadyExists) {
      ...
      console.error(
        "Error fetching subsystems registry for User Config:",
        error,
      );
      return [];
    }

...
export const InvoicingService = {
    ...
    async updateSalesOrder(orderId: number, data: Record<string, unknown>): Promise<{ success: boolean }> {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.details || errorData.error || "Failed to update sales order");
        }
        return response.json();
    },
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (errorData.details && Array.isArray(errorData.details)) {
                    errorMsg = errorData.details.map((d: { message?: string }) => d.message || JSON.stringify(d)).join(", ");
                } else {
    ...
    async getInvoiceDetails(invoiceId: number | string): Promise<Record<string, unknown>[]> {

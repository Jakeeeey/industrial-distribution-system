...
export function PhysicalInventoryGlobalRFIDScannerDialog(props: Props) {
    ...
    (
      ...
      signalResetTimerRef.current = window.setTimeout(() => {
        ...
        setLastSignalMessage(
          canEdit
            ? "Ready to scan. Present an RFID tag."
            : "Scanner is disabled.",
        );
        setLastScannedTag("");
        setScanAnimation("none");
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to load existing RFID tags.";
      toast.error(message);
    } finally {
  ...
  const processScan = React.useCallback(
    async (rawTag: string) => {
      ...
      try {
        ...
        if (!resolved.item) {
          const message =
            resolved.message || "RFID not found in on-hand records.";
          toast.error(message, {
            description: `RFID: ${normalized}`,
          });
          setTemporarySignal("error", message, normalized);
  ...
  const totalRfidEligibleRows = flattenedRows.filter(
    (row) => row.requires_rfid,
  ).length;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
            <div className="flex w-[min(640px,96vw)] max-w-2xl flex-col overflow-hidden rounded-2xl border bg-background shadow-2xl">
              ...
              <input
                ...
                onChange={(event) => {
                  scanBufferRef.current = extractHexCharacters(
                    event.target.value,
                  );
                }}
                onKeyDown={(event) => {

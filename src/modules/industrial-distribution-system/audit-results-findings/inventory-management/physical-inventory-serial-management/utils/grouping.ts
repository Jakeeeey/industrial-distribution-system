...
type BuildGroupedRowsParams = {
    ...
    ph_id?: number | null;
    serialCountByDetailId?: Record<number, number>;
};
...
export function buildGroupedPhysicalInventoryRows(
    ...
    for (const [familyKey, familyVariants] of familyMap.entries()) {
        ...
        const childRows: GroupedPhysicalInventoryChildRow[] = ordered.map((variant) => {
            ...
            const serialCount =
                detailId !== null
                    ? coalesceNumber(serialCountByDetailId[detailId], 0)
                    : 0;

            const isSerialRow = requiresSerial(variant.is_serialized);
            const physicalCount = isSerialRow
                ? serialCount

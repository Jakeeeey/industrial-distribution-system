//src/modules/supply-chain-management/physical-inventory-serial-management/components/PhysicalInventoryGlobalSerialScannerDialog.tsx
"use client";
...
export function PhysicalInventoryGlobalSerialScannerDialog(props: Props) {
    ...
    const [existingPiTags, setExistingPiTags] = React.useState<PhysicalInventoryDetailSerialRow[]>(
        [],
    );
        ...
        (
            ...
            signalResetTimerRef.current = window.setTimeout(() => {
                ...
                setLastSignalMessage(
                    canEdit ? "Ready to scan. Present a Serial tag." : "Scanner is disabled.",
                );
                setLastScannedTag("");
                setScanAnimation("none");
                resetScannerBuffer();
    ...
    const loadExistingPiTags = React.useCallback(async () => {
        ...
        try {
            ...
            const message =
                error instanceof Error ? error.message : "Failed to load existing Serial tags.";
            toast.error(message);
        } finally {
    ...
    React.useEffect(() => {
        ...
        setScannerSignal(canEdit ? "ready" : "idle");
        setLastSignalMessage(
            canEdit ? "Ready to scan. Present a Serial tag." : "This PI can no longer be edited.",
        );
        setLastScannedTag("");
        setScanAnimation("none");
        setSavedCountPulse(false);
    ...
    const hasDuplicateInCurrentPi = React.useCallback(
        (serialTag: string): boolean => {
            ...
            if (existingPiTags.some((row) => sameTag(row.serial_number, normalized))) {
                return true;
            }
        ...
        async (rawTag: string) => {
            ...
            try {
                ...
                if (matchedRow.requires_serial) {
                    ...
                    const nextVariance = computeVariance(nextSerialCount, matchedRow.system_count);
                    const nextDifferenceCost = computeDifferenceCost(
                        nextVariance,
                        matchedRow.unit_price,
                    );
                    const nextAmount = computeAmount(nextSerialCount, matchedRow.unit_price);
                ...
                const message =
                    error instanceof Error ? error.message : "Failed to process Serial scan.";
    ...
    const totalSerialEligibleRows = flattenedRows.filter((row) => row.requires_serial).length;
                                ...
                                onKeyDown={(event) => {
                                    ...
                                    if (event.key === "Enter") {
                                        ...
                                        finalizeScanTimerRef.current = window.setTimeout(() => {
                                            ...
                                            const finalizedFromInput = finalizeSerialTag(rawFromInput);
                                            const finalizedFromBuffer = finalizeSerialTag(rawFromBuffer);

...
export function PhysicalInventorySerialDialog(props: Props) {
  const { open, row, branchId, products = [], onOpenChange, onSaved } = props;

  const [tags, setTags] = React.useState<PhysicalInventoryDetailSerialRow[]>(
    [],
  );
  const [allPiTags, setAllPiTags] = React.useState<
    PhysicalInventoryDetailSerialRow[]
  >([]);
  ...
  const persistTag = React.useCallback(
    async (serialTag: string) => {
      ...
      if (!isValidExactSerialTag(serialTag)) {
        toast.error(`Invalid Serial tag. It cannot be empty.`);
        setSerialInput("");
        focusInput(true);
        return;
      }
      ...
      if (serialProductId !== null && serialProductId !== row?.product_id) {
        const mismatchedProduct = products.find(
          (p) => p.product_id === serialProductId,
        );
        const mismatchedName =
          mismatchedProduct?.product_name || `Product ID ${serialProductId}`;
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to add registered serials.";
      toast.error(message);
    } finally {
      setIsSaving(false);
    }
        ...
        <div className="shrink-0 px-6 pt-6 pb-2">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs sm:text-sm">
              Review saved Serial tags, manually add missing tags, or remove
              incorrect ones.
            </DialogDescription>
          </DialogHeader>
        </div>
                ...
                <div className="bg-amber-50/50 p-3 flex flex-col gap-2 border-b">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {pendingSerials.map((s) => (
                      <span
                        key={s}
                        className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-medium border border-amber-200"
                      >
                        {s}
                      </span>
                    ))}

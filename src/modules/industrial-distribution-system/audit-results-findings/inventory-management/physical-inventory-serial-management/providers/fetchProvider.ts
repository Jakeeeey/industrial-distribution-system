...
const API_BASE = "/api/ids/arf/inventory-management/physical-inventory";
...
async function apiPost<TPayload, TResult>(
  path: string,
  payload: TPayload,
): Promise<TResult> {
  const url = new URL(path, window.location.origin);
...
async function apiPatch<TPayload, TResult>(
  path: string,
  payload: TPayload,
): Promise<TResult> {
  const url = new URL(path, window.location.origin);
...
function mapRunningInventoryRow(
  row: RunningInventoryApiRow,
): RunningInventoryRow {
  return {
    id: row.id,
...
export async function fetchPhysicalInventoryList(): Promise<
  PhysicalInventoryHeaderRow[]
> {
  return directusGetItems<PhysicalInventoryHeaderRow>(
...
export async function fetchPhysicalInventoryById(
  ...
  return directusGetItem<PhysicalInventoryHeaderRow>(
    TABLES.physical_inventory,
    id,
    {
      fields:
...
export async function fetchPhysicalInventoryDetails(
  ...
  return directusGetItems<PhysicalInventoryDetailRow>(
...
export async function fetchPhysicalInventorySerialCountByHeader(
  ...
  const rows = await directusGetItems<
    Pick<PhysicalInventoryDetailSerialRow, "pi_detail_id">
  >(TABLES.physical_inventory_details_serial, {
    filter: JSON.stringify({
      pi_detail_id: { _in: detailIds },
    }),
...
export async function createPhysicalInventoryHeader(
  ...
  >(`/api/ids/arf/inventory-management/physical-inventory/header`, payload);
...
export async function updatePhysicalInventoryHeader(
  ...
  const json = await apiPatch<
    ...
    DirectusItemResponse<PhysicalInventoryHeaderRow>
  >(
    `/api/ids/arf/inventory-management/physical-inventory/header/${id}`,
    payload,
  );
...
export async function commitPhysicalInventory(
  ...
  const json = await apiPost<
    ...
    DirectusItemResponse<PhysicalInventoryHeaderRow>
  >(
    `/api/ids/arf/inventory-management/physical-inventory/header/${id}/commit`,
    {},
  );
...
export async function cancelPhysicalInventory(
  ...
  const json = await apiPost<
    ...
    DirectusItemResponse<PhysicalInventoryHeaderRow>
  >(
    `/api/ids/arf/inventory-management/physical-inventory/header/${id}/cancel`,
    {},
  );
...
export async function createPhysicalInventoryDetailsBulk(
  ...
  return directusPostItems<
    PhysicalInventoryDetailUpsertPayload,
    PhysicalInventoryDetailRow
  >(TABLES.physical_inventory_details, payloads);
}
...
export async function updatePhysicalInventoryDetailsBulk(
  ...
  >(`/api/arf/inventory-management/physical-inventory/details/bulk`, {
    updates,
  });
...
export async function updateCylinderAsset(
  ...
  return directusPatchItem<
    Partial<CylinderAssetUpsertPayload>,
    CylinderAssetRow
  >(TABLES.cylinder_assets, id, payload);
}
...
export async function fetchCylinderAssetBySerial(
  ...
  const rows = await directusGetItems<CylinderAssetRow>(
    TABLES.cylinder_assets,
    {
      filter: JSON.stringify({
...
export async function deletePhysicalInventoryDetailSerial(
  id: number,
): Promise<void> {
...
export function resolveRunningInventoryFilterParams(input: {
  ...
  if (!branch?.branch_name?.trim()) {
    throw new Error(
      "Unable to resolve branch name for running inventory filter.",
    );
  }
  ...
  if (!supplier?.supplier_shortcut?.trim()) {
    throw new Error(
      "Unable to resolve supplier shortcut for running inventory filter.",
    );
  }
  ...
  if (!category?.category_name?.trim()) {
    throw new Error(
      "Unable to resolve category name for running inventory filter.",
    );
  }
  ...
  return {
    ...
    ...(isAllCategory
      ? {}
      : { productCategory: category.category_name.trim() }),
  };
}
...
function buildNextPhNoFromLatest(
  latestPhNo: string | null | undefined,
): string {
...
export async function fetchNextPhysicalInventoryNumber(): Promise<string> {
  ...
  const rows = await directusGetItems<
    Pick<PhysicalInventoryHeaderRow, "id" | "ph_no">
  >(TABLES.physical_inventory, {
    fields: "id,ph_no",
    sort: "-ph_no",
    limit: "100",

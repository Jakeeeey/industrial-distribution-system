//src/modules/industrial-distribution-system/supply-chain-management/physical-inventory-list/components/PhysicalInventoryListTable.tsx
"use client";
...
function StatusBadge({
  ...
  status: PhysicalInventoryListRow["status"];
}) {
  if (status === "Committed") {
    return (
      <Badge className="bg-emerald-600 hover:bg-emerald-600">Committed</Badge>
    );
  }
...
function StockTypeBadge({ stockType }: { stockType: string | null }) {
  if (stockType === "BAD") {
    return (
      <Badge
        variant="outline"
        className="border-red-200 bg-red-50 text-red-700 hover:bg-red-50 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-300"
      >
        BAD STOCK
      </Badge>
    );
  ...
  return (
    <Badge
      variant="outline"
      className="border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-300"
    >
      GOOD STOCK
    </Badge>
  );
          ...
          return (
            <Card
              ...
              className={[
                ...
                isSelected
                  ? "border-primary bg-muted/30 ring-1 ring-primary/20"
                  : "",
              ].join(" ")}
              onClick={() => onOpen(row)}
            >
              <CardContent className="pt-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1 space-y-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      Start: {fmtDate(row.starting_date)} • Cutoff:{" "}
                      {fmtDate(row.cutOff_date)}
                    </p>
                  </div>

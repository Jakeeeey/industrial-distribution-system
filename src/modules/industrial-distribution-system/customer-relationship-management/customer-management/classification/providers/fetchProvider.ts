...
export async function fetchClassifications(
	...
	const response = await fetch(`/api/ids/crm/customer-management/classification?${query}`, {
		method: "GET",
		cache: "no-store",
	});
...
export async function createClassification(payload: UpsertClassificationPayload): Promise<void> {
	const response = await fetch("/api/ids/crm/customer-management/classification", {
		method: "POST",

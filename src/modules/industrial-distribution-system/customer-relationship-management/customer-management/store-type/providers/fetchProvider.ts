...
): Promise<StoreTypeListResponse> {
  const query = toQueryString(filters);
  const res = await fetch(
    `/api/ids/crm/customer-management/store-type?${query}`,
    {
      method: "GET",
      cache: "no-store",
    },
...
export async function createStoreType(
  payload: UpsertStoreTypePayload,
): Promise<void> {
  const res = await fetch("/api/ids/crm/customer-management/store-type", {
    method: "POST",
...
export async function updateStoreType(
  payload: UpdateStoreTypePayload,
): Promise<void> {
  const res = await fetch("/api/ids/crm/customer-management/store-type", {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },

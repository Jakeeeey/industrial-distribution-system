...
export function useDuplicateCustomers() {
  ...
  const [resolvedRecords, setResolvedRecords] = useState<
    { customer_id: number; group_id: string }[]
  >([]);

  // Load resolved records from database on mount
  const fetchResolutions = useCallback(async () => {
    try {
      const res = await fetch("/api/ids/crm/customer/duplicate");
      if (res.ok) {
  ...
  const fetchData = useCallback(async (showLoading = false) => {
    try {
      ...
      const res = await fetch(
        `/api/ids/crm/customer/scan?${params.toString()}`,
        {
          cache: "no-store",
        },
      );
      ...
      const data = await res.json();
      // Match the structure from our new /api/ids/crm/customer/scan endpoint
  ...
  const duplicateGroups = useMemo(() => {
    ...
    const dbGroups = resolvedRecords.reduce(
      (acc, rec) => {

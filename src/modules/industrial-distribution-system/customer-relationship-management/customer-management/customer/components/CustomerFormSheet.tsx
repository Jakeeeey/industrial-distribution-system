...
import { mapPriceTypeName } from "@/modules/industrial-distribution-system/financial-management/product-pricing-management/product-pricing/utils/constants";
...
const renderImagePreview = (imageId: string | null | undefined) => {
    ...
    const imageUrl = isUrl ? imageId : `/api/ids/crm/upload/asset?id=${encodeURIComponent(imageId)}`;
...
const customerSchema = z.object({
    ...
    price_type_id: z.preprocess(
        (val) => (val === "" || val === null || val === undefined || val === 0 || val === "0" ? undefined : Number(val)),
        z.number({ error: "Price type is required" })
    ),
...
const getDefaultValues = (): CustomerFormValues => ({
    ...
    payment_term: 0, store_type: undefined as unknown as number, classification: null, price_type: "", price_type_id: undefined as unknown as number, isActive: 1, isVAT: 0, isEWT: 0,
...
export function CustomerFormSheet({ open, onOpenChange, customer, onSubmit, defaultTab = "basic" }: CustomerFormSheetProps) {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.error("Failed to upload image. Please ensure the /api/ids/crm/upload endpoint exists.");
        } finally {
    ...
    useEffect(() => {
        ...
        const fetchPaymentTerms = async () => {
            ...
            try {
                const res = await fetch("/api/ids/crm/customer/references?type=payment_term");
    ...
    useEffect(() => {
        ...
        const fetchBankNames = async () => {
            ...
            try {
                const res = await fetch("/api/ids/crm/customer/references?type=bank_name");
    ...
    useEffect(() => {
        ...
        const fetchPriceTypes = async () => {
            ...
            try {
                const res = await fetch("/api/ids/crm/customer/references?type=price_type");
                if (!res.ok) throw new Error("Failed to fetch price types");
                const json = await res.json();
                if (isMounted) {
                    const fetchedTypes = json.data?.map((item: { price_type_id: number; price_type_name: string }) => ({
                        id: item.price_type_id,
                        name: item.price_type_name
                    })) || [];
                    setPriceTypes(fetchedTypes);

                    // 🚀 Auto-sync price_type_id if customer has price_type string ("A", "B", etc.) but no price_type_id
                    const currentPtId = form.getValues("price_type_id");
                    const currentPtName = form.getValues("price_type");
                    if (!currentPtId && currentPtName) {
                        const match = fetchedTypes.find((pt: { id: number; name: string }) =>
                            pt.name.trim().toUpperCase() === currentPtName.trim().toUpperCase()
                        );
                        if (match) {
                            form.setValue("price_type_id", match.id);
                        }
                    }
                }
            } catch {
        ...
        if (open) {
            if (customer) {
                form.reset({
                    ...
                    price_type_id: customer.price_type_id || undefined as unknown as number,

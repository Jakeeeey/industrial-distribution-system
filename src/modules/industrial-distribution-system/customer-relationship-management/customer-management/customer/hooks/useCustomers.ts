    ...
    async (showLoading = false) => {
      try {
        ...
        const [customerRes, userRes] = await Promise.all([
          fetch(`/api/ids/crm/customer?${params.toString()}`, {
            cache: "no-store",
          }),
          fetch("/api/ids/crm/customer/references?type=user", {
            cache: "no-store",
          }),
        ]);
      ...
      try {
        ...
        if (newAccounts) {
          ...
          const currentRes = await fetch(
            `/api/ids/crm/customer/bank-account?customer_id=${id}`,
          );
          ...
          for (const acc of toDelete) {
            const delRes = await fetch(
              `/api/ids/crm/customer/bank-account?id=${acc.id}`,

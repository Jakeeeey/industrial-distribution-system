    ...
    const fetchData = useCallback(async (showLoading = false) => {
        try {
            ...
            const [customerRes, userRes] = await Promise.all([
                fetch(`/api/ids/crm/customer?${params.toString()}`, { cache: "no-store" }),
                fetch("/api/ids/crm/customer/references?type=user", { cache: "no-store" })
            ]);
        ...
        try {
            ...
            if (bank_accounts && bank_accounts.length > 0) {
                await Promise.all(bank_accounts.map(async (account) => {
                    ...
                    const baRes = await fetch("/api/ids/crm/customer/bank-account", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
    ...
    const updateCustomer = useCallback(async (id: number, data: Partial<CustomerWithRelations>) => {
        try {
            ...
            if (newAccounts) {
                ...
                const currentRes = await fetch(`/api/ids/crm/customer/bank-account?customer_id=${id}`);
                ...
                for (const acc of toDelete) {
                    const delRes = await fetch(`/api/ids/crm/customer/bank-account?id=${acc.id}`, { method: "DELETE", cache: "no-store" });
                ...
                for (const acc of toPost) {
                    ...
                    const postRes = await fetch("/api/ids/crm/customer/bank-account", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                ...
                for (const acc of toPatch) {
                    ...
                    const patchRes = await fetch("/api/ids/crm/customer/bank-account", {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },

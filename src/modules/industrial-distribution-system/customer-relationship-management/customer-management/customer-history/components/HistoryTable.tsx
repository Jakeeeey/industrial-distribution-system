...
interface HistoryTableProps {
    data: CustomerHistoryRecord[];
    isLoading: boolean;
    onView: (customer: CustomerHistoryRecord) => void;
    page: number;
    ...
    if (isLoading) {
        return (
            <div className="w-full h-[400px] flex flex-col items-center justify-center bg-background border border-border/40 rounded-2xl shadow-sm">
                ...
                <p className="mt-4 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground animate-pulse">Loading Customer History...</p>
            </div>
        );
    }
    ...
    if (data.length === 0) {
        return (
            <div className="w-full h-[400px] flex flex-col items-center justify-center bg-background border border-dashed border-border/50 rounded-2xl shadow-inner">
                ...
                <h3 className="text-sm font-black uppercase tracking-tight italic">No Customer History Records Found</h3>
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">Try relaxing filters or search terms</p>
            </div>
        );
    }
        ...
        <div className="w-full space-y-4">
                ...
                <Table>
                    <TableHeader className="bg-muted/5 border-b border-border/60">
                        <TableRow className="hover:bg-transparent border-none">
                            ...
                            <TableHead className="text-[10px] font-black uppercase tracking-widest text-foreground/70 py-4 px-6">Customer Name</TableHead>
                    ...
                    <TableBody>
                        {data.map((record) => (
                            <TableRow key={record.id} className="group hover:bg-muted/30 border-b border-border/50 last:border-none transition-all">
                                <TableCell className="py-4 px-6">
                                    <span className="text-sm font-bold text-blue-600 hover:underline cursor-pointer" onClick={() => onView(record)}>
                                        {record.customer_code || `PENDING-${record.id}`}
                                    </span>
                                </TableCell>
                                <TableCell className="py-4 px-6">
                                    ...
                                    <div className="flex items-start gap-2 max-w-[200px]">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <p className="text-[10px] font-medium italic text-muted-foreground mt-1">
                                                {record.store_signage || "No signage recorded"}
                                            </p>
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell className="py-4 px-6 text-center">
                                    <Badge variant="outline" className={cn(
                                        ...
                                        record.type === "Employee" ? "bg-amber-100 text-amber-700" : "bg-blue-100/50 text-blue-700"
                                    )}>
                                        {record.type}
                                    </Badge>
                                </TableCell>
                                ...
                                <TableCell className="py-4 px-6">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
                                            ...
                                            {record.customer_email || "no-email@registry.com"}
                                        </div>
                                        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
                                ...
                                <TableCell className="py-4 px-6 text-center">
                                    {record.isActive === 1 ? (
                                        <Badge className="bg-emerald-600 text-white border-none rounded-full px-5 py-1 text-[10px] font-black uppercase tracking-widest shadow-md shadow-emerald-500/20">
                                            Active
                                        </Badge>
                                    ) : (
            ...
            <div className="px-10 py-6 flex items-center justify-between bg-muted/5 border-t border-border/40">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 flex items-center gap-2">
                    ...
                    Showing <span className="text-foreground">{data.length}</span> of <span className="text-foreground">{totalCount}</span> History Entries
                </p>

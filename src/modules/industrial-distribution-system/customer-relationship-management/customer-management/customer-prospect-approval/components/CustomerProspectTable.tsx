...
export function CustomerProspectTable({
  ...
  const [selectedProspect, setSelectedProspect] =
    useState<CustomerProspect | null>(null);
  ...
  const [activeComparisonGroup, setActiveComparisonGroup] =
    useState<SimilarityGroup | null>(null);
    ...
    try {
      if (action === "Approve") {
        ...
        toast.success("Prospect Approved", {
          description: "The prospect has been promoted to a Customer.",
        });
      } else {
        await onReject(id);
        toast.error("Prospect Rejected", {
          description: "The prospect request has been denied.",
        });
      }
    } catch (err) {
  ...
  const checkForDuplicates = async (prospect: CustomerProspect) => {
    ...
    try {
      ...
      const res = await fetch("/api/ids/crm/customer/scan?limit=1000");
  ...
  const fuzzyMatch = (stored: string, apiName: string) => {
    ...
    return (
      sNormalized === aNormalized ||
      sNormalized.includes(aNormalized) ||
      aNormalized.includes(sNormalized)
    );
  };
  ...
  useEffect(() => {
    ...
    const provObj = provincesList.find((p) =>
      fuzzyMatch(editForm.province || "", p.name),
    );
    if (!provObj) return;
  ...
  useEffect(() => {
    ...
    const cityObj = citiesList.find((c) =>
      fuzzyMatch(editForm.city || "", c.name),
    );
    if (!cityObj) return;
  ...
  useEffect(() => {
    ...
    const brgyObj = barangaysList.find((b) =>
      fuzzyMatch(editForm.brgy || "", b.name),
    );
    ...
    try {
      ...
      setSelectedProspect({
        ...
        store_type: editForm.store_type
          ? Number(editForm.store_type)
          : selectedProspect.store_type,
        classification: editForm.classification
          ? Number(editForm.classification)
          : selectedProspect.classification,
      });
        ...
        <Table>
          <TableHeader className="bg-muted/30 border-b">
            <TableRow className="hover:bg-transparent">
              <TableHead className="w-[250px] text-xs font-bold uppercase tracking-wider">
                Prospect Details
              </TableHead>
              ...
              <TableHead className="w-[150px] text-xs font-bold uppercase tracking-wider">
                Salesman
              </TableHead>
              <TableHead className="w-[120px] text-xs font-bold uppercase tracking-wider text-center">
                Status
              </TableHead>
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-1.5 mt-1 ml-5 text-[10px] text-muted-foreground uppercase tracking-tight">
                        ...
                        <Badge
                          variant="secondary"
                          className="px-1 py-0 h-3 text-[9px] font-bold"
                        >
                          {prospect.type}
                        </Badge>
                      </div>
              ...
              <div className="space-y-6">
                    ...
                    >
                      ...
                      <div className="relative h-44 w-44 rounded-3xl border-4 border-background overflow-hidden shadow-2xl bg-muted flex items-center justify-center">
                        ...
                        <img
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src =
                              ...
                              encodeURIComponent(
                                selectedProspect.customer_name || "Prospect",
                              ) +
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/40 to-transparent p-2 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="bg-white/20 backdrop-blur-md px-2 py-0.5 rounded text-[8px] font-bold text-white uppercase tracking-widest">
                            Click to Zoom
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="flex flex-col col-span-2">
                      ...
                      {isEditing ? (
                        ...
                        <span className="font-semibold">
                          {selectedProspect.customer_name || "None"}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span className="font-medium text-primary">
                        {selectedProspect.customer_code || "AUTO-ASSIGN"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">
                      ...
                      <Badge
                        variant="outline"
                        className="w-fit h-5 text-[10px] font-bold"
                      >
                    ...
                    {!isEditing && (
                      <div className="flex flex-col">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {selectedProspect.prospect_date
                            ? new Date(
                                selectedProspect.prospect_date,
                              ).toLocaleDateString()
                            : "None"}
                    ...
                    <div className="flex flex-col col-span-2 pt-2 border-t mt-1 border-dashed">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <StatusBadge
                          status={selectedProspect.prospect_status}
                        />
                        {selectedProspect.prospect_status !== "Pending" &&
                          selectedProspect.updated_by_name && (
                ...
                <section className="space-y-4">
                  ...
                  <div className="rounded-xl overflow-hidden border border-success/20 shadow-sm">
                    <ProspectMapViewer
                      ...
                      address={[
                        selectedProspect.brgy,
                        selectedProspect.city,
                        selectedProspect.province,
                      ]
                        .filter(Boolean)
                        .join(", ")}
                    />
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="flex flex-col">
                      ...
                      <span className="text-info font-medium">
                        {selectedProspect.contact_number || "None"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">
                      ...
                      <span className="text-info font-medium truncate">
                        {selectedProspect.customer_email || "None"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase">
                ...
                <section className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      {isEditing ? (
                        ...
                        <>
                          <span className="font-semibold text-base">
                            {selectedProspect.store_name || "None"}
                          </span>
                          <span className="text-xs text-muted-foreground italic">
                            {selectedProspect.store_signage || "No signage"}
                          </span>
                        </>
                      )}
                    </div>
                    <div className="flex flex-col col-span-2">
                      <Label className="text-[10px] font-bold text-muted-foreground uppercase mb-1">
                        Full Address (Province, City, Brgy)
                      </Label>
                      {isEditing ? (
                        ...
                        <span className="text-xs leading-tight">
                          {selectedProspect.province || "N/A"},{" "}
                          {selectedProspect.city || "N/A"},{" "}
                          {selectedProspect.brgy || "N/A"}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col">
                      ...
                      {isEditing ? (
                        <SearchableSelect
                          options={storeTypes.map((st) => ({
                            value: st.id.toString(),
                            label: st.store_type,
                          }))}
                          value={editForm.store_type?.toString()}
                          onValueChange={(v) =>
                            setEditForm({ ...editForm, store_type: Number(v) })
                          }
                        ...
                        <span className="font-medium">
                          {storeTypes.find(
                            (st) =>
                              st.id === Number(selectedProspect.store_type),
                          )?.store_type || "None"}
                        </span>
                      )}
                    </div>
                      ...
                      {isEditing ? (
                        <SearchableSelect
                          options={classifications.map((cl) => ({
                            ...
                            label: cl.classification_name,
                          }))}
                          value={editForm.classification?.toString()}
                          onValueChange={(v) =>
                            setEditForm({
                              ...editForm,
                              classification: Number(v),
                            })
                          }
                        ...
                        <span className="font-medium">
                          {classifications.find(
                            (c) =>
                              c.id === Number(selectedProspect.classification),
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="flex flex-col">
                      ...
                      {isEditing ? (
                        ...
                        <span className="font-mono font-medium">
                          {selectedProspect.customer_tin || "None"}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        {selectedProspect.isVAT === 1 && (
                          <Badge className="bg-success-bg text-success hover:bg-success-bg border-success/20 text-[9px] h-4 font-bold uppercase">
                            VAT
                          </Badge>
                        )}
                        {selectedProspect.isEWT === 1 && (
                          <Badge className="bg-warning-bg text-warning hover:bg-warning-bg border-warning/20 text-[9px] h-4 font-bold uppercase">
                            EWT
                          </Badge>
                        )}
                        {!selectedProspect.isVAT && !selectedProspect.isEWT && (
                          <span className="text-xs text-muted-foreground italic">
                            Non-Vatable
                          </span>
                        )}
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-medium">
                        {paymentTerms.find(
                          (pt) =>
                            pt.id === Number(selectedProspect.payment_term),
                        )?.payment_name ||
                          (selectedProspect.payment_term
                            ? `Term ID: ${selectedProspect.payment_term}`
                            : "None")}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span
                        ...
                        title={selectedProspect.bank_details || ""}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-medium text-success">
                        {discountTypes.find(
                          (dt) =>
                            dt.id === Number(selectedProspect.discount_type),
                        )?.discount_type ||
                          (selectedProspect.discount_type
                            ? `ID: ${selectedProspect.discount_type}`
                            : "None")}
                      </span>
                    </div>
                    <div className="flex flex-col">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-medium text-success">
                        {storeTypes.find(
                          (st) => st.id === Number(selectedProspect.store_type),
                        )?.store_type ||
                          (selectedProspect.store_type
                            ? `ID: ${selectedProspect.store_type}`
                            : "None")}
                      </span>
                    </div>
                  </div>
                ...
                {selectedProspect.prospect_status === "Pending" && (
                  <>
                    <Button
                      ...
                      onClick={() =>
                        selectedProspect &&
                        handleAction(selectedProspect.id, "Reject")
                      }
                    ...
                    >
                      {processingId === selectedProspect?.id ? (
                        <Loader2 className="h-4 w-4 animate-spin mr-2" />
                      ) : (
                    ...
                    <Button
                      onClick={() =>
                        selectedProspect &&
                        handleAction(selectedProspect.id, "Approve")
                      }
                      disabled={processingId !== null}
      ...
      <Dialog open={isZoomOpen} onOpenChange={setIsZoomOpen}>
        ...
        >
          <DialogHeader className="sr-only">
            ...
            <DialogDescription>
              Full scale view of the customer profile image.
            </DialogDescription>
          </DialogHeader>

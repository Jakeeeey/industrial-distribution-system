...
export function useCustomerProspects(): UseCustomerProspectsReturn {
  ...
  const [metadata, setMetadata] = useState<
    CustomerProspectsAPIResponse["metadata"]
  >({
    total_count: 0,
    filter_count: 0,
    page: 1,
    ...
    async (showLoading = false) => {
      try {
        ...
        const res = await fetch(
          `/api/ids/crm/customer-prospect?${params.toString()}`,
          { cache: "no-store" },
        );
        ...
        if (
          ...
          const [dtRes, slRes, stRes, ptRes, clRes] = await Promise.all([
            fetch("/api/ids/crm/customer/references?type=discount_type"),
            fetch("/api/ids/crm/customer/references?type=salesman"),
  ...
  const handleAction = useCallback(
    async (id: number, action: "Approve" | "Reject") => {
      try {
        const res = await fetch("/api/ids/crm/customer-prospect", {
          method: "POST",
  ...
  const updateProspect = useCallback(
    async (id: number, data: Partial<CustomerProspect>) => {
      try {
        const res = await fetch("/api/ids/crm/customer-prospect", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },

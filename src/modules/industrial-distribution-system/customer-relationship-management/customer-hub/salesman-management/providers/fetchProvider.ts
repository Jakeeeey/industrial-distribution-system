...
const API_BASE = "/api/ids/crm/customer-hub/salesman-management";
...
export const salesmanProvider = {
  getSalesmen: async (
    ...
    search: string = "",
    activeOnly?: boolean,
  ): Promise<{ data: Salesman[]; total: number }> => {
  ...
  getAssignedCustomers: async (salesmanId: number) => {
    const res = await fetch(
      `${API_BASE}?action=assigned-customers&id=${salesmanId}`,
    );
    const result = await res.json();
    return result.data || [];
  },
  ...
  unassignCustomer: async (junctionId: number) => {
    const res = await fetch(
      `${API_BASE}?action=unassign-customer&junctionId=${junctionId}`,
      {
        method: "DELETE",
      },
    );

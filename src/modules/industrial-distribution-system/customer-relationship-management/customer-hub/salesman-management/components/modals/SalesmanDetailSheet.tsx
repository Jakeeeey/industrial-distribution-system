...
const CustomerCardView = ({
  ...
  const validStoreName =
    c.store_name && c.store_name !== "0" ? c.store_name : null;
  const validCustomerName =
    c.customer_name && c.customer_name !== "0"
      ? c.customer_name
      : "Unknown Entity";
  const displayStoreName = validStoreName || validCustomerName;
  const locParts = [c.brgy, c.city, c.province].filter(
    (val) => val && val !== "0" && val.trim() !== "",
  );
  const displayLoc =
    locParts.length > 0 ? locParts.join(", ") : "No location data registered";
  const validContact =
    c.contact_number && c.contact_number !== "0" ? c.contact_number : null;

  const storeTypeName =
    typeof c.store_type === "object" ? c.store_type?.store_type : null;
  const classificationName =
    typeof c.classification === "object"
  ...
  if (typeof c.payment_term === "object" && c.payment_term !== null) {
    ...
    const foundTerm = paymentTerms.find(
      (pt) => String(pt.id) === String(c.payment_term),
    );
    paymentTermName = foundTerm
      ? foundTerm.payment_name
      : `Term ID: ${c.payment_term}`;
  }

  return (
    <div className="flex flex-col gap-1.5 w-full">
      <div className="flex items-center justify-between">
        <span className="text-sm font-black text-foreground uppercase truncate pr-4">
          {displayStoreName}
        </span>
        <Badge
          variant="outline"
        ...
        >
          {c.customer_code && c.customer_code !== "0"
            ? c.customer_code
            : "NO CODE"}
        </Badge>
      </div>
...
export function SalesmanDetailSheet({
  salesman,
  open,
  onOpenChange,
  onSuccess,
}: SalesmanDetailSheetProps) {
  const [activeTab, setActiveTab] = useState("profile");
  const [vehicles, setVehicles] = useState<
    { id: number; plate_number: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/ids/crm/customer/references?type=payment_term")
      .then((res) => res.json())
    ...
    if (salesman && open) {
      ...
      const initializeSheet = async () => {
        form.reset({
          ...
          inventory_day: salesman.inventory_day
  ...
  useEffect(() => {
    ...
    const handleSearch = async () => {
      ...
      setIsSearching(true);
      const results = await salesmanProvider.searchAvailableCustomers(
        searchQuery,
        salesman?.price_type,
      );
      const assignedIds = customers.map((c) => c.id);
      setSearchResults(
        results.filter((r: Customer) => !assignedIds.includes(r.id)),
      );
  ...
  const handleUpdateProfile = async (values: ProfileFormValues) => {
    ...
    try {
      const payload: Partial<Salesman> = {
        ...
        inventory_day: values.inventory_day
          ? Number(values.inventory_day)
          : undefined,
  ...
  const handleAssignCustomer = async (customerId: number) => {
    if (!salesman) return;
    try {
      const res = await salesmanProvider.assignCustomer(
        salesman.id,
        customerId,
      );
      if (res.success) {
  ...
  const handlePrintTerritory = () => {
    ...
    doc.text(
      `Salesman: ${salesman?.salesman_name} (${salesman?.salesman_code})`,
      14,
      30,
    );
    ...
    const getPaymentName = (term: unknown) => {
      if (typeof term === "object" && term !== null)
        return (term as PaymentTerm).payment_name || "Cash";
      if (term) {
                        ...
                        <FormItem className="flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm">
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground font-medium mt-0.5">
                              Enable or disable salesman operations
                            </p>
                          </div>
                          <FormControl>
                            <Switch
                      ...
                      render={({ field }) => (
                        <FormItem className="flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm">
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground font-medium mt-0.5">
                              Allow managing truck inventory
                            </p>
                          </div>
                          <FormControl>
                            <Switch
                      ...
                      render={({ field }) => (
                        <FormItem className="flex items-center justify-between p-4 rounded-xl bg-card border border-border shadow-sm">
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground font-medium mt-0.5">
                              Allow cash collections from customers
                            </p>
                          </div>
                          <FormControl>
                            <Switch
                ...
                {loadingCustomers ? (
                  ...
                  <div className="space-y-2.5">
                    {customers.map((c) => (
                      <Card
                        key={c.junction_id}
                        className="shadow-sm group hover:border-muted-foreground/30 transition-colors"
                      >

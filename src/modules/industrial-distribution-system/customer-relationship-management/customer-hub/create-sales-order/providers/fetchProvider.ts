...
const API_BASE = "/api/ids/crm/customer-hub/create-sales-order";
...
export const salesOrderProvider = {
  ...
  getCustomers: async (salesmanId: number): Promise<Customer[]> => {
    const res = await fetch(
      `${API_BASE}?action=customers&salesman_id=${salesmanId}`,
    );
    return res.json();
  },
  ...
  getSalesmanByCustomer: async (customerId: number): Promise<Salesman[]> => {
    const res = await fetch(
      `${API_BASE}?action=salesman_by_customer&customer_id=${customerId}`,
    );
    return res.json();
  },

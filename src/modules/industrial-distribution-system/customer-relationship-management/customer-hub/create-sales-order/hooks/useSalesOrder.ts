...
export function useSalesOrder() {
  ...
  const externalSalesOrderId =
    searchParams.get("orderId") || searchParams.get("sales_order_id");
  ...
  const [selectedReceiptTypeId, setSelectedReceiptTypeId] =
    useState<string>("");
  ...
  useEffect(() => {
    ...
    const selectedTerm = paymentTermsList.find(
      (pt) => Number(pt.id) === Number(paymentTerms),
    );
  ...
  const selectedSalesman = useMemo(
    () =>
      Array.isArray(salesmen)
        ? salesmen.find(
            (s) => (s.user_id || s.id)?.toString() === selectedSalesmanId,
          )
        : undefined,
  ...
  const selectedReceiptType = useMemo(
    () =>
      Array.isArray(receiptTypes)
        ? receiptTypes.find((rt) => rt.id.toString() === selectedReceiptTypeId)
        : undefined,
    [receiptTypes, selectedReceiptTypeId],
  );
  const selectedSalesType = useMemo(
    () =>
      Array.isArray(salesTypes)
        ? salesTypes.find((st) => st.id.toString() === selectedSalesTypeId)
        : undefined,
  ...
  useEffect(() => {
    const init = async () => {
      const [sm, sup, br, pt, rec, ops, pterms] = await Promise.all([
        ...
        fetch(
          "/api/ids/crm/customer-hub/create-sales-order?action=invoice_types",
        ).then((r) => r.json()),
        fetch(
          "/api/ids/crm/customer-hub/create-sales-order?action=operations",
        ).then((r) => r.json()),
        fetch(
          "/api/ids/crm/customer-hub/create-sales-order?action=payment_terms",
        ).then((r) => r.json()),
      ]);
      ...
      if (Array.isArray(rec) && rec.length > 0)
        setSelectedReceiptTypeId(rec[0].id.toString());
      if (Array.isArray(ops) && ops.length > 0)
        setSelectedSalesTypeId(ops[0].id.toString());
      ...
      if (
        currentId &&
        (currentId !== lastId.current || !isAutoFilled.current)
      ) {
        console.log(
          `[useSalesOrder] Auto-filling for ${currentId}. Last was ${lastId.current}`,
        );
        ...
        try {
          ...
          if (attachmentId) {
            const attachment = await fetch(
              `/api/ids/crm/customer-hub/create-sales-order?action=get_attachment&id=${attachmentId}`,
            ).then((r) => r.json());
            ...
            if (attachment) {
              if (attachment.sales_order_id) {
                ...
                console.log(
                  `[useSalesOrder] Found order_no ${attachment.sales_order_no} on attachment, looking up existing order...`,
                );
                const lookup = await fetch(
                  `/api/ids/crm/customer-hub/create-sales-order?action=get_order&order_no=${encodeURIComponent(attachment.sales_order_no)}`,
                ).then((r) => r.json());
                if (lookup && lookup.header && !lookup.error) {
                  finalSalesOrderId = (
                    lookup.header.order_id || lookup.header.id
                  ).toString();
                  console.log(
                    `[useSalesOrder] Resolved ${attachment.sales_order_no} to existing Order ID: ${finalSalesOrderId}`,
                  );
                }
              }
            }
            ...
            if (!finalSalesOrderId && attachment) {
              console.log(
                `[useSalesOrder] New attachment detected. Initializing with: ${attachment.customer_code} | Ref: ${attachment.sales_order_no}`,
              );

              // Initialize the Order Reference so it matches the group
              if (attachment.sales_order_no)
                setExistingOrderNo(attachment.sales_order_no);
              ...
              if (attachment.salesman_id) {
                const smData = await fetch(
                  `${salesOrderProvider.API_BASE}?action=salesman_by_id&id=${attachment.salesman_id}`,
                ).then((r) => r.json());
                if (smData) {
                  const uidStr = (
                    smData.employee_id ||
                  ...
                  if (uidStr) {
                    ...
                    const accts = await fetch(
                      `${salesOrderProvider.API_BASE}?action=accounts&user_id=${uidStr}`,
                    ).then((r) => r.json());
                    setAccounts(accts);
                    ...
                    if (sIdStr) {
                      ...
                      console.log(
                        `[useSalesOrder] Auto-selected Account ID from attachment: ${sIdStr}`,
                      );

                      // Trigger accurate price type calculation based on Salesman details
                      let finalPriceTypeId: number | null = null;
                      if (
                        smData.price_type_id !== null &&
                        smData.price_type_id !== undefined
                      ) {
                      ...
                      if (finalPriceTypeId && Array.isArray(pt)) {
                        const ptModel = pt.find(
                          (m) => m.price_type_id === finalPriceTypeId,
                        );
                        if (ptModel) {
                          setPriceType(ptModel.price_type_name);
                          console.log(
                            `[useSalesOrder] Extracted Price Type: ${ptModel.price_type_name} (ID: ${finalPriceTypeId})`,
                          );
              ...
              if (attachment.customer_code) {
                ...
                const custs = await salesOrderProvider.getAllCustomers(
                  attachment.customer_code,
                  0,
                );
                if (custs.length > 0) {
                  setCustomers(custs);
                  ...
                  if (custs[0].payment_term !== undefined)
                    setPaymentTerms(custs[0].payment_term);

                  console.log(
                    `[useSalesOrder] Auto-selected Customer: ${custs[0].customer_name} (ID: ${targetCustId})`,
                  );
                }
              }
            }
          ...
          if (finalSalesOrderId) {
            ...
            const orderData = await fetch(
              `/api/ids/crm/customer-hub/create-sales-order?action=get_order&order_id=${finalSalesOrderId}`,
            ).then((r) => r.json());
            const { header, items } = orderData;
            console.log("[useSalesOrder] Loading Existing Order Data:", {
              header,
              items,
            });
            ...
            if (header) {
              ...
              if (header.salesman_id) {
                const smUser = await fetch(
                  `${salesOrderProvider.API_BASE}?action=salesman_by_id&id=${header.salesman_id}`,
                ).then((r) => r.json());
                if (smUser) {
                  const uid = (
                    smUser.employee_id ||
                  ...
                  if (uid) {
                    ...
                    const accts = await fetch(
                      `${salesOrderProvider.API_BASE}?action=accounts&user_id=${uid}`,
                    ).then((r) => r.json());
                    setAccounts(accts);
              ...
              if (header.payment_terms !== undefined)
                setPaymentTerms(header.payment_terms);
              ...
              if (header.price_type_id) {
                ...
                if (Array.isArray(pt)) {
                  const ptModel = pt.find(
                    (m: PriceTypeModel) => m.price_type_id === pTIdNum,
                  );
                  if (ptModel) {
                    setPriceType(ptModel.price_type_name);
                  }
              ...
              if (header.supplier_id)
                setSelectedSupplierId(header.supplier_id.toString());
              if (header.branch_id)
                setSelectedBranchId(header.branch_id.toString());
              if (header.receipt_type)
                setSelectedReceiptTypeId(header.receipt_type.toString());
              if (header.sales_type)
                setSelectedSalesTypeId(header.sales_type.toString());

              if (items && Array.isArray(items)) {
                console.log(
                  "[useSalesOrder] Mapping Items with Enrichment:",
                  items.length,
                );
                ...
                try {
                  ...
                  if (Array.isArray(pData)) {
                    pData.forEach((p) =>
                      enrichedProductsMap.set(Number(p.product_id), p),
                    );
                  }
                } catch (e) {
                  console.error(
                    "[useSalesOrder] Product enrichment failed:",
                    e,
                  );
                }
                ...
                const mappedItems = items.map((it: any) => {
                  ...
                  const originalPK =
                    it.detail_id || it.order_detail_id || it.id;

                  const pid = Number(
                    typeof it.product_id === "object"
                  ...
                  const qty =
                    rawOrdered !== undefined && rawOrdered !== null
                      ? Number(rawOrdered)
                      : Number(rawQty ?? 0);
                  ...
                  const discounts =
                    enrichedP?.discounts && enrichedP.discounts.length > 0
                      ? enrichedP.discounts
                      : p.discounts || [];
                  ...
                  console.log(
                    `[useSalesOrder] Item PID=${pid}, ordQty=${qty}, allocQty=${allocMap[tempId]}, uPrice=${uPrice}, discounts=${JSON.stringify(discounts)}, netUnit=${netUnitPrice}, calcNet=${calcNet}`,
                  );
                  ...
                  return {
                    ...
                    discountType:
                      p.discount_level ||
                      (typeof it.discount_type === "number" &&
                      it.discount_type !== 0
                        ? String(it.discount_type)
                    ...
                    product: {
                      ...
                      unit_count:
                        p.unit_count || p.unit_of_measurement_count || 1,
                    },
                    discounts: discounts,
                    netAmount: calcNet,
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      try {
        const data = await salesOrderProvider.getAllCustomers(
          customerSearch,
          0,
        );
    ...
    if (id) {
      ...
      try {
        ...
        const res = await fetch(
          `/api/ids/crm/customer-hub/create-sales-order?action=accounts&user_id=${id}`,
        );
        ...
        const masterUser = salesmen.find(
          (s) => (s.user_id || s.id)?.toString() === id,
        );
        if (
          masterUser &&
          masterUser.linked_account_ids &&
          masterUser.linked_account_ids.length === 1
        ) {
          const linkedId = masterUser.linked_account_ids[0].toString();
          const linkedAccount = data.find(
            (a: Salesman) => a.id.toString() === linkedId,
          );
          console.log(
            `[handleSalesmanChange] Master User ${id} has one linked account for this customer: ${linkedId}. Auto-selecting...`,
          );
  ...
  const handleCustomerChange = async (id: string) => {
    ...
    if (customer) {
      ...
      if (customer.price_type_id)
        setPriceTypeId(Number(customer.price_type_id));
      if (customer.payment_term !== undefined)
        setPaymentTerms(customer.payment_term);
    }

    if (id) {
      try {
        // 🚀 GET LINKED SALESMEN (Via customer_salesmen link table)
        const linkedUsers = await salesOrderProvider.getSalesmanByCustomer(
          Number(id),
        );
        const activeSalesmen =
        ...
        const isCurrentValid = activeSalesmen.some(
          (s) => (s.user_id || s.id)?.toString() === selectedSalesmanId,
        );
        ...
        if (activeSalesmen.length === 1) {
          ...
          if (uid && (!selectedSalesmanId || !isCurrentValid)) {
            console.log(
              `[handleCustomerChange] Single salesman detected: ${uid}. Auto-selecting...`,
            );
            ...
            const res = await fetch(
              `/api/ids/crm/customer-hub/create-sales-order?action=accounts&user_id=${uid}`,
            );
            ...
            if (sm.linked_account_ids && sm.linked_account_ids.length === 1) {
              ...
              const linkedAccount = acctsData.find(
                (a: Salesman) => a.id.toString() === aid,
              );
              console.log(
                `[handleCustomerChange] Single account link detected: ${aid}. Auto-selecting Account...`,
              );
      ...
      setLineItems((prev) => {
        ...
        const next = prev.map((li) => {
          const match = supplierProducts.find(
            (sp) => Number(sp.product_id) === Number(li.product.product_id),
          );
          if (!match) return li;
          ...
          const priceChanged =
            isEditable &&
            (newBasePrice !== li.unitPrice ||
              JSON.stringify(newDiscounts) !== JSON.stringify(li.discounts));
          const metaChanged =
            match.available_qty !== li.product.available_qty ||
            match.display_name !== li.product.display_name;
          ...
          if (priceChanged || metaChanged) {
            ...
            if (priceChanged) {
              ...
              const netPrice = calculateChainNetPrice(
                newBasePrice,
                newDiscounts,
              );
            ...
            return {
              ...
              discountType: (match.discount_level ||
                match.discount_type ||
                li.discountType) as string | undefined,
              product: {
                ...li.product,
                ...
                display_name:
                  match.display_name ||
                  match.product_name ||
                  li.product.display_name,
                description: match.description || li.product.description,
                unit_count: (match.unit_count || li.product.unit_count) as
                  | number
                  | null,
              },
            };
  ...
  const addProduct = (product: Product, quantity: number, uom: string) => {
    ...
    const initialAlloc = Math.max(
      0,
      Math.min(quantity, Number(product.available_qty) || 0),
    );
  ...
  const updateLineItemQty = useCallback(
  ...
  const summary = useMemo(() => {
    ...
    const orderedGross = lineItems.reduce(
      (sum, item) => sum + item.unitPrice * item.quantity,
      0,
    );
    const orderedNet = lineItems.reduce((sum, item) => {
      const netPricePerUnit = calculateChainNetPrice(
        item.unitPrice,
        item.discounts,
      );
    ...
    const allocatedGross = lineItems.reduce((sum, item) => {
      const qty =
        allocatedQuantities[item.id] !== undefined
  ...
  const isValidAllocation = useMemo(() => {
    return lineItems.every((item) => {
      ...
      return (
        allocated >= 0 && allocated <= item.quantity && allocated <= available
      );
    });
  }, [lineItems, allocatedQuantities]);
  ...
  const enterCheckout = () => {
    ...
    lineItems.forEach((item) => {
      if (allocatedQuantities[item.id] === undefined) {
        const initialAlloc = Math.max(
          0,
          Math.min(item.quantity, Number(item.product.available_qty) || 0),
        );
        setAllocatedQuantities((prev) => ({
          ...prev,
          [item.id]: initialAlloc,
        }));
      }
    });
    setIsCheckout(true);
  ...
  const handleSubmitOrder = useCallback(
    async (forcedStatus?: string) => {
      ...
      const hasZeroAllocation = Object.values(allocatedQuantities).some(
        (v) => v === 0,
      );
      ...
      try {
        ...
        const itemsWithAllocation = lineItems.map((item) => {
          ...
          return {
            ...
            allocated_amount:
              calculateChainNetPrice(item.unitPrice, item.discounts) *
              (qtyVal ?? 0),
          };
        });

        console.log(
          `[SubmitOrder] Sending ${itemsWithAllocation.length} item(s) to API. Existing order: ${!!existingOrderId}`,
        );
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        itemsWithAllocation.forEach((item: any, idx: number) => {
          console.log(
            `  [Item ${idx}] detail_id=${item.detail_id}, product=${item.product?.display_name || item.product?.product_id}`,
          );
        });

        const res = await salesOrderProvider.createOrder(
          payload,
          itemsWithAllocation,
        );
        if (res.success) {

...
export function CallSheetTable({
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    if (s === "pending")
      return (
        <Badge
          variant="outline"
          className="gap-1.5 font-bold uppercase text-[10px] py-0 border-amber-200 bg-amber-50 text-amber-700 shadow-sm"
        >
    ...
    if (s === "approved")
      return (
        <Badge
          variant="outline"
          className="gap-1.5 font-bold uppercase text-[10px] py-0 border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm"
        >
    ...
    return (
      <Badge
        variant="outline"
        className="gap-1.5 font-bold uppercase text-[10px] py-0 border-rose-200 bg-rose-50 text-rose-700 shadow-sm"
      >
  ...
  const handleViewDocument = (fileId: string, filename?: string) => {
    if (!fileId) return;
    const url = `/api/ids/crm/customer-hub/callsheet/file?id=${fileId}${filename ? `&filename=${encodeURIComponent(filename)}` : ""}`;
    window.open(url, "_blank");
  };

  const handleDownload = (fileId: string, filename?: string) => {
    if (!fileId) return;
    const url = `/api/ids/crm/customer-hub/callsheet/file?id=${fileId}&download=true${filename ? `&filename=${encodeURIComponent(filename)}` : ""}`;
    window.open(url, "_blank");
  };
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border/50">
                <TableHead className="w-[60px] text-center font-bold text-foreground capitalize">
                  #
                </TableHead>
                <TableHead className="w-[180px] font-bold text-foreground capitalize">
                  Sales Executive
                </TableHead>
                <TableHead className="font-bold text-foreground capitalize">
                  Customer Hub
                </TableHead>
                <TableHead className="w-[200px] font-bold text-foreground capitalize">
                  Filing Reference
                </TableHead>
                <TableHead className="w-[150px] font-bold text-foreground capitalize">
                  Created Date
                </TableHead>
                <TableHead className="w-[120px] text-center font-bold text-foreground capitalize">
                  Status
                </TableHead>
                <TableHead className="w-[200px] text-right font-bold text-foreground capitalize pr-6">
                  Workflow
                </TableHead>
              </TableRow>
            </TableHeader>
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-0.5">
                            ...
                            {row.po_no && (
                              <Badge
                                variant="outline"
                                className="text-[8px] py-0 px-1.5 border-emerald-500/30 bg-emerald-500/5 text-emerald-600 uppercase tracking-widest leading-none"
                              >
                                PO: {row.po_no}
                              </Badge>
                            )}
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1.5">
                          ...
                          {row.related_attachments &&
                          row.related_attachments.length > 0 ? (
                            <Button
                              variant="ghost"
                              size="sm"
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleViewDocument(
                                  row.related_attachments![0].file_id || "",
                                  row.related_attachments![0].attachment_name ||
                                    "",
                                );
                              }}
                            >
                              ...
                              <span className="truncate max-w-[150px] group-hover/file:underline decoration-primary underline-offset-4">
                                {row.related_attachments.length} Attachment
                                {row.related_attachments.length > 1 ? "s" : ""}
                              </span>
                            </Button>
                          ) : row.file_id ? (
                            <Button
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 pt-4 pb-8 border-t border-border/20">
            <div className="flex flex-col gap-0.5">
              <p className="text-[10px] uppercase font-black text-muted-foreground tracking-widest opacity-40">
                Records Statistics
              </p>
              <p className="text-xs text-muted-foreground font-bold">
                Displaying{" "}
                <span className="text-foreground">{data.length}</span> of{" "}
                <span className="text-foreground">{metadata.total_count}</span>{" "}
                callsheet records
              </p>
            </div>

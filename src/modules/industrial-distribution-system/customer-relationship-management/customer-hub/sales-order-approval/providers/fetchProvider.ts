export async function getPendingOrders(
  ...
  const res = await fetch(
    `/api/ids/crm/customer-hub/sales-order-approval?${params.toString()}`,
  );
...
export async function getOrderHeader(orderId: number) {
  const res = await fetch(
    `/api/ids/crm/customer-hub/sales-order-approval?type=order-header&orderId=${orderId}&_t=${Date.now()}`,
    { cache: "no-store" },
  );
...
export async function getOrderAttachments(orderId: number, orderNo: string) {
  const res = await fetch(
    `/api/ids/crm/customer-hub/sales-order-approval?type=attachments&orderId=${orderId}&orderNo=${encodeURIComponent(orderNo)}`,
  );
...
export async function getInvoiceDetails(orderId: number, orderNo: string) {
  const res = await fetch(
    `/api/ids/crm/customer-hub/sales-order-approval?type=invoice-details&orderId=${orderId}&orderNo=${encodeURIComponent(orderNo)}`,
  );
...
export async function getPaymentSummary(
  ...
  if (!orderIds || orderIds.length === 0)
    return { invoiceTotal: 0, paidTotal: 0, unpaidTotal: 0 };

...
interface ApprovalModalProps {
  ...
  onSaveDetails: (
    orderId: number,
    header: Record<string, number | string | null | undefined>,
    items: {
      detail_id: number;
...
export function ApprovalModal({
  ...
  const isInvoiceStatus = [
    ...
    "En Route",
    "Delivered",
  ].includes(activeOrder?.order_status || "");
    ...
    if (open && order) {
      ...
      const fetchDiscountTypes = async () => {
        try {
          const res = await fetch(
            `${window.location.origin}/api/ids/crm/customer-hub/sales-order-approval?type=discount-types`,
          );
          if (res.ok) {
            const data = await res.json();
            const map: Record<number, string> = {};
            (data || []).forEach(
              (dt: { id: number; discount_type: string }) => {
      ...
      if (isInvoiceStatus) {
        ...
        const fetchAttachments = async () => {
          try {
            const atts = await getOrderAttachments(
              order.order_id,
              order.order_no,
            );
            setAttachments(atts);
  ...
  const isActionable = ["For Approval", "On Hold", "Draft"].includes(
  ...
  const getLineDiscount = (item: OrderDetail) => {
    ...
    if (item._recalculated_discount !== undefined)
      return item._recalculated_discount;
  ...
  const getLineNet = (item: OrderDetail) => {
    ...
    if (item.ordered_quantity !== undefined) {
      ...
      const gross =
        Number(item.ordered_quantity || 0) * Number(item.unit_price || 0);
      return gross - disc;
    }
  ...
  const getLineAllocated = (item: OrderDetail) => {
    ...
    if (
      item.allocated_quantity !== undefined &&
      Number(item.allocated_quantity) > 0
    ) {
      const discount = getLineDiscount(item);
      const gross =
        Number(item.allocated_quantity || 0) * Number(item.unit_price || 0);
  ...
  const calculatedGross = details.reduce(
    (sum, item) => sum + getLineGross(item),
    0,
  );
  const calculatedDiscount = details.reduce(
    (sum, item) => sum + getLineDiscount(item),
    0,
  );
  const calculatedOrderedTotal = details.reduce(
    (sum, item) => sum + getLineNet(item),
    0,
  );
  const calculatedAllocatedTotal = details.reduce(
    (sum, item) => sum + getLineAllocated(item),
    0,
  );
  const isFullyUnallocated =
    details.length > 0 &&
    details.every((li) => (li.allocated_quantity || 0) <= 0);
  ...
  const handleSaveAndAction = async (action: "approve" | "hold" | "cancel") => {
    ...
    try {
      ...
      if (action === "approve") {
        ...
        success = await onHold([activeOrder.order_id]);
      else if (action === "cancel")
        success = await onCancel([activeOrder.order_id]);
        ...
        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mt-5 mb-1">
              <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm flex flex-col gap-1 transition-all hover:border-slate-200">
                ...
                <p className="font-bold text-[13px] text-slate-900 dark:text-slate-100 leading-none mt-0.5">
                  {activeOrder.order_date
                    ? format(new Date(activeOrder.order_date), "MMM d, yyyy")
                    : "N/A"}
                </p>
              </div>
              <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm flex flex-col gap-1 transition-all hover:border-slate-200">
              ...
              <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm flex flex-col gap-1 transition-all hover:border-slate-200">
                ...
                <p className="font-bold text-[13px] text-sky-600 leading-none mt-0.5">
                  {activeOrder.payment_terms
                    ? `${activeOrder.payment_terms} Days`
                    : "COD"}
                </p>
              </div>
              <div className="bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 rounded-xl p-3 shadow-sm flex flex-col gap-1 transition-all hover:border-indigo-200">
                <p className="text-[9px] text-indigo-600 dark:text-indigo-400 uppercase font-black tracking-widest leading-none">
                  Price Type
                </p>
              ...
              <div className="bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm flex flex-col gap-1 transition-all hover:border-slate-200">
                ...
                <p className="font-bold text-[13px] text-slate-900 dark:text-slate-100 leading-none mt-0.5">
                  {formatCurrency(
                    activeOrder.net_amount || calculatedOrderedTotal,
                  )}
                </p>
              </div>
          ...
          {attachments.length > 0 && (
            <div className="px-4 sm:px-6 py-2 bg-muted/10 border-b border-border flex items-center gap-3 overflow-x-auto">
              <span className="text-[10px] font-black uppercase text-muted-foreground shrink-0 self-center">
                Attachments:
              </span>
              <div className="flex items-center gap-2">
                {attachments.map((att, i) => (
                  <a
                    key={i}
                    href={`/api/ids/crm/customer-hub/callsheet/file?id=${att.file_id}&filename=${encodeURIComponent(att.attachment_name || "attachment.jpg")}`}
                    target="_blank"
                    rel="noreferrer"
              ...
              loadingInvoice ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[280px] text-center px-8 gap-5">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-base sm:text-xl font-black text-slate-900 uppercase">
                      Billing Record Pending
                    </h3>
                    <p className="text-[11px] sm:text-sm text-slate-500 max-w-xs sm:max-w-sm font-medium leading-relaxed">
                      This order has been promoted to a billing state, but the
                      physical invoice has not yet been committed to the data
                      vault.
                    </p>
                  </div>
                </div>
                    ...
                    <Table>
                      <TableHeader className="bg-muted sticky top-0 z-10 border-b">
                        <TableRow className="hover:bg-transparent border-none">
                          <TableHead className="pl-4 sm:pl-8 h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                            Product / SKU
                          </TableHead>
                          <TableHead className="text-right h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                            Unit Price
                          </TableHead>
                          <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest w-[80px]">
                            Qty
                          </TableHead>
                          <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                            UOM
                          </TableHead>
                          <TableHead className="text-right pr-4 sm:pr-8 h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest w-[130px]">
                            Amount
                          </TableHead>
                        </TableRow>
                      </TableHeader>
                          ...
                          >
                            <TableCell className="pl-4 sm:pl-8 py-2.5 sm:py-3">
                              <div className="flex flex-col gap-0.5">
                                <span className="font-bold text-foreground text-[12px] sm:text-sm">
                                  {item.product_id?.product_name || "N/A Item"}
                                </span>
                                <span className="text-[9px] text-muted-foreground font-bold tracking-tighter font-mono">
                                  {item.product_id?.product_code}
                                </span>
                              </div>
                            </TableCell>
                            <TableCell className="text-center font-bold text-muted-foreground font-mono tracking-tight tabular-nums text-[12px] sm:text-sm">
                              {formatCurrency(item.unit_price)}
                            </TableCell>
                            <TableCell className="text-center font-bold text-muted-foreground text-[12px] sm:text-sm tabular-nums">
                              {item.quantity}
                            </TableCell>
                            <TableCell className="text-center">
                              <Badge
                                variant="outline"
                                className="text-[10px] font-bold px-1.5 py-0 border-border bg-muted/50 text-muted-foreground"
                              >
                  ...
                  <Table>
                    <TableHeader className="bg-muted sticky top-0 z-10 border-b">
                      <TableRow className="hover:bg-transparent border-none">
                        ...
                        <TableHead className="h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                          SKU
                        </TableHead>
                        <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                          Unit
                        </TableHead>
                        <TableHead className="text-right h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest whitespace-nowrap">
                          Unit Price
                        </TableHead>
                        <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest whitespace-nowrap">
                          Ordered Qty
                        </TableHead>
                        <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest whitespace-nowrap bg-sky-50/50">
                          Available
                        </TableHead>
                        <TableHead className="text-center h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest w-[120px] whitespace-nowrap">
                          Alloc Qty
                        </TableHead>
                        <TableHead className="text-right h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest">
                          Discount
                        ...
                        <TableHead className="text-right pr-4 sm:pr-8 h-10 uppercase text-[9px] font-black text-muted-foreground tracking-widest whitespace-nowrap">
                          Alloc Total
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                      ...
                      {loadingDetails ? (
                        Array.from({ length: 6 }).map((_, i) => (
                          <TableRow key={i} className="border-slate-50">
                            ...
                            <TableCell>
                              <div className="h-3.5 w-14 bg-slate-100 animate-pulse rounded ml-auto" />
                            </TableCell>
                            <TableCell>
                              <div className="h-3.5 w-8 bg-slate-100 animate-pulse rounded mx-auto" />
                            </TableCell>
                            <TableCell>
                              <div className="h-3.5 w-16 bg-slate-100 animate-pulse rounded mx-auto" />
                            </TableCell>
                            <TableCell className="pr-4 sm:pr-8">
                              <div className="h-3.5 w-16 bg-slate-100 animate-pulse rounded ml-auto" />
                            </TableCell>
                          </TableRow>
                        ))
                      ) : details.length === 0 ? (
                        <TableRow>
                          <TableCell
                            colSpan={10}
                            className="h-64 text-center text-slate-400 font-bold uppercase text-[10px] tracking-widest"
                          >
                        ...
                        details.map((li, idx) => {
                          const productName =
                            li.product_id?.product_name ||
                            li.product_id?.description ||
                            "Unknown";
                          const productCode =
                            li.product_id?.product_code || "N/A";
                          const lineAllocated = getLineAllocated(li);
                          const isExceeding =
                            li.allocated_quantity > li.ordered_quantity ||
                            (li.available_qty !== undefined &&
                              li.allocated_quantity > li.available_qty);

                          return (
                            <TableRow
                              key={li.detail_id || li.order_detail_id || idx}
                              className={cn(
                                "hover:bg-slate-50/50 transition-colors border-slate-50 group",
                                isExceeding &&
                            ...
                            >
                              ...
                              <TableCell className="py-2.5 sm:py-3.5">
                                <span className="text-[10px] font-bold text-slate-500 tracking-tighter font-mono">
                                  {productCode}
                                </span>
                              </TableCell>
                              <TableCell className="text-center">
                                <Badge
                                  variant="outline"
                                  className="text-[10px] font-bold px-1.5 py-0 border-border bg-muted/50 text-muted-foreground whitespace-nowrap"
                                >
                              ...
                              <TableCell className="text-right text-muted-foreground font-mono tabular-nums text-[12px] whitespace-nowrap px-4 tracking-tighter">
                                {getLineDiscount(li) > 0 ? (
                                  <span className="text-rose-500 font-bold">
                                    -{formatCurrency(getLineDiscount(li))}
                                  </span>
                                ) : (
                                  "none"
                                )}
                              ...
                              <TableCell className="text-center">
                                <Badge
                                  variant="outline"
                                  className="text-[10px] font-bold px-2 py-0.5 border-amber-200 bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:border-amber-900/50 dark:text-amber-400 whitespace-nowrap"
                                >
                                  {(() => {
                                    const val = li.discount_type;
                                    if (!val) return "none";
                                    const numVal = Number(val);
                                    if (!isNaN(numVal) && discountTypes[numVal])
                                      return discountTypes[numVal];
                                    return val;
                                  })()}
                                </Badge>
                              </TableCell>
          ...
          <div className="px-4 sm:px-8 py-4 sm:py-6 border-t bg-muted/30 backdrop-blur-md flex flex-row items-center justify-between gap-4 shrink-0 mt-auto">
            <div className="flex items-center gap-6 sm:gap-14 min-w-0">
              <div className="flex flex-col gap-0.5 shrink-0">
                <p className="text-[8px] sm:text-[9px] text-muted-foreground uppercase font-black tracking-widest leading-none">
                  {" "}
                  Gross Total
                </p>
                <p className="font-black text-sm sm:text-lg text-foreground leading-none mt-1 tabular-nums">
                  {formatCurrency(
                    isInvoiceStatus
                      ? invoiceData?.invoice?.gross_amount || 0
                      : calculatedGross,
                  )}
                </p>
              </div>
              <div className="flex flex-col gap-0.5 shrink-0">
                <p className="text-[8px] sm:text-[9px] text-rose-500 uppercase font-black tracking-widest leading-none">
                  Discount
                </p>
                <p className="font-black text-sm sm:text-lg text-rose-500 leading-none mt-1 tabular-nums">
                  -
                  {formatCurrency(
                    isInvoiceStatus
              ...
              <div className="flex flex-col gap-0.5 min-w-0">
                <p className="text-[8px] sm:text-[9px] text-muted-foreground uppercase font-black tracking-widest leading-none truncate">
                  {isInvoiceStatus ? "Invoice Net" : "Net Amount"}
                </p>
                <div className="flex items-baseline gap-1 leading-none mt-1">
                  <span className="text-[9px] sm:text-[11px] font-black text-muted-foreground/30 uppercase italic shrink-0">
                    PHP
                  </span>
      ...
      <Dialog open={isPdfOpen} onOpenChange={setIsPdfOpen}>
        <DialogContent
          className={cn(
            ...
            invoiceData?.pdf?.width_mm && invoiceData.pdf.width_mm < 100
              ? "sm:max-w-[400px]"
              : "sm:max-w-[90dvh] lg:max-w-4xl",
          )}
        >
          <div className="flex flex-col h-[90dvh]">
              ...
              <div className="flex items-center gap-2">
                {invoiceData?.pdf?.url && (
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      window.open(invoiceData?.pdf?.url || "", "_blank")
                    }
            ...
            <div className="flex-1 bg-slate-100 flex justify-center p-2 sm:p-4 overflow-auto scrollbar-thin scrollbar-thumb-slate-300">
              <div
                ...
                style={{
                  width: invoiceData?.pdf?.width_mm
                    ? `${(invoiceData.pdf.width_mm * 96) / 25.4}px`
                    : "100%",
                  minWidth: invoiceData?.pdf?.width_mm ? "auto" : "100%",
                  maxWidth: "100%",
                  height: invoiceData?.pdf?.height_mm
                    ? `${(invoiceData.pdf.height_mm * 96) / 25.4}px`
                    : "100%",
                  minHeight: "100%",
                }}
              >
            ...
            {(invoiceData?.pdf?.width_mm || invoiceData?.pdf?.height_mm) && (
              <div className="px-4 py-2 bg-slate-800 text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-4">
                <span>Sizing:</span>
                {invoiceData?.pdf?.width_mm && (
                  <span>W: {invoiceData.pdf.width_mm}mm</span>
                )}
                {invoiceData?.pdf?.height_mm && (
                  <span>H: {invoiceData.pdf.height_mm}mm</span>
                )}
                <span className="ml-auto text-sky-500 animate-pulse">
                  OPTIMIZED FIT ACTIVE
                </span>
              </div>
            )}
          </div>

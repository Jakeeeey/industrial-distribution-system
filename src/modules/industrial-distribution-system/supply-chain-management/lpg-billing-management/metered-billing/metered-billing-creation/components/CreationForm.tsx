...
export function CreationForm({ onSuccess, onCancel, transactionHeader, initialFlowType, salesInvoice, perInvoice = true, autoPeriodFrom = true, currentUserId = null }: Props) {
  ...
  //   // Converts back to a standard HTML date input format: 'YYYY-MM-DD'
      ...
      try {
        ...
        if (isMounted && data !== null && data?.last_current_reading !== undefined) {
          setForm((f) => ({
            ...
            // AUTO_PERIOD_FROM: use transaction_date of last reading as new period start
            // When false: lock period from/to to the transaction header values
            ...(autoPeriodFrom && data.transaction_date
              ? { billingPeriodFrom: String(data.transaction_date) }
              : transactionHeader
                ? {
  ...
  const handleSubmit = async () => {
    // IDS-CHANGE: Record baseline should POST the onboarding baseline setup transaction,
    // while regular routine billing transactions are saved as DRAFT first.
    const createdTx = await submit(isOnboarding ? "POSTED" : "DRAFT");
    if (createdTx) {
  ...
  const getValidationErrors = () => {
    ...
    if (!isValidReading) {
      errors.push(
        `Meter reading sequence is invalid (Current reading must be ${meterDirection === "DECREASING"
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 border-b border-zinc-200 dark:border-zinc-800 pb-4 sm:pb-6 pt-2 sm:pt-0 sm:border-b-0 sm:pb-0">
        ...
        {/* Desktop Buttons */}
        <div className="hidden sm:flex items-center gap-3 w-auto shrink-0 pb-1">
          {!canPost && (
          ...
          <Button
            ...
            className={`h-10 px-6 font-medium shadow-sm transition-all ${isOnboarding
      ...
      <div className="sticky top-0 z-30 sm:hidden bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md py-3 px-4 -mx-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center gap-2">
        <Button
          variant="outline"
          onClick={onCancel}
          className="flex-1 h-10 border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 font-medium"
        >
          Cancel
        </Button>
        {isOnboarding && (
      ...
      <div className="hidden sm:block border-b border-zinc-200 dark:border-zinc-800 pb-2" />
          ...
          >
            ...
            <section className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-zinc-50/50 dark:bg-zinc-800/20 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                <div className="space-y-2.5">
                  {/* RULE DEV: Renamed from Period From → Previous Transaction Date.
                      Always disabled — auto-resolved from the last transaction's billing_period_to. */}
                  <Label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider flex items-center gap-1.5">
                    Previous Transaction Date
                    {/* <span className="text-[9px] bg-zinc-100 dark:bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded font-mono">AUTO</span> */}
                  </Label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                    <Input
                      ...
                      readOnly
                      disabled
                      className="pl-9 bg-zinc-50 dark:bg-zinc-950/50 border-dashed text-zinc-500 dark:text-zinc-400 cursor-not-allowed"
                    />
                  </div>
                </div>
                <div className="space-y-2.5">
                  {/* RULE DEV: Renamed from Period To → Current Transaction Date.
                      Editable — the user actively sets the current billing transaction date (defaults to today). */}
                  <Label className="text-xs font-semibold text-zinc-500 uppercase tracking-wider">
  ...
  return (
    <div className="space-y-2.5 w-full">
      ...
      {previewUrl ? (
        <>
          <div
            className={`relative group overflow-hidden border ${hasError
              ? "border-red-400"
              : "border-zinc-200 dark:border-zinc-800"
              } bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center rounded-xl shadow-sm transition-all hover:ring-2 hover:ring-violet-500/50 ${previewClassName ?? "h-32 w-full"
              }`}
          >
        ...
        <div>
          ...
          <div
            ...
            className={`relative group transition-all duration-200 rounded-xl overflow-hidden border-2 border-dashed ${hasError
          ...
          >
            <div className="flex flex-col items-center justify-center gap-2 p-4 text-center">
              {uploading ? (
                ...
                <>
                  <ImagePlus
                    className={`h-6 w-6 mb-1 ${hasError
                      ? "text-red-500"
                      : isReadOnly

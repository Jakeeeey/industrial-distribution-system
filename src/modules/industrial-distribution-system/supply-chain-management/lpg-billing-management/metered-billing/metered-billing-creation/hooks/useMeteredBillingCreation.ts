...
export function useMeteredBillingCreation(
  ...
  // Chaining logic: Previous Transaction Date & readings chaining
  useEffect(() => {
    ...
    const fetchLastTransaction = async () => {
      try {
        ...
        setForm((prev) => ({
          ...prev,
          // RULE DEV: Previous Transaction Date = lastTx.transaction_date || transactionHeader.period_from
          billingPeriodFrom: lastTx?.transaction_date || transactionHeader?.period_from || prev.billingPeriodFrom || today,
          billingPeriodTo: today,
  ...
  }, [form.siteId, form.transactionType, transactionHeader]);
  ...
  const submit = useCallback(
    async (statusOverride?: TransactionStatus): Promise<MeteredWiwoTransaction | null> => {
      setSubmitting(true);
      try {

...
import { useMeteredBillingSummaryList } from "../hooks/useMeteredBillingSummary";
...
const TX_TYPE_BADGE: Record<TransactionType, React.ReactNode> = {
  ...
  ADJUSTMENT: (
    <Badge className="bg-blue-100 text-blue-700 border-none text-[9px] px-1.5 py-0 font-bold uppercase tracking-wider">
      Adjustment
    </Badge>
  ),
};
...
export function SummaryList({ selectedId, onSelect }: Props) {
  const [search, setSearch] = useState("");
  const [typeFilter, setTypeFilter] = useState<TypeFilter>("ALL");
  const [statusFilter, setStatusFilter] = useState<string>("ALL");

  const { rows, total, loading, params, setParams, refresh } = useMeteredBillingSummaryList({
    transactionType: "ALL",
  });
        ...
        <div className="flex gap-1.5 overflow-x-auto hide-scrollbar">
          {["ALL", "DRAFT", "POSTED", "CANCELLED"].map((st) => (
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-bold rounded-full border transition-all whitespace-nowrap ${statusFilter === st

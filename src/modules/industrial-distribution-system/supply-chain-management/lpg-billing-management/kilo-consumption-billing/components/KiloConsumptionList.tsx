...
import { useKiloWiwoList } from "../hooks/useKiloConsumption";
...
const STATUS_BADGE: Record<string, React.ReactNode> = {
  PENDING: (
    <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-200 border-none text-[10px] font-bold uppercase tracking-wider">
      Pending
    </Badge>
  ),
  BILLED: (
...
export function KiloConsumptionList({ selectedId, onSelect }: Props) {
  const [search, setSearch] = useState("");
  const { rows, total, loading, params, setParams, refresh } = useKiloWiwoList();
        ...
        {loading && rows.length === 0 ? (
          ...
          rows.map((row) => {
            ...
            const date = row.transaction_date
              ...
              : "—";
            const customer = row.customer?.customer_name ?? row.customer_code;
            const site = row.site?.site_name ?? "—";
            const totalKg = row.total_wiwo_kg != null ? Number(row.total_wiwo_kg) : 0;
              ...
              >
                <div className="flex items-start justify-between gap-3 w-full">
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className={`text-xs font-bold truncate font-mono ${selected ? "text-orange-600 dark:text-orange-400" : "text-foreground"}`}>
                      {txNo}
                    </div>

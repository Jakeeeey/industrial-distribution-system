...
export default function StockConversionModule({
  ...
  const handleManualSourceRfidSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/ids/scm/warehouse-management/stock-transfer?action=lookup_rfid&rfid=${encodeURIComponent(tag)}&branch_id=${branchId}&_t=${Date.now()}`);
  ...
  useRFIDScanner({
    ...
    onScan: async (val) => {
      try {
        const branchId = selectedBranchId > 0 ? selectedBranchId : userRef.current.branchId;
        const res = await fetch(`/api/ids/scm/warehouse-management/stock-transfer?action=lookup_rfid&rfid=${encodeURIComponent(val)}&branch_id=${branchId}&_t=${Date.now()}`);
        if (!res.ok) {

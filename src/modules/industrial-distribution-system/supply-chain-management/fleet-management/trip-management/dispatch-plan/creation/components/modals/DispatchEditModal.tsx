...
import { useDispatchCreation } from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/trip-management/dispatch-plan/creation/hooks/useDispatchCreation";
import { dispatchCreationLifecycleService } from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/trip-management/dispatch-plan/creation/services/lifecycle";
import {
...
export function DispatchEditModal({
  ...
  const loadApprovedPlans = useCallback(
    async (branchId: number, currentPdpIds?: number[]) => {
      setIsLoadingPlans(true);
      try {
        const url = new URL(
          "/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation",
  ...
  const handlePlanSelect = async (pdpIdStr: string) => {
    ...
    const retainedStops = planDetails.filter(
    ...
    try {
      ...
      const res = await fetch(
        `/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=plan_details&plan_ids=${newIds.join(",")}&trip_id=${planId}`,
        { cache: "no-store" },
      );
      const result = await res.json();
      if (result.error) throw new Error(result.error);
      ...
      const fetchedInvoices = fetchedItems.filter(
        (i: { isManualStop?: boolean; isPoStop?: boolean }) =>
          !i.isManualStop && !i.isPoStop,
      );
  ...
  useEffect(() => {
    if (open && planId) {
      const fetchDetails = async () => {
        ...
        try {
          const res = await fetch(
            `/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=post_plan_details&plan_id=${planId}`,
          );
          ...
          if (loadedIds.length > 0) {
            ...
            const detailsRes = await fetch(
              `/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=plan_details&plan_ids=${detailPlanIds}&trip_id=${planId}`,
            );
  ...
  const vehicleCapacity = useMemo(() => {
    ...
    const v = masterData.vehicles.find(
  ...
  const onSubmit = async (values: DispatchCreationFormValues) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to update dispatch trip",
      );
    } finally {
      setIsSaving(false);
    }

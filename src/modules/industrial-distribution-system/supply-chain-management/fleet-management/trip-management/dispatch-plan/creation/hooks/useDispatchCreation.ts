import { dispatchCreationLifecycleService } from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/trip-management/dispatch-plan/creation/services";
...
import { DispatchCreationMasterData } from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/trip-management/dispatch-plan/creation/types/dispatch.types";
import type { DispatchPlanSummary } from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/trip-management/dispatch-plan/creation/components/data-table";
...
export function useDispatchCreation() {
  ...
  const fetchMasterData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=master",
      );
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation/summary",
        { cache: "no-store" },
      );
      const result = await res.json();
      if (result.error) throw new Error(result.error);
      ...
      try {
        const budgetRes = await fetch(
          "/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=budget_summary",
        );
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error
          ? err.message
          : "An unexpected error occurred during trip creation.",
      );
      throw err;
    } finally {

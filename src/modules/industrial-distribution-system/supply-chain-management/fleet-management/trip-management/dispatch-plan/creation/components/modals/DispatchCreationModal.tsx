...
export function DispatchCreationModal({
    ...
    async (
      ...
      try {
        const res = await fetch(
          `/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=approved_plans&branch_id=${branchId}&limit=25&offset=${(currentPage - 1) * 25}${currentSearch ? `&search=${encodeURIComponent(currentSearch)}` : ""}`,
          { cache: "no-store" },
        );
        ...
        setApprovedPlans((prev) =>
          isLoadMore ? [...prev, ...newPlans] : newPlans,
        );
      } catch {
  ...
  const handlePlanSelect = async (pdpIdStr: string) => {
    ...
    try {
      const res = await fetch(
        `/api/ids/scm/fleet-management/trip-management/dispatch-plan/creation?type=plan_details&plan_ids=${newIds.join(",")}`,
        { cache: "no-store" },
      );
      const result = await res.json();
      if (result.error) throw new Error(result.error);
  ...
  const handleConfirmCreate = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to create dispatch trip",
      );
    }
  };

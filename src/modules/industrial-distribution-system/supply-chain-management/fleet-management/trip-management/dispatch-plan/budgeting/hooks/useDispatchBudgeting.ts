...
export function useDispatchBudgeting() {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/ids/scm/fleet-management/trip-management/dispatch-plan/budgeting/summary",
        { cache: "no-store" },
      );
      const result = await res.json();
      if (result.error) throw new Error(result.error);
  ...
  const updateBudget = async (
    ...
    try {
      const res = await fetch(
        `/api/ids/scm/fleet-management/trip-management/dispatch-plan/budgeting?plan_id=${planId}`,
        {
          method: "PATCH",

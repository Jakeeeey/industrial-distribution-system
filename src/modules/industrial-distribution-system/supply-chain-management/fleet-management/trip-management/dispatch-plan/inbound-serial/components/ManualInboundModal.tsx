...
export function ManualInboundModal({ plan, open, onOpenChange, onSuccess }: ManualInboundModalProps) {
    ...
    const [deliveryStatuses, setDeliveryStatuses] = React.useState<Record<number, DeliveryStatus>>({});
    ...
    const handleStatusChange = (invoiceId: number, status: DeliveryStatus) => {
        setDeliveryStatuses(prev => {
            const current = prev[invoiceId];
            return {
                ...prev,
                [invoiceId]: current === status ? null : status
            };
        });
    };
    ...
    const isFormValid = React.useMemo(() => {
        ...
        // Enforce is_visit === 0 validation
        for (const customer of customers) {
            for (const inv of customer.invoices) {
                if (inv.is_visit === 0) {
                    if (deliveryStatuses[inv.invoice_id] !== "not_delivered") {
                        return false;
                    }
                }
            }
        ...
        return true;
    }, [driverPresent, customers, deliveryStatuses]);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent showCloseButton={false} className="sm:max-w-[1100px] w-[calc(100vw-32px)] rounded-3xl overflow-hidden p-0 border-none shadow-2xl flex flex-col max-h-[90vh]">
                                            ...
                                            customer.invoices.map((inv) => (
                                                <tr key={inv.invoice_id} className="hover:bg-muted/10 transition-colors">
                                                        ...
                                                        <div className="flex items-start gap-4">
                                                            ...
                                                            <div className="space-y-1 min-w-0 flex-1">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    <span className="text-[10px] font-black text-muted-foreground/60 tracking-widest uppercase">INV: {inv.no}</span>
                                                                    <span className="text-[10px] font-black text-foreground/70 tracking-widest uppercase">• ₱{inv.amount.toLocaleString()}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-5 text-center align-middle">
                                                        <Checkbox 
                                                            checked={deliveryStatuses[inv.invoice_id] === "not_delivered"}
                                                            onCheckedChange={() => handleStatusChange(inv.invoice_id, "not_delivered")}
                                                            disabled={inv.is_visit === 1}
                                                            className="h-8 w-8 rounded-xl border-2 border-muted-foreground/20 data-[state=checked]:bg-rose-500 data-[state=checked]:border-rose-500 disabled:opacity-30 [&_svg]:h-5 [&_svg]:w-5 transition-all shadow-sm"
                                                        />
                                                    </td>
                                                    <td className="px-4 py-5 text-center align-middle">
                                                        <Checkbox 
                                                            checked={deliveryStatuses[inv.invoice_id] === "has_concern"}
                                                            onCheckedChange={() => handleStatusChange(inv.invoice_id, "has_concern")}
                                                            disabled={inv.is_visit === 0}
                                                            className="h-8 w-8 rounded-xl border-2 border-muted-foreground/20 data-[state=checked]:bg-orange-500 data-[state=checked]:border-orange-500 disabled:opacity-30 [&_svg]:h-5 [&_svg]:w-5 transition-all shadow-sm"
                                                        />
                                                    </td>
                                                    <td className="px-4 py-5 text-center align-middle">
                                                        <Checkbox 
                                                            checked={deliveryStatuses[inv.invoice_id] === "has_return"}
                                                            onCheckedChange={() => handleStatusChange(inv.invoice_id, "has_return")}
                                                            disabled={inv.is_visit === 0}
                                                            className="h-8 w-8 rounded-xl border-2 border-muted-foreground/20 data-[state=checked]:bg-amber-500 data-[state=checked]:border-amber-500 disabled:opacity-30 [&_svg]:h-5 [&_svg]:w-5 transition-all shadow-sm"
                                                        />
                                                    </td>
                                                </tr>
                                            ))
                ...
                <div className="px-8 py-6 bg-muted/10 border-t flex items-center justify-end gap-4 shrink-0">
                    ...
                    <Button 
                        className="h-14 px-10 rounded-2xl font-black text-sm uppercase tracking-[0.15em] !text-white !bg-rose-600 hover:!bg-rose-700 shadow-xl shadow-rose-500/20 border-none transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 disabled:pointer-events-none"

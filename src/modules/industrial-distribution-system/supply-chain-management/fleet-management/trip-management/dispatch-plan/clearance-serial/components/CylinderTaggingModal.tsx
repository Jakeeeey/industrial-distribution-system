...
const CylinderTaggingModal: React.FC<CylinderTaggingModalProps> = ({
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (serials.length > 0) {
                const response = await fetch('/api/ids/scm/fleet-management/trip-management/dispatch-plan/clearance-serial/cylinder-assets/confirm', {
                    method: 'POST',
                    headers: {
    ...
    return (
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent showCloseButton={false} className="sm:max-w-[480px] w-[95vw] p-0 bg-background rounded-2xl border-none shadow-2xl overflow-hidden flex flex-col">

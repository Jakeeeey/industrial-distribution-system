...
const API_BASE =
  "/api/ids/scm/fleet-management/logistics-delivery/pending-invoices";
...
export function ExportDialog({
  ...
  const salesmanOptions = React.useMemo(() => {
    return (options?.salesmen ?? []).map((s) => {
      ...
      let displayName =
        parts.length > 1 ? parts.slice(1).join(" - ").trim() : s.label;
  ...
  async function handleExport() {
    try {
      ...
      const dateStr =
        preset === "All Time" ? "All Time" : `${dateFrom} to ${dateTo}`;
      ...
      <DialogContent className="max-w-[700px] p-0 overflow-visible gap-0">
        <DialogHeader className="p-6 pb-4 border-b">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Filters select the criteria for the printed report.
          </p>
        </DialogHeader>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <Select value={status} onValueChange={setStatus}>
              ...
              <SelectContent>
                {["All", "Unlinked", "For Dispatch", "Inbound", "Cleared"].map(
                  (s) => (

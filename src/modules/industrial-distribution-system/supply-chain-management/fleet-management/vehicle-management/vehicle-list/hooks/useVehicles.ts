...
import {
  ...
  listFuelTypes,
  listEngineTypes,
} from "@/modules/industrial-distribution-system/supply-chain-management/fleet-management/vehicle-management/vehicle-list/providers/fetchProviders";
...
function mapVehicle(
  ...
  const name = String(
    v?.name ?? v?.vehicle_name ?? v?.model ?? v?.vehicle_model ?? "",
  ).trim();
  ...
  const fuelTypeName =
    fuelTypeId && fuelMap.has(fuelTypeId)
      ? cleanStr(fuelMap.get(fuelTypeId), "N/A")
      : null;

  const engineTypeName =
    engineTypeId && engineMap.has(engineTypeId)
      ? cleanStr(engineMap.get(engineTypeId), "N/A")
      : null;

  const currentMileage =
    toNum(v?.current_mileage ?? v?.mileage_km ?? v?.mileage ?? v?.odometer) ??
    0;
...
export function useVehicles() {
  ...
  const [engineMap, setEngineMap] = React.useState<Map<number, string>>(
    new Map(),
  );
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      const fMap = buildNameMap(
        (fuels || []).map((f: FuelTypeApiRow) => ({ id: f.id, name: f.name })),
      );
      const eMap = buildNameMap(
        (engines || []).map((e: EngineTypeApiRow) => ({
          id: e.id,
          name: e.name,
        })),
      );
      ...
      setRows(
        (vehicles || []).map((v) =>
          mapVehicle(v, tMap, uMap, latestDriverByVehicle, fMap, eMap),
        ),
      );
    } catch (e) {

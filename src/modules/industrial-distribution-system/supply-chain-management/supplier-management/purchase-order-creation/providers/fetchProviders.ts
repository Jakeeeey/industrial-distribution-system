...
export async function fetchProductSupplierLinks(
  supplierId: string | number,
): Promise<unknown[]> {
  const sp = new URLSearchParams();
  sp.set("supplierId", String(supplierId));
  return fetchData<unknown[]>(
    `${BASE}/product-supplier-links?${sp.toString()}`,
  );
}
...
export async function fetchPaymentTerms(): Promise<PaymentTerm[]> {
  return fetchData<PaymentTerm[]>(
    "/api/ids/scm/supplier-management/payment-terms",
  );
}

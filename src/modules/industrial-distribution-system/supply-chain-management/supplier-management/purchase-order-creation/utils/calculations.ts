// src/modules/industrial-distribution-system/supply-chain-management/supplier-management/purchase-order-creation/utils/calculations.ts
...
export function deriveUnitsPerBoxFromText(
  ...
  // =========================================================
  {
    ...
    // =========================================================
    const hasSizeOnlyAlt = /\b\d+(?:\.\d+)?\s*(?:KLS|KGS|KILO|KILOS)\b/.test(
      text,
    );
    ...
    const xpcs = text.match(
      /(?:X|×|\*)\s*(\d{1,5})\s*(PCS?|PC|PIECES|PCE|EA|CT)\b/,
    );
    if (xpcs?.[1]) {
    ...
    const hasSizeOnly =
      /\b\d+(?:\.\d+)?\s*(?:ML|G|KG|L|OZ|LTR|LITER|LITRE)\b/.test(text);

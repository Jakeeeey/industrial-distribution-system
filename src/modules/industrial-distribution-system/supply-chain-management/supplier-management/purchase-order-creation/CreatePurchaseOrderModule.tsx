...
function normalizeSupplier(raw: RawSupplier): Supplier {
  return {
    ...
    supplierType: String(
      raw?.supplier_type ?? raw?.supplierType ?? "TRADE",
    ).toUpperCase(),
  };
}
    ...
    <div className="space-y-1.5 w-full min-w-0">
      <div className="flex items-center justify-between gap-2 text-xs font-bold uppercase text-muted-foreground tracking-tight">
        <div className="flex items-center gap-2">
          ...
          <Badge
            variant="outline"
            className="text-[9px] font-black uppercase bg-primary/5 text-primary border-primary/20 px-1.5 h-4 flex items-center leading-none"
          >
            Trade Only
          </Badge>
        </div>
    ...
    <div className="space-y-2 w-full min-w-0">
      <div className="flex items-center justify-between gap-3 px-1">
        ...
        {props.value.length ? (
          <Badge
            variant="outline"
            className="text-[9px] font-black uppercase bg-primary/5 text-primary border-primary/20 transition-all animate-in zoom-in-95"
          >
        ...
        {props.value.slice(0, 8).map((id) => {
          ...
          return (
            <Badge
              key={id}
              variant="secondary"
              className="text-[9px] font-bold h-5 uppercase"
            >
              {b?.code ?? id}
            </Badge>
          );
...
export default function CreatePurchaseOrderModule({
  ...
  const [selectedPaymentTermId, setSelectedPaymentTermId] = React.useState<
    number | null
  >(null);
  ...
  const defaultNoDiscountId = React.useMemo(() => {
    const byName = discountTypes.find(
      (d) => String(d.name ?? "").toLowerCase() === "no discount",
    );
      ...
      setTempCart(
        (branch.items ?? []).map((it: CartItem) => ({
          ...
          discountTypeId: String(
            it.discountTypeId || defaultNoDiscountId || FALLBACK_NO_DISCOUNT_ID,
          ),
        })),
      );
  ...
  const updateQty = React.useCallback(
    (branchId: string, productId: string, qty: number) => {
      setAllocations((prev) =>
        prev.map((b) => {
    ...
    (branchId: string, productId: string) => {
      setAllocations((prev) =>
        prev.map((b) => {
          ...
          return {
            ...b,
            items: b.items.filter((it: any) => it.id !== productId),
          };
        }),
      );
    },
  ...
  const updateTempUom = React.useCallback((productId: string, _uom: string) => {
    setTempCart((prev) =>
      prev.map((x: any) =>
        x.id === productId
          ? { ...x, selectedUom: x.uom, uom: x.uom, uomId: x.uomId }
          : x,
      ),
    );
  }, []);
  ...
  const updateTempQty = React.useCallback((productId: string, qty: number) => {
    setTempCart((prev) =>
      prev.map((x: any) =>
        x.id === productId ? { ...x, orderQty: Math.max(1, qty) } : x,
      ),
    );
  }, []);
  ...
  const confirmPicker = React.useCallback(() => {
    ...
    setAllocations((prev) =>
      prev.map((b) =>
        b.branchId === branchId ? { ...b, items: normalized as any } : b,
      ),
    );

    // ✅ Toast: confirm product selection
    const branchLabel =
      allocations.find((x) => x.branchId === branchId)?.branchName ?? "branch";
  ...
  const allItemsFlat = React.useMemo(() => {
    return allocations.flatMap((b) =>
      b.items.map((item) => ({ branchName: b.branchName, item })),
    );
  }, [allocations]);
  ...
  const grossAmount = React.useMemo(() => {
    return allItemsFlat.reduce(
      (sum, x) =>
        sum + Number(x.item.price || 0) * Number(x.item.orderQty || 0),
      0,
    );
  }, [allItemsFlat]);
  ...
  const canSave =
    Boolean(selectedSupplier?.id) && allItemsFlat.length > 0 && !isSaving;
        ...
        <div className="lg:col-span-5 min-w-0">
          <SupplierSelect
            ...
            onChange={(s) => {
              ...
              if (s?.terms) {
                const matched = paymentTerms.find(
                  (pt) =>
                    pt.payment_name.toLowerCase().trim() ===
                    s.terms.toLowerCase().trim(),
                );
                if (matched) {
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                setAllocPage((p) => Math.min(allocTotalPages, p + 1))
              }
            >
              Next
            </Button>

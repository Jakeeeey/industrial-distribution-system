...
type Ctx = {
  ...
  saveReceipt: (
    porMetaData?: Record<
      string,
      { lotNo: string; batchNo?: string; expiryDate: string }
    >,
  ) => Promise<void>;
  ...
  metaDataByPorId: Record<
    string,
    { batchNo?: string; lotNo?: string; lotId?: string; expiryDate?: string }
  >;
  setMetaDataByPorId: React.Dispatch<
  ...
  // ✅ SERIALS
  serialsByPorId: Record<
    string,
    { sn: string; tareWeight?: string; expiryDate?: string }[]
  >;
  setSerialsByPorId: React.Dispatch<
    React.SetStateAction<
      Record<string, { sn: string; tareWeight?: string; expiryDate?: string }[]>
    >
  >;
...
const API_URL =
  "/api/ids/scm/supplier-management/purchase-order-receiving-manual";

// ✅ PERSISTENCE: localStorage helpers
const DRAFT_KEY_PREFIX = "scm_manual_draft_";
function getDraftKey(poId: string) {
  return `${DRAFT_KEY_PREFIX}${poId}`;
}
...
function saveDraft(poId: string, state: DraftState) {
  try {
    localStorage.setItem(getDraftKey(poId), JSON.stringify(state));
  } catch {
...
function clearDraft(poId: string) {
  try {
    localStorage.removeItem(getDraftKey(poId));
  } catch {
    /* ignore */
  }
}

export function ReceivingProductsManualProvider({
  children,
  receiverId,
}: {
  ...
  const [manualCounts, setManualCounts] = React.useState<
  ...
  const [receiptSaved, setReceiptSaved] =
    React.useState<ReceiptSavedInfo | null>(null);
  ...
  // ✅ METADATA
  const [metaDataByPorId, setMetaDataByPorId] = React.useState<
    Record<
      string,
      { batchNo?: string; lotNo?: string; lotId?: string; expiryDate?: string }
    >
  ...
  // ✅ SERIALS
  const [serialsByPorId, setSerialsByPorId] = React.useState<
    Record<string, { sn: string; tareWeight?: string; expiryDate?: string }[]>
  >({});
  ...
  const refreshList = React.useCallback(async () => {
    ...
    try {
      ...
      setList(
        Array.isArray((j as { data: ReceivingListItem[] })?.data)
          ? (j as { data: ReceivingListItem[] }).data
          : [],
      );
    } catch (e: unknown) {
  ...
  const resetSession = React.useCallback(
    (opts?: { clearStorage?: boolean; poId?: string }) => {
      setSaveError("");
      setManualCounts({});
  ...
  const addExtraProductLocally = React.useCallback(
    (item: {
      productId: string;
      name: string;
      barcode: string;
      ...
      setSelectedPO((prev) => {
        ...
        const existingItem = branchAlloc.items.find(
  ...
  const saveReceipt = React.useCallback(
    async (
      porMetaData?: Record<
        string,
        { lotNo: string; batchNo?: string; expiryDate: string }
      ...
      const snapshotAllocs = Array.isArray(selectedPO?.allocations)
        ? selectedPO!.allocations
        : [];
      const snapshotItems = snapshotAllocs.flatMap(
        (a: { items: ReceivingPOItem[] }) =>
      ...
      try {
        ...
        const sourceItems = (
          Array.isArray(sourceAllocs) ? sourceAllocs : []
        ).flatMap((a: { items: ReceivingPOItem[] }) =>
          Array.isArray(a?.items) ? a.items : [],
        );
        ...
        const savedItems: SavedItem[] = itemsToUse.map(
          (it: ReceivingPOItem) => {
            // ✅ Try literal ID first, then fallback to productId-branchId key
            const scannedNow = Number(
              snapshotCounts[it.id] ||
                snapshotCounts[`${it.productId}-${it.branchId}`] ||
                0,
            );
            ...
            return {
              ...
              uom: String(it.uom || ""),
              batchNo:
                porMetaData?.[it.id]?.batchNo ||
                porMetaData?.[`${it.productId}-${it.branchId}`]?.batchNo ||
                "",
              lotId:
                porMetaData?.[it.id]?.lotNo ||
                porMetaData?.[`${it.productId}-${it.branchId}`]?.lotNo ||
                "",
              expiryDate:
                porMetaData?.[it.id]?.expiryDate ||
                porMetaData?.[`${it.productId}-${it.branchId}`]?.expiryDate ||
                "",
              rfids: [],
            };
          },
  ...
  return (
    <ReceivingProductsManualContext.Provider value={value}>
      {children}
    </ReceivingProductsManualContext.Provider>
  );
}
...
export function useReceivingProductsManual() {
  ...
  if (!ctx)
    throw new Error(
      "useReceivingProductsManual must be used within ReceivingProductsManualProvider",
    );
  return ctx;
}

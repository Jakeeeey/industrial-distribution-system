...
type Ctx = {
  ...
  saveReceipt: (
    porMetaData?: Record<
      string,
      { lotId: string; batchNo: string; expiryDate: string }
    >,
  ) => Promise<void>;
...
const API_URL =
  "/api/ids/scm/supplier-management/purchase-order-receiving-rfid";
...
export function ReceivingProductsProvider({
  ...
  // ✅ NEW BUFFER
  const [localScannedRfids, setLocalScannedRfids] = React.useState<
    Ctx["localScannedRfids"]
  >([]);
  ...
  React.useEffect(() => {
    ...
    if (
      localScannedRfids.length === 0 &&
      activity.length === 0 &&
      verifiedBarcodes.length === 0
    )
      return;
    saveDraft(poId, {
      localScannedRfids,
  ...
  const removeActivity = React.useCallback((id: string) => {
    setActivity((prev) => {
      ...
      setLocalScannedRfids((buffer) =>
        buffer.filter((b) => b.rfid !== row.rfid),
      );
  ...
  const scanRFID = React.useCallback(
    async (rfidOverride?: string) => {
      setScanError("");
      setLastMatched(null);
      ...
      if (!activeProductId) {
        playBeep("error");
        return setScanError(
          "Please select a target product from the list above before scanning.",
        );
      }
      ...
      if (!isExtra && currentScansTotal >= limit) {
        playBeep("error");
        return setScanError(
          `Quantity limit reached (${limit}) for ${activeItem.name}.`,
        );
      }
      ...
      try {
        ...
        if (porId !== targetPorId) {
          playBeep("error");
          setScanError(
            `This RFID belongs to a different product on this PO. Please select the correct product.`,
          );
          setRfid("");
          return;
        }
        ...
        if (data?.alreadyReceived) {
          ...
          playBeep("error");
          setScanError(
            "This RFID is already received. It was not counted again.",
          );
          setRfid("");
          return;
        }
  ...
  const verifyBarcode = React.useCallback(
    async (barcode: string) => {
      ...
      const allocs = Array.isArray(selectedPO.allocations)
      ...
      for (const alloc of allocs) {
        for (const item of alloc.items) {
          if (
            ...
            const uomTrimmed = String(item.uom || "")
              .trim()
              .toUpperCase();
            if (uomTrimmed === "BOX") {
              matchingItem = item;
      ...
      if (!matchingItem) {
        ...
        if (!extraMatch) {
          playBeep("error");
          setScanError(
            `Product not found in this Purchase Order, and not found in Master Catalog.`,
          );
          return false;
        }
        ...
        const branchName =
          selectedPO.allocations[0]?.branch?.name || "Unassigned";
  ...
  const saveReceipt = React.useCallback(
    async (
      ...
      try {
        ...
        const allocs = Array.isArray(detail?.allocations)
          ? detail.allocations
          : [];
        ...
        const savedItems: SavedItem[] = (allItems as ReceivingPOItem[]).map(
          (it) => {
            ...
            const itemRfids = activity
              .filter(
                (a: ActivityRow) =>
                  a.status === "ok" && String(a.porId) === porId,
              )
              .map((a: ActivityRow) => a.rfid);

            const meta =
              porMetaData && typeof porMetaData === "object"
                ? porMetaData[porId]
                : null;

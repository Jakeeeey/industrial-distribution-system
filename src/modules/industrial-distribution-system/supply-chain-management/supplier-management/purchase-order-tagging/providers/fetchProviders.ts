...
export async function fetchTaggingPODetail(
  poId: string,
...
export function getTaggingProgress(detail: TaggingPODetail | null | undefined) {
  ...
  const expectedTotal = items.reduce(
    (sum, it) => sum + Math.max(0, Number(it?.expectedQty ?? 0)),
    0,
  );
  const taggedTotal = items.reduce(
    (sum, it) => sum + Math.max(0, Number(it?.taggedQty ?? 0)),
    0,
  );

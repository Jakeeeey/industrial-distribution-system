 ...
 */
function normalizeRfidInput(raw: string): {
  rfid: string;
  hadMultiple: boolean;
} {
  const up = String(raw ?? "")
...
export default function TaggingOfPOModule() {
  ...
  const onTagItem = React.useCallback(
    async (sku: string, rfidRaw: string, strict: boolean) => {
      ...
      const lockKey = `${selectedId}::${String(sku ?? "")
        .trim()
        .toLowerCase()}`;
      ...
      try {
        ...
        if (serverDetail && detailRef.current) {
          const merged: TaggingPODetail = {
            ...
            activity:
              serverDetail.activity.length > 0
  ...
  async function onSendPartiallyTagged() {
    ...
    try {
      setLoadingDetail(true);
      const data = await provider.updateTaggingStatus(
        selectedId,
      ...
      toast.success("Ready for Receiving", {
        description:
          "This PO status is now updated and visible in Receiving Products.",
      });
      onBack();
    } catch (e: unknown) {

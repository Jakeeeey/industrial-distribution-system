...
export default function PurchaseOrderSummaryModule({
  ...
  const [filterIndustrialSupplier, setFilterIndustrialSupplier] =
    useState("all");
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          "/api/ids/scm/supplier-management/purchase-order-summary",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              action: "get_po_details",
              poId: selectedPO!.purchase_order_id,
            }),
          },
        );
  ...
  const getInventoryStatusColor = (status: string) => {
    ...
    if (s.includes("route") || s.includes("transit"))
    ...
    if (s.includes("pending"))
      return "bg-yellow-500/15 text-yellow-600 border-yellow-500/30 dark:text-yellow-400";
  ...
  const filteredData = useMemo(() => {
    return sortedPoData.filter((item) => {
      ...
      const supplierName =
        suppliers.find((s) => s.id === item.supplier_name)?.supplier_name || "";
      ...
      const matchesSupplier =
        filterSupplier === "all" ||
        item.supplier_name?.toString() === filterSupplier;
      const matchesInv =
        filterInvStatus === "all" ||
        item.inventory_status?.toString() === filterInvStatus;
      const matchesPay =
        filterPayStatus === "all" ||
        item.payment_status?.toString() === filterPayStatus;
      const matchesTrans =
        filterTransType === "all" ||
        item.transaction_type?.toString() === filterTransType;

      const matchesDate =
        (!dateFrom || item.date >= dateFrom) &&
        (!dateTo || item.date <= dateTo);

      const matchesIndustrial =
        filterIndustrial === "all" ||
        (filterIndustrial === "yes"
          ? item.is_industrial_po
          : !item.is_industrial_po);
      const matchesSerialized =
        filterSerialized === "all" ||
        (filterSerialized === "yes"
          ? item.is_serialized_po
          : !item.is_serialized_po);
      const matchesIndustrialSupplier =
        filterIndustrialSupplier === "all" ||
        (filterIndustrialSupplier === "yes"
          ? item.is_industrial_supplier
          : !item.is_industrial_supplier);
  ...
  const previewTotals = useMemo(() => {
    return filteredData.reduce(
      (acc, po) => {
        const gross = Number(
          po.gross_amount ?? po.grossAmount ?? po.subtotal ?? 0,
        );
        const disc = Number(
          po.discounted_amount ??
            po.discountAmount ??
            po.discount_amount ??
            po.discount_value ??
            0,
  ...
  function SearchableFilter({
    ...
    return (
      <div className="space-y-1.5">
        <span className="text-[9px] font-bold uppercase text-muted-foreground/70 ml-1">
          {label}
        </span>
            ...
            <div className="flex items-center gap-1 shrink-0 ml-2">
              {value !== "all" && (
                <span
                  onClick={(e) => {
                    e.stopPropagation();
                    handleFilterChange(() => onChange("all"));
                  }}
              ...
              <div className="max-h-[200px] overflow-y-auto p-1">
                ...
                {filtered.map((o) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      handleFilterChange(() => onChange(o.id));
                      setOpen(false);
                    }}
  ...
  const payStatusOptions = useMemo(
    () =>
      paymentStatuses.map((s) => ({ id: s.id.toString(), label: s.status })),
    [paymentStatuses],
  );
  ...
  const columns: ColumnDef<PurchaseOrder>[] = useMemo(
    () => [
            ...
            <div className="font-bold text-primary flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {row.original.is_industrial_po && (
                  <Badge
                    variant="outline"
                    className="text-[8px] px-1 h-3.5 font-black bg-blue-50 text-blue-600 border-blue-200 uppercase"
                  >
                    IND
                  </Badge>
                )}
                {row.original.is_serialized_po && (
                  <Badge
                    variant="outline"
                    className="text-[8px] px-1 h-3.5 font-black bg-emerald-50 text-emerald-600 border-emerald-200 uppercase"
                  >
                    SER
                  </Badge>
                )}
                {row.original.is_industrial_supplier && (
                  <Badge
                    variant="outline"
                    className="text-[8px] px-1 h-3.5 font-black bg-purple-50 text-purple-600 border-purple-200 uppercase"
                  >
                    IND SUP
                  </Badge>
                )}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const supplierName =
            suppliers.find((s) => s.id === row.original.supplier_name)
        ...
        cell: ({ row }) => {
          const invStatusText =
            transactionStatuses.find(
              (s) => s.id === row.original.inventory_status,
            )?.status || "Unknown";
          return (
            <div className="flex justify-center">
              <Badge
                variant="outline"
                className={`${getInventoryStatusColor(invStatusText)} px-2.5 py-1 text-[10px] font-black border rounded-md shadow-none uppercase`}
              >
                {invStatusText}
              </Badge>
            </div>
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const payStatusText =
            paymentStatuses.find((s) => s.id === row.original.payment_status)
              ?.status || "Unknown";
          return (
            <div className="flex justify-center">
              <Badge
                variant="outline"
                className={`${getPaymentStatusColor(payStatusText)} px-2.5 py-1 text-[10px] font-black border rounded-md shadow-none uppercase`}
              >
                {payStatusText}
              </Badge>
            </div>
      ...
      <div className="bg-card p-6 rounded-xl border border-border shadow-sm space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-border/50">
            <label className="text-[10px] font-bold uppercase text-muted-foreground mb-3 block">
              Filter By
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                {isInitialLoad ? (
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      handleFilterChange(() => setFilterTransType(v));
                    }}
                  >
              ...
              <div className="space-y-1.5">
                ...
                {isInitialLoad ? (
                  ...
                  <Select
                    value={filterIndustrial}
                    onValueChange={(v) => {
                      handleFilterChange(() => setFilterIndustrial(v));
                    }}
                  >
                    <SelectTrigger className="h-10 text-xs bg-background border-border">
              ...
              <div className="space-y-1.5">
                ...
                {isInitialLoad ? (
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      handleFilterChange(() => setFilterSerialized(v));
                    }}
                  >
                    <SelectTrigger className="h-10 text-xs bg-background border-border">
                      <SelectValue placeholder="All Types" />
              ...
              <div className="space-y-1.5">
                ...
                {isInitialLoad ? (
                  ...
                  <Select
                    value={filterIndustrialSupplier}
                    onValueChange={(v) => {
                      handleFilterChange(() => setFilterIndustrialSupplier(v));
                    }}
                  >
                    <SelectTrigger className="h-10 text-xs bg-background border-border">
                      <SelectValue placeholder="All Types" />
          ...
          <div className="p-6 space-y-8 max-h-[75vh] overflow-y-auto">
            {selectedPO &&
              (() => {
                const invStatusText =
                  transactionStatuses.find(
                    (s) => s.id === selectedPO.inventory_status,
                  )?.status || "Unknown";
                const payStatusText =
                  paymentStatuses.find(
                    (s) => s.id === selectedPO.payment_status,
                  )?.status || "Unknown";
                const supplierName =
                  suppliers.find((s) => s.id === selectedPO.supplier_name)
                    ?.supplier_name || selectedPO.supplier_name;
                const gross = Number(
                  selectedPO.gross_amount ??
                    selectedPO.grossAmount ??
                    selectedPO.subtotal ??
                    0,
                );
                const disc = Number(
                  selectedPO.discounted_amount ??
                    selectedPO.discountAmount ??
                    selectedPO.discount_amount ??
                    selectedPO.discount_value ??
                    0,
                );
                const net = Number(
                  selectedPO.total_amount ??
                    selectedPO.total ??
                    selectedPO.net_amount ??
                    0,
                );
                      ...
                      <Table>
                        <TableHeader className="bg-muted/80">
                          <TableRow className="hover:bg-transparent border-b border-border">
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest h-12 px-6">
                              PO Number
                            </TableHead>
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest px-4">
                              Type
                            </TableHead>
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest px-4">
                              Supplier
                            ...
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center px-4">
                              Inv Status
                            </TableHead>
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center px-4">
                              Pay Status
                            </TableHead>
                            <TableHead className="font-bold text-[10px] uppercase tracking-widest px-6">
                              Remarks
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          <TableRow className="hover:bg-transparent border-none">
                            ...
                            <TableCell className="text-center px-4">
                              <Badge
                                variant="outline"
                                className={`${getInventoryStatusColor(invStatusText)} px-3 py-1 text-[10px] font-black border-2 rounded-lg uppercase shrink-0 shadow-sm`}
                              >
                                {invStatusText}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-center px-4">
                              <Badge
                                variant="outline"
                                className={`${getPaymentStatusColor(payStatusText)} px-3 py-1 text-[10px] font-black border-2 rounded-lg uppercase shrink-0 shadow-sm`}
                              >
                                {payStatusText}
                              </Badge>
                            </TableCell>
                      ...
                      allocations.length > 0 && (
                        <div className="space-y-4">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge
                              variant="outline"
                              className="text-[9px] font-black border-primary/30 text-primary px-2 py-0.5"
                            >
                              {allocations.length}{" "}
                          ...
                          <div className="rounded-xl border border-border overflow-hidden bg-card/50 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <Table>
                              <TableHeader className="bg-muted/40">
                                <TableRow className="hover:bg-transparent border-b border-border">
                                  <TableHead className="text-[9px] font-bold uppercase tracking-widest h-10 px-4">
                                    Product
                                  </TableHead>
                                  <TableHead className="text-[9px] font-bold uppercase tracking-widest px-3">
                                    Branch
                                  </TableHead>
                                  <TableHead className="text-[9px] font-bold uppercase tracking-widest text-center px-2">
                                    Ordered
                                  </TableHead>
                                  <TableHead className="text-[9px] font-bold uppercase tracking-widest text-center px-2">
                                    Received
                                  ...
                                  <TableHead className="text-[9px] font-bold uppercase tracking-widest text-right px-4">
                                    Amount (Est.)
                                  </TableHead>
                                </TableRow>
                              </TableHeader>
                                ...
                                {allocations.map((item) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <TableCell
                                        className={`text-[11px] font-black text-center px-2 font-mono ${isComplete ? "text-green-600" : isPartial ? "text-blue-600" : "text-muted-foreground/50"}`}
                                      >
                                        {item.receivedQty}
                                      </TableCell>
                                      <TableCell
                                        className={`text-[11px] font-black text-center px-2 font-mono ${balance > 0 ? "text-destructive" : "text-green-600 opacity-30"}`}
                                      >
                                        {balance}
                                      </TableCell>
                              ...
                              <TableFooter className="bg-muted/30 hover:bg-muted/30">
                                <TableRow>
                                  <TableCell
                                    colSpan={2}
                                    className="text-[10px] font-bold uppercase text-muted-foreground px-4 py-3"
                                  >
                                    Total Estimated
                                  </TableCell>
                    ...
                    {isLoadingDetails ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          {receipts.map((receipt) => {
                            ...
                            const totalAmt = receipt.items.reduce(
                              (s, i) => s + i.total,
                              0,
                            );
                            return (
                              <div
                                ...
                                className="rounded-xl border border-border overflow-hidden bg-card/50 shadow-sm"
                              >
                                {/* Receipt Header */}
                                <button
                                  type="button"
                                  onClick={() =>
                                    toggleReceipt(receipt.receiptNo)
                                  }
                                ...
                                >
                                  <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 min-w-0">
                                      <span className="text-xs font-black text-foreground font-mono tracking-tight">
                                        {receipt.receiptNo}
                                      </span>
                                      <Badge
                                        variant="outline"
                                        className={`text-[8px] font-black px-1.5 py-0 border rounded-md uppercase ${
                                          receipt.isPosted
                                  ...
                                  <div className="border-t border-border overflow-x-auto">
                                    <Table>
                                      <TableHeader className="bg-muted/30">
                                        <TableRow className="hover:bg-transparent border-b border-border">
                                          <TableHead className="text-[9px] font-bold uppercase tracking-widest h-9 px-4">
                                            Product
                                          </TableHead>
                                          <TableHead className="text-[9px] font-bold uppercase tracking-widest px-3">
                                            Branch
                                          </TableHead>
                                          <TableHead className="text-[9px] font-bold uppercase tracking-widest text-center px-3">
                                            Qty
                                          </TableHead>
                                          <TableHead className="text-[9px] font-bold uppercase tracking-widest text-right px-3">
                                            Unit Price
                                          </TableHead>
                                          <TableHead className="text-[9px] font-bold uppercase tracking-widest text-right px-3">
                                            Discount
                                      ...
                                      <TableBody>
                                        {receipt.items.map((item, idx) => (
                                          ...
                                          >
                                            ...
                                            <TableCell className="text-[10px] text-muted-foreground font-medium px-3">
                                              {item.branchName}
                                            </TableCell>
                                            <TableCell className="text-[10px] font-black text-center px-3">
                                              {item.quantity}
                                            </TableCell>
                      ...
                      <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                        ...
                        <p className="text-[10px] text-muted-foreground/60">
                          Receipts will appear here once items are received.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <div className="border border-border rounded-lg bg-card overflow-hidden shadow-sm">
              <Table>
                <TableHeader className="bg-muted/40 sticky top-0 z-10">
                  <TableRow className="hover:bg-transparent border-b border-border">
                    <TableHead className="font-bold text-foreground h-10 text-[10px] uppercase tracking-wider">
                      Date
                    </TableHead>
                    <TableHead className="font-bold text-foreground text-[10px] uppercase tracking-wider">
                      Supplier
                    </TableHead>
                    <TableHead className="font-bold text-foreground text-[10px] uppercase tracking-wider">
                      PO #
                    ...
                    <TableHead className="font-bold text-foreground text-[10px] uppercase tracking-wider text-right">
                      Gross Amount
                    </TableHead>
                    <TableHead className="font-bold text-foreground text-[10px] uppercase tracking-wider text-right">
                      Discount
                ...
                <TableBody>
                  {filteredData.length > 0 ? (
                    filteredData.map((po) => {
                      const supplierName =
                        suppliers.find((s) => s.id === po.supplier_name)
                      ...
                      const net = Number(
                        po.total_amount ?? po.total ?? po.net_amount ?? 0,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-[10px] font-semibold text-foreground uppercase">
                            {supplierName}
                          </TableCell>
                          <TableCell className="text-[10px] text-muted-foreground">
                  ...
                  {filteredData.length > 0 && (
                    <TableRow className="bg-muted/20 hover:bg-muted/20">
                      <TableCell
                        colSpan={4}
                        className="text-right text-[10px] font-bold uppercase tracking-wider text-foreground"
                      >
                        Total
                      </TableCell>
                      <TableCell className="text-right text-[10px] font-bold font-mono text-foreground">
                        {previewTotals.gross.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="text-right text-[10px] font-bold font-mono text-foreground">
                        {previewTotals.discount.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="text-right text-[10px] font-bold font-mono text-foreground">
                        {previewTotals.net.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                    </TableRow>

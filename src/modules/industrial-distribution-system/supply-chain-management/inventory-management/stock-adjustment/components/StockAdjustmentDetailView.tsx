        ...
        <div className="flex items-center gap-4 mt-2 mb-4 print:mb-8">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {isPosted && (
                <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 flex items-center gap-1 font-bold">
        ...
        <div className="rounded-xl border border-border overflow-hidden shadow-sm bg-card">
                  ...
                  <TableRow key={item.id} className="border-border hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        {/* Preserve Cylinder Serials Display */}
                        {item.is_serialized && item.serial_numbers && item.serial_numbers.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1.5 max-w-md">
                            {item.serial_numbers.map((sn, sIdx) => (
                              <Badge key={sIdx} variant="outline" className="font-mono text-[9px] px-1.5 py-0 bg-primary/5 text-primary border-primary/20 shadow-none rounded">
                                <Tag className="h-2 w-2 mr-1 inline-block" />
                                {sn}
                              </Badge>
                            ))}
                          </div>
                    ...
                    <TableCell className="text-right font-bold text-primary">₱{total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                    <TableCell className="text-center">
          ...
          <div className="p-8 bg-muted/30 border-t border-border flex flex-col items-end gap-3">
            ...
            <div className="flex items-center gap-12 w-full max-w-md justify-between">
              ...
              <span className="text-2xl font-bold text-primary font-sans">₱{data.amount?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
      </div>

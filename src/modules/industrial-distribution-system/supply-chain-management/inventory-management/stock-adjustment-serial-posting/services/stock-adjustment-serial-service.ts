...
export const stockAdjustmentService = {
  ...
  async fetchAllHeaders(params?: { search?: string; branchId?: number; type?: string; status?: string }) {
    ...
    const mappedHeaders = parsedHeaders.map(header => {
   ...
   * 5. Existing Asset Check: If in cylinder_assets and NOT on-hand -> Allowed & scanned through.
   * 6. Draft / Unregistered Check: If not in cylinder_assets -> Flagged as unregistered (triggers registration).
   */
  async validateSerialForAdjustment(params: {
    ...
    try {
      ...
      // Check if present in cylinder_assets_draft (already registered in draft table, awaiting document posting)
  ...
  async fetchProducts(params?: { search?: string }) {
    // AG-COMMENT: Include is_serialized in fields to properly filter non-serialized items in Stock Adjustment Posting
  ...
  async fetchProductsBySupplier(supplierId: number, search?: string) {
    ...
    // AG-COMMENT: Include is_serialized in fields parameter for supplier products query in posting

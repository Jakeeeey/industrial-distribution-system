...
import { StockAdjustmentHeader } from "../types/stock-adjustment-serial.schema";

/**
 * Hook for the Stock Adjustment **list page** only.
 *
 * Form-specific data (products, serial, inventory lookups) lives in the
 * separate `useStockAdjustmentForm` hook so opening the form does NOT
 * re-fetch the adjustment list and vice-versa.
 */
export function useStockAdjustmentSerial(defaultStatus?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/ids/scm/inventory-management/stock-adjustment-serial-registration?${queryParams.toString()}`
      );
  ...
  const deleteAdjustment = async (id: number) => {
    try {
      const response = await fetch(
        `/api/ids/scm/inventory-management/stock-adjustment-serial-registration/${id}`,
        { method: "DELETE" }
      );
      const result = await response.json();
      if (result.error) throw new Error(result.error);

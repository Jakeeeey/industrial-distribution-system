...
import { StockAdjustmentDetailView } from "../../stock-adjustment-serial-registration/components/StockAdjustmentDetailView";
      ...
      <DialogContent showCloseButton={false} className="max-w-6xl w-full sm:max-w-6xl max-h-[90vh] overflow-y-auto p-0 gap-0 rounded-2xl border border-border shadow-xl">
        ...
        {id !== null && (
          <div className="relative w-full flex flex-col min-h-0">
            <button
              onClick={onClose}
              className="absolute top-5 right-5 z-50 flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background shadow-xs hover:bg-muted text-muted-foreground transition-all hover:scale-105 focus:outline-none"
              title="Close Details"
            >

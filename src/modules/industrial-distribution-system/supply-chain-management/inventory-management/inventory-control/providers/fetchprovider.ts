...
export async function fetchSerialOnhand(branchId: number): Promise<SerialOnhandRecord[]> {
    const res = await apiGet<SerialOnhandRecord[]>(
        API_BASE,
...
export async function fetchProducts(): Promise<ProductInfo[]> {
    return directusGetItems<ProductInfo>("products", {
        fields: "product_id,product_name,barcode,product_category",
        sort: "product_name",
        limit: "-1",

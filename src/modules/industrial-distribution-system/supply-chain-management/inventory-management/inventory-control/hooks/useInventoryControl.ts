...
import { computeSummary, enrichSerials, groupByCategory } from "../utils/grouping";
import type {
    BranchInfo,
    CategoryGroup,
    EnrichedSerial,
...
export function useInventoryControl(): UseInventoryControlReturn {
    ...
    const [selectedBranchId, setSelectedBranchIdState] = useState<number | null>(196);
    ...
    const [printOptions, setPrintOptionsState] = useState<PrintOptions>(DEFAULT_PRINT_OPTIONS);
    ...
    const filteredSerials: EnrichedSerial[] = selectedProduct
        ? selectedProduct.serials.filter((s) => {
            ...
            return (s.barcode ?? "").toLowerCase().includes(q);
        })
        : [];

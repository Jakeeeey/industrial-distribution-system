...
import { PaginatedConsolidators } from "@/modules/industrial-distribution-system/supply-chain-management/warehouse-management/consolidation/delivery-picking/providers/fetchProvider";
import { ConsolidatorDto as DeliveryConsolidatorDto } from "@/modules/industrial-distribution-system/supply-chain-management/warehouse-management/consolidation/delivery-picking/types";
...
export const fetchActiveBranches = async (): Promise<BranchDto[]> => {
  try {
    const url = new URL(
      "/api/ids/scm/warehouse-management/consolidation/branches",
...
export const fetchActivePickingBatches = async (
  ...
  try {
    const url = new URL(
      "/api/ids/scm/warehouse-management/consolidation/delivery-picking",
      window.location.origin,
    );
...
export async function submitManualPick(payload: {
  ...
  const res = await fetch(
    "/api/ids/scm/warehouse-management/consolidation/picking/manual",
    {
      method: "POST",
      headers: HEADERS,
...
export const transmitItemScan = async (payload: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        message: err.message || `HTTP ${response.status} Error`,
      };
    }
...
export const lookupRfidTag = async (
  rfidTag: string,
): Promise<number | null> => {
  try {
    const url = new URL(
      "/api/ids/scm/warehouse-management/consolidation/picking/lookup",
      window.location.origin,
    );
...
export const completePickingBatch = async (
  batchId: number,
): Promise<boolean> => {
  try {
    const url =
      "/api/ids/scm/warehouse-management/consolidation/picking/complete";
    const response = await fetch(url, {
      method: "POST",
      headers: HEADERS,
...
export const fetchConsolidators = async (
  ...
  try {
    ...
    const d = data as {
      content?: DeliveryConsolidatorDto[];
    ...
    return {
      content: Array.isArray(d.content)
        ? (d.content as DeliveryConsolidatorDto[])
        : [],

    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/api/ids/scm/warehouse-management/stock-transfer?action=products${query}${branchQuery}`,
        );
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-muted/5 scrollbar-hide">
              {loading ? (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground transition-all">
                    Fetching Catalog...
                  </p>
                </div>
              ) : products.length === 0 ? (
                      ...
                      <div className="p-3 flex-1 flex flex-col gap-3">
                        <div className="space-y-1.5 flex-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <div className="flex items-center gap-1 text-[9px] text-primary/70 font-bold uppercase tracking-tighter">
                              ...
                              <span>
                                {typeof product.unit_of_measurement ===
                                  "object" &&
                                product.unit_of_measurement !== null
                                  ? (
                                      product.unit_of_measurement as {
                                        unit_name?: string;
                                      }
                                    ).unit_name
                                  : String(
                                      product.unit_of_measurement || "PCS",
                                    )}
                              </span>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border/30">
                          <div className="text-xs font-black text-primary font-mono bg-primary/5 px-2 py-0.5 rounded">
                            ₱
                            {Number(
                              (product as { cost_per_unit?: number })
                                .cost_per_unit || 0,
                            ).toLocaleString()}
                          </div>
                          <Button
          ...
          <div className="w-72 border-l border-border bg-muted/10 flex flex-col hidden lg:flex">
            <div className="p-6 border-b border-border bg-card">
              <h3 className="font-bold text-sm flex items-center gap-2 text-foreground">
                ...
                <Badge
                  variant="secondary"
                  className="ml-auto font-mono text-[10px] bg-primary/10 text-primary border-none"
                >
                ...
                {selectedProducts.length === 0 ? (
                  ...
                  selectedProducts.map((p, idx) => {
                    ...
                    const uom =
                      ...
                      p.unit_of_measurement !== null
                        ? (p.unit_of_measurement as { unit_name?: string })
                            .unit_name
                        : p.unit_of_measurement || "PCS";
                    const qty =
                      (p as EnrichedProduct & { quantity?: number }).quantity ||
                      1;

                    return (
                      <div
                        key={idx}
                        className="bg-background border border-border/40 rounded-lg p-2.5 hover:border-primary/30 transition-all group/item shadow-none"
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1 border border-border/60 rounded p-0.5 bg-muted/20">
                            <Button
                              ...
                              onClick={() =>
                                onUpdateQty?.(Number(pid), Math.max(1, qty - 1))
                              }
                            >
                              <Minus className="w-2.5 h-2.5" />
                            </Button>
                            <span className="text-[10px] font-black w-6 text-center font-mono">
                              {qty}
                            </span>
                            <Button
                              variant="ghost"

...
export function useStockTransferDispatch() {
  ...
  const [scannedItemsState, setScannedItemsState] = useState<
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem(
        LOCAL_STORAGE_KEY,
        JSON.stringify(scannedItemsState),
      );
    }
  }, [scannedItemsState]);
    ...
    return base.baseOrderGroups.map((group) => {
      const enrichedItems = group.items.map((st: OrderGroupItem) => {
        ...
        const targetQty = Math.max(
          0,
          st.allocated_quantity ?? st.ordered_quantity ?? 0,
        );
        const rawAvailable =
          scannedInventory[pid as number] ??
          (st as OrderGroupItem).qtyAvailable ??
          0;
        ...
        for (const item of selectedGroup.items) {
          ...
          if (res.ok) {
            ...
            const availableCount = inventoryList.reduce(
              (acc: number, inv: { runningInventory: number }) =>
  ...
  const updateOrderStatus = async (orderNo: string, status: string) => {
    ...
    try {
      ...
      base.setStockTransfers((prev) =>
        prev.map((st) => (st.order_no === orderNo ? { ...st, status } : st)),
      );
    } catch (err) {
  ...
  const handleScanRFID = async (rfid: string) => {
    ...
    try {
      const match = await stockTransferLifecycleService.lookupRfid(
        rfid,
        selectedGroup.sourceBranch!,
      );
      ...
      const currentRfids =
        scannedItemsState[base.selectedOrderNo]?.[productId] || [];
      ...
      const allScannedInOrder = Object.values(
        scannedItemsState[base.selectedOrderNo] || {},
      ).flat();
      ...
      const targetQty = Math.max(
        0,
        itemInOrder.allocated_quantity ?? itemInOrder.ordered_quantity ?? 0,
      );
      ...
      const message =
        error instanceof Error ? error.message : "RFID lookup failed";

...
export function useStockTransferApproval() {
  ...
  useEffect(() => {
    ...
    const fetchAvailable = async () => {
      ...
      try {
        const sourceBranchName = base.getBranchName(
          base.selectedGroup!.sourceBranch,
        );
          ...
          if (res.ok) {
            ...
            const inventoryList = list.filter(
              (inv: {
                ...
                String(inv.branchId) ===
                  String(base.selectedGroup!.sourceBranch),
            );
  ...
  const updateStatus = async (
    ...
    const group = base.baseOrderGroups.find(
      (g: OrderGroup) => g.orderNo === orderNo,
    );
    if (!group) return;
    ...
    try {
      ...
      if (status === "approved") {
        ...
        for (const item of group.items) {
          const allocated =
            allocatedQtys[item.id] ?? item.ordered_quantity ?? 0;

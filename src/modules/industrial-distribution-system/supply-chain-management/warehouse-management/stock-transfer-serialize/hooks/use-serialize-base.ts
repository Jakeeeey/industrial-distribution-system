...
export function useSerializeBase({ statuses, autoFetch = true }: UseSerializeBaseProps) {
  ...
  const fetchTransfers = useCallback(async (currentPage: number, currentSearch: string, isLoadMore = false) => {
    ...
    try {
      ...
      const url = `/api/ids/scm/warehouse-management/stock-transfer-serialize?action=list_groups&status=${statusesStr}&search=${encodeURIComponent(currentSearch)}&limit=${PAGE_SIZE}&offset=${offset}`;
      ...
      if (branches.length === 0) {
        const branchRes = await fetch('/api/ids/scm/warehouse-management/stock-transfer?action=branches');

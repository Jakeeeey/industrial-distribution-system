...
import {
  ConsolidatorDto,
  ConsolidatorDetailsDto,
  ConsolidatorDispatchesDto,
} from "../types";
...
export function ConsolidatorDetailSheet({
  consolidator,
  isOpen,
  onClose,
  onStatusUpdate,
}: ConsolidatorDetailSheetProps) {
  ...
  const uniqueClusters = useMemo(() => {
    const clusters =
      consolidator?.dispatches?.map((d) => d.clusterName).filter(Boolean) || [];
    return [...new Set(clusters)].length;
  }, [consolidator?.dispatches]);

  const groupedDetails = useMemo(() => {
    if (!consolidator?.details) return {};
    const filtered = consolidator.details.filter(
      (d) =>
  ...
  const totalPicked =
    consolidator.details?.reduce((acc, curr) => acc + curr.pickedQuantity, 0) ||
    0;
  const totalOrdered =
    consolidator.details?.reduce(
      (acc, curr) => acc + curr.orderedQuantity,
      0,
    ) || 0;
  const batchProgress =
  ...
  const fetchAndPrintManifest = async () => {
    ...
    try {
      const manifestRes = await fetch(
        `/api/ids/scm/warehouse-management/consolidation/delivery-picking/manifest/${consolidator.consolidatorNo}`,
      );
      if (manifestRes.ok) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        const res = await fetch(
          `/api/ids/scm/warehouse-management/consolidation/delivery-picking/start`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              consolidatorNo: consolidator.consolidatorNo,
              checkerId,
            }),
          },
        );
  ...
  return (
    <>
        ...
        <SheetContent className="w-full sm:max-w-2xl overflow-hidden p-0 flex flex-col bg-background rounded-l-[2rem]">
          <div className="p-5 lg:p-6 bg-muted/10 border-b border-border/50 shrink-0">
            <SheetHeader className="space-y-3 text-left">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[9px] font-black uppercase tracking-widest border-muted-foreground/20"
                    >
                    ...
                    <span className="text-[10px] text-muted-foreground font-bold flex items-center gap-1">
                      ...
                      {format(
                        parseISO(consolidator.createdAt),
                        "MMM dd, HH:mm",
                      )}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <MiniStat
                  label="Items"
                  value={consolidator.details?.length || 0}
                  icon={<Box className="text-blue-500" />}
                />
                <MiniStat
                  label="Routes"
                  value={consolidator.dispatches?.length || 0}
              ...
              <div className="px-5 py-3 border-b border-border/40 shrink-0 bg-card/30 backdrop-blur-md flex items-center justify-between gap-4">
                ...
                <div className="relative flex-1 max-w-[180px]">
                  ...
                  <Input
                    ...
                    className="h-8 pl-8 text-[11px] font-bold bg-muted/20 border-none rounded-lg"
                    value={skuSearch}
                    ...
                    ([supplier, categories]) => (
                      <div key={supplier} className="space-y-4">
                        <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm py-2 flex items-center gap-2 border-b border-border/20">
                          ...
                          <h3 className="font-black uppercase tracking-[0.2em] text-[10px] text-foreground/80">
                            {supplier}
                          </h3>
                        </div>
                        {Object.entries(categories).map(([category, items]) => (
                          <div
                            key={category}
                            className="pl-4 border-l-2 border-primary/10 space-y-2"
                          >
          ...
          <div className="p-6 border-t border-border/50 bg-card/50 flex items-center gap-3 shrink-0 z-30">
            <Button
              ...
              onClick={fetchAndPrintManifest}
              disabled={isPrinting || consolidator.status === "Pending"}
            >
              {isPrinting ? (
            ...
            {consolidator.status === "Pending" ? (
              <Button
                onClick={() => setIsStartPickingOpen(true)}
                disabled={isSyncing}
                className="flex-1 font-black uppercase text-[10px] tracking-widest h-12 rounded-xl bg-primary"
              >
                {isSyncing ? (
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
                ) : (
              ...
              <Button
                disabled
                className="flex-1 font-black uppercase text-[10px] tracking-widest h-12 rounded-xl bg-emerald-500/10 text-emerald-500 border-none"
              >
                <ShieldCheck className="h-4 w-4 mr-2" />
                {consolidator.status === "Picked"
                  ? "Verify Batch"
                  : "Picking Active"}
              </Button>
            )}
      ...
      <StartPickingDialog
        isOpen={isStartPickingOpen}
        onClose={() => setIsStartPickingOpen(false)}
        onConfirm={handleConfirmPicking}
        batchNo={consolidator.consolidatorNo}
      />
    </>
  );
}
...
function MiniStat({ label, value, icon, className }: MiniStatProps) {
  return (
    <div
      className={`flex items-center gap-1.5 bg-background/50 border border-border/40 rounded-lg px-2.5 py-1.5 shadow-sm ${className}`}
    >
...
function SKUCard({ detail }: SKUCardProps) {
  const pickProgress =
    detail.orderedQuantity > 0
      ...
      : 0;
  const isShortage =
    detail.pickedQuantity < detail.orderedQuantity && detail.pickedQuantity > 0;

  return (
    <div
      className={`relative border border-border/40 rounded-xl px-4 py-3 bg-card/40 flex items-center justify-between gap-4 overflow-hidden group ${isShortage ? "bg-amber-500/5 border-amber-500/30" : ""}`}
    >
      {isShortage && (
        <div className="absolute top-0 left-0 w-1 h-full bg-amber-500" />
      )}
      <div className="flex-1 min-w-0">
        <h4 className="font-black text-[11px] text-foreground/90 truncate uppercase tracking-tight">
          {detail.productName}
        </h4>
        <div className="flex items-center gap-3 mt-1 font-mono text-[9px] text-muted-foreground/60 font-bold uppercase tracking-tighter">
          ...
          {detail.barcode && (
            <span className="flex items-center gap-1">
              <Barcode className="h-2.5 w-2.5" /> {detail.barcode}
            </span>
          )}
        </div>
      </div>
      <div className="text-right shrink-0">
        <div className="flex items-baseline justify-end gap-1 font-mono">
          <span
            className={`text-sm font-black ${pickProgress >= 100 ? "text-emerald-500" : "text-amber-500"}`}
          >
        ...
        <p className="text-[8px] text-muted-foreground/50 font-black uppercase tracking-widest mt-0.5">
          {detail.unitName}
        </p>
      </div>
      <Progress
        value={pickProgress}
        className={`absolute bottom-0 left-0 w-full h-[3px] rounded-none bg-transparent ${pickProgress >= 100 ? "[&>div]:bg-emerald-500" : "[&>div]:bg-amber-500"}`}
      />
    </div>
  );
}
    ...
    <div className="group relative border border-border/40 rounded-2xl p-4 bg-card/30 backdrop-blur-sm shadow-sm hover:border-primary/40 transition-all duration-300">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-black text-sm tracking-tighter uppercase">
            {disp.dispatchNo}
          </span>
        </div>
        <Badge
          variant="outline"
          className="bg-primary/5 text-primary border-primary/20 font-black uppercase text-[9px] tracking-widest px-2 py-0.5 rounded-md"
        >
          {disp.status || "PENDING"}
        </Badge>
      </div>
      <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-border/40">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-[11px] font-black uppercase tracking-tight truncate">
            {disp.driverName || "Pending Assign"}
          </span>
        </div>
        <div className="flex flex-col gap-1 border-l border-border/40 pl-4">
          ...
          <span className="text-[11px] font-black uppercase tracking-tighter text-blue-600/80">
            {disp.clusterName || "Global"}
          </span>
        </div>
      </div>
    </div>

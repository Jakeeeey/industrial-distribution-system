...
const API_BASE = "/api/ids/scm/logistics/file-maintenance/cluster-management";
 ...
 */
export async function fetchAllBarangays(
  provinceCode: string,
): Promise<BarangayItem[]> {
...
export async function getCluster(clusterId: number): Promise<ClusterWithAreas> {
  const res = await fetch(`${API_BASE}?type=detail&cluster_id=${clusterId}`);
  if (!res.ok) throw new Error(await readError(res));
  const json = await res.json();
  return json?.data;
}

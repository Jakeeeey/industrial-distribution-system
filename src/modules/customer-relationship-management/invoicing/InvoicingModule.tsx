...
import { Loader2, FileText, Users, TrendingUp, CircleDollarSign } from "lucide-react";
...
const InvoicingModule: React.FC = () => {
    ...
    const fetchSalesOrders = useCallback(async (currentFilters: InvoicingFilters) => {
        setIsLoading(true);
        try {
            let data = await InvoicingService.getSalesOrders(currentFilters);

            // Apply Status Filter (Client Side) - Strict Categorization
            if (currentFilters.status && currentFilters.status !== "All") {
                data = data.filter(order => {
                    const hasVoid = (order.void_invoices && order.void_invoices.length > 0);
                    const hasRecycled = (order.existing_invoices && order.existing_invoices.length > 0) || !!order.existing_invoice_no || !!order.existing_invoice_display_no;

                    if (currentFilters.status === "Void") {
                        return hasVoid;
                    }
                    if (currentFilters.status === "Recycled") {
                        return hasRecycled;
                    }
                    if (currentFilters.status === "Normal") {
                        // Strict Normal: Must NOT have any void or recycled attributes
                        return !hasVoid && !hasRecycled;
                    }
                    return true;
                });
            }

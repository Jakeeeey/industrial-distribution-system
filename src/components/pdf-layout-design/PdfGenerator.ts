...
import { CompanyData, PdfConfig, PdfElementConfig, PaperSize } from "./types";
import { PAPER_SIZES } from "./constants";

/**
 * Renders a single PDF element onto a jsPDF document.
 */
export const renderElement = (doc: jsPDF, el: PdfElementConfig, data: any) => {
    ...
    const getElementValue = (el: PdfElementConfig): string | null => {
        ...
        switch (el.id) {
            ...
            case 'company_address': return data.company_address;
            case 'company_brgy': return data.company_brgy;
            case 'company_city': return data.company_city;
            case 'company_province': return data.company_province;
            case 'company_zipCode': return data.company_zipCode;
            case 'company_contact': return data.company_contact;
            case 'company_email': return data.company_email;
            default: return data[el.id] || null;
        }
    };
    ...
    // Handle Images
    if (el.type === 'image') {
        const imageUrl = el.id === 'company_logo' ? data?.company_logo : (data?.[el.id] || el.content);
        if (imageUrl) {
            try {
...
export const generatePdf = async (config: PdfConfig, data: any): Promise<jsPDF> => {
...
export const getPdfBlobUrl = async (config: PdfConfig, data: any): Promise<string> => {

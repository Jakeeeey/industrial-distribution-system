...
import { CreateSalesOrderModule } from "@/modules/industrial-distribution-system/customer-relationship-management/customer-hub/create-sales-order";
  ...
  try {
    ...
    if (attachmentId) {
      const res = await fetch(
        `${DIRECTUS_URL}/items/sales_order_attachment/${attachmentId}?fields=file_id,attachment_name`,
      ...
      const res = await fetch(
        `${DIRECTUS_URL}/items/sales_order_attachment?filter[sales_order_id][_eq]=${orderId}&fields=file_id,attachment_name&limit=1`,
        { headers, cache: "no-store" },
      );
      if (res.ok) {
        const json = await res.json();
    ...
    if (targetFileId) {
      documentViewerUrl = `/api/ids/crm/customer-hub/callsheet/file?id=${targetFileId}`;
      if (targetFileName)
        documentViewerUrl += `&filename=${encodeURIComponent(targetFileName)}`;
    }
  } catch (e) {

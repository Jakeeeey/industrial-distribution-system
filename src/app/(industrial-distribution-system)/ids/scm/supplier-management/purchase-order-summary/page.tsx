...
import {
  getDirectusBase,
  directusHeaders,
} from "@/modules/industrial-distribution-system/supply-chain-management/supplier-management/purchase-order-summary/providers/fetchProviders";
...
import {
  ...
  Supplier,
  StatusRef,
} from "@/modules/industrial-distribution-system/supply-chain-management/supplier-management/purchase-order-summary/types";
...
async function getData(): Promise<{
  ...
  try {
    ...
    const supsJson = (await supRes.json()) as {
      data: Record<string, unknown>[];
    };
    const allSuppliers = supsJson.data || [];
    const industrialSupplierIds = allSuppliers
      .map((s) => Number(s.id))
      .filter(Boolean);
    ...
    if (!industrialSupplierIds.length) {
      const [payRes, transRes] = await Promise.all([
        fetch(`${base}/items/payment_status?limit=-1`, {
          cache: "no-store",
          headers,
        }),
        fetch(`${base}/items/transaction_status?limit=-1`, {
          cache: "no-store",
          headers,
        }),
      ]);
    ...
    const serializedProductIds = new Set(
      (serProdData.data || []).map((p: Record<string, unknown>) =>
        toNum(p.product_id),
      ),
    );
    ...
    const enrichedPoList = poList.map((po: Record<string, unknown>) => {
      ...
      const supplierId: number =
        typeof supplierObj === "object" && supplierObj !== null
          ? toNum(
              (supplierObj as Record<string, unknown>)?.id ??
                (supplierObj as Record<string, unknown>)?.supplier_id ??
                0,
            )
          : toNum(supplierObj ?? 0);
...
export default async function Page() {
  ...
  const { poData, suppliers, paymentStatuses, transactionStatuses } =
    await getData();
        ...
        <div className="flex h-full items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Purchase Order Summary</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>

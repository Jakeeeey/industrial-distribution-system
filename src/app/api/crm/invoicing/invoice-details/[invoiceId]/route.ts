...
export async function GET(
    ...
    try {
        ...
        const details = (data.data || []).map((d: { 
            detail_id: number; 
            order_id: string; 
            invoice_no: string; 
            product_id: { product_id: number; product_name: string; unit_of_measurement?: { unit_shortcut: string } } | number | null; 
            product_name?: string;
            unit?: string;
            unit_price: number; 
            quantity: number; 
            ...
            const isObj = d.product_id !== null && typeof d.product_id === 'object';
            const p = isObj ? (d.product_id as { product_id: number; product_name: string; unit_of_measurement?: { unit_shortcut: string } }) : null;
            
            return {
                detail_id: d.detail_id,
                order_id: d.order_id,
                invoice_no: d.invoice_no,
                product_id: p?.product_id ?? d.product_id,
                product_name: p?.product_name ?? d.product_name ?? "Unknown Product",
                unit_shortcut: p?.unit_of_measurement?.unit_shortcut ?? d.unit ?? "N/A",

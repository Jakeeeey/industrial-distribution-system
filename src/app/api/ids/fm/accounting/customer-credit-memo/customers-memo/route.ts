// src/app/api/ids/scm/accounting/customers-memo/route.ts
    ...
    switch (action) {
      ...
      case "chart-of-accounts": {
        const result = await directusFetch<
          DirectusListResponse<ChartOfAccountRow>
        >(
      ...
      case "next-memo-number": {
        ...
        const lastRes = await directusFetch<
          DirectusListResponse<CustomersMemoRow>
        >(
      ...
      case "list": {
        ...
        const fields = [
          ...
          "chart_of_account.coa_id",
          "chart_of_account.gl_code",
        ...
        if (result.data && result.data.length > 0) {
          const encoderIds = Array.from(
            new Set(
              result.data
                .map((m) =>
                  typeof m.encoder_id === "number" ? m.encoder_id : null,
                )
          ...
          if (encoderIds.length > 0) {
            try {
              const userRes = await directusFetch<
                DirectusListResponse<UserRow>
              >(
                `${DIRECTUS_URL}/items/user?fields=user_id,user_fname,user_lname&filter[user_id][_in]=${encoderIds.join(",")}`,
              );
              if (userRes.data) {
                const userMap = new Map(
                  userRes.data.map((u) => [u.user_id, u]),
                );
                result.data = result.data.map((m) => ({
                  ...m,
                  encoder_id:
                    typeof m.encoder_id === "number"
                      ? userMap.get(m.encoder_id) || m.encoder_id
                      : m.encoder_id,
                }));
              }
            } catch (e) {
              console.warn(
                "[Customers Memo API] Encoder enrichment failed:",
                e,
              );
            }
          }
      ...
      case "memo-details": {
        ...
        const headerRes = await directusFetch<
          DirectusItemResponse<CustomersMemoRow>
        ...
        if (header && typeof header.encoder_id === "number") {
          try {
            ...
            console.warn(
              "[Customers Memo API] Detail encoder enrichment failed:",
              e,
            );
          }
        }
        ...
        const collections = await directusFetch<
          DirectusListResponse<CollectionMemoItem>
        >(
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (history.length > 0) {
      ...
      const collectionMemos: CollectionMemoInsertPayload[] = history.map(
        (item) => ({
          memo_id: memoId,
      ...
      const invoiceLinksRaw: CollectionInvoiceInsertPayload[] = history.flatMap(
        (item) =>
      ...
      if (uniqueInvoiceLinks.length > 0) {
        try {
          await directusFetch<
            DirectusItemResponse<CollectionInvoiceInsertPayload[]>

...
export async function GET(request: NextRequest) {
  if (!DIRECTUS_BASE || !DIRECTUS_TOKEN) {
    console.error(
      "[HRM/EmployeeHistory] Missing NEXT_PUBLIC_API_BASE_URL or DIRECTUS_STATIC_TOKEN",
    );
  ...
  try {
    ...
    const [
      ...
      fetchCollection("user", {
        ...
        limit: "1",
        fields:
          "user_id,user_fname,user_lname,user_email,user_position,user_image,user_department",
      }),
      ...
      fetchCollection("department_schedule", {
        fields:
          "department_id,work_start,work_end,lunch_start,lunch_end,break_start,break_end,working_days,workdays_note,grace_period",
      }),
    ...
    const deptMap = new Map(
      depts.map((d: Record<string, unknown>) => [d.department_id, d]),
    );
    const schedMap = new Map(
      scheds.map((s: Record<string, unknown>) => [s.department_id, s]),
    );
    const dept = deptMap.get(rawUser.user_department) ?? {};
    const deptSched = schedMap.get(rawUser.user_department) ?? {};

    const deptSchedFields = extractScheduleFields(
      null,
    ...
    if (from && to) {
      ...
      while (cursor <= end) {
        ...
        const logObj = logsMap.get(dateStr) as
        ...
        if (schedFields.is_oncall) {
          ...
          enrichedLog.late = calculateLate(
            log.time_in as string | null,
            schedFields.work_start as string | null,
            schedFields.grace_period as number,
          );
          enrichedLog.overtime = calculateOvertime(
            log.time_in as string | null,
            log.time_out as string | null,
            schedFields.work_start as string | null,
            schedFields.work_end as string | null,
            log.lunch_start as string | null,
            log.lunch_end as string | null,
          );
        } else {

...
} from "../../../../../modules/industrial-distribution-system/human-resource-management/workforce/attendance-report/department-report/utils/oncall";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [deptsRes, schedRes, oncallListRes, oncallSchedRes] =
      await Promise.all([
    ...
    const userParams: Record<string, string> = {
      fields:
        "user_id,user_fname,user_lname,user_position,user_image,user_department,is_deleted",
    };
    ...
    const merged = allUsers.flatMap((user: Record<string, unknown>) => {
      const deptInfo = (deptMap.get(user.user_department) ?? {}) as Record<
        string,
        unknown
      >;
      const schedInfo = (schedMap.get(user.user_department) ?? {}) as Record<
        string,
        unknown
      >;
      ...
      return dates.map((date) => {
        ...
        const effectiveWorkStart = (schedFields.work_start ?? null) as
          | string
          | null;
        const effectiveWorkEnd = (schedFields.work_end ?? null) as
          | string
          | null;
        ...
        const lunchStart = (log?.lunch_start ??
          schedFields.lunch_start ??
          null) as string | null;
        const lunchEnd = (log?.lunch_end ?? schedFields.lunch_end ?? null) as
          | string
          | null;
        const breakStart = (log?.break_start ??
          schedFields.break_start ??
          null) as string | null;
        const breakEnd = (log?.break_end ?? schedFields.break_end ?? null) as
          | string
          | null;
        ...
        if (log) {
          ...
          status =
            !hasTimeIn && !hasTimeOut && !exemptStatuses.includes(rawStatus)
              ? "Incomplete"
              : rawStatus || "Present";
        ...
        const workMins = calculateWorkMins(
          timeIn,
          timeOut,
          lunchStart,
          lunchEnd,

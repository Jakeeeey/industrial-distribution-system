...
} from "../../../../../modules/industrial-distribution-system/human-resource-management/workforce/attendance-report/todays-report/utils/oncall";
...
export async function GET(request: NextRequest) {
  if (!DIRECTUS_BASE || !DIRECTUS_TOKEN) {
    console.error(
      "[HRM/Attendance] Missing NEXT_PUBLIC_API_BASE_URL or DIRECTUS_STATIC_TOKEN",
    );
  ...
  try {
    const [
      ...
      fetchCollection("department_schedule", {
        fields:
          "schedule_id,department_id,work_start,work_end,lunch_start,lunch_end,break_start,break_end,grace_period,working_days,workdays_note",
      }),
      fetchCollection("oncall_list", {}),
      fetchCollection("oncall_schedule", {
        fields:
          "id,department_id,group,working_days,work_start,work_end,lunch_start,lunch_end,break_start,break_end,workdays,grace_period,schedule_date",
      }),
    ]);
    ...
    console.log(
      `[HRM/Attendance] logs=${logs.length} users=${users.length} depts=${depts.length} scheds=${scheds.length}`,
    );
    ...
    function buildRecord(
      ...
      const record: Record<string, unknown> = {
        ...
        department_name:
          (dept as Record<string, unknown>).department_name ?? "—",

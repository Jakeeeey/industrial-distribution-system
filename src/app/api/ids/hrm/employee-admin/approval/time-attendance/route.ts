...
import { TAApprovalService } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/time-attendance/services/time-attendance.service";
import {
  RequestStatus,
  RequestType,
} from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/time-attendance/types";
import { TAActionPayloadSchema } from "@/modules/industrial-distribution-system/human-resource-management/employee-admin/approval/time-attendance/types/time-attendance.schema";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filters = {
      ...
      types:
        (searchParams.get("types")?.split(",") as RequestType[]) || undefined,
    ...
    if (searchParams.get("action") === "logs") {
      ...
      const data = await TAApprovalService.fetchApproverLogs(
        authUserId,
        limit,
        isHRHead,
        filters,
    ...
    const data = await TAApprovalService.fetchManagerQueue(
      authUserId,
      filters,
      isHRHead,
    );

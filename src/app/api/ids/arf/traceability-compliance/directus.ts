/**
 * src/lib/directus.ts
 * ─────────────────────────────────────────────────────────────
 * Single source of truth for server-side Directus configuration.
 * All API routes must import helpers from here — never hardcode
 * the Directus base URL or token directly in route files.
 *
 * Required .env.local variables:
 *   DIRECTUS_URL          - e.g. http://100.110.197.61:8056
 *   DIRECTUS_STATIC_TOKEN - Static API token from Directus
 */

/** Returns the Directus base URL (no trailing slash). Throws if not set. */
export function getDirectusBase(): string {
    const raw =
        process.env.DIRECTUS_URL ||
        process.env.NEXT_PUBLIC_DIRECTUS_URL ||
        process.env.NEXT_PUBLIC_API_BASE_URL ||
        "";

    const cleaned = raw.trim().replace(/\/$/, "");
    if (!cleaned) {
        throw new Error(
            "DIRECTUS_URL is not set. Add it to .env.local and restart the dev server."
        );
    }
    return /^https?:\/\//i.test(cleaned) ? cleaned : `http://${cleaned}`;
}

/** Returns the Directus static token. Throws if not set. */
export function getDirectusToken(): string {
    const token = (process.env.DIRECTUS_STATIC_TOKEN || process.env.DIRECTUS_TOKEN || "").trim();
    if (!token) {
        throw new Error(
            "DIRECTUS_STATIC_TOKEN is not set. Add it to .env.local and restart the dev server."
        );
    }
    return token;
}

/** Returns headers for authenticated Directus requests. */
export function directusHeaders(): Record<string, string> {
    return {
        "Content-Type": "application/json",
        Authorization: `Bearer ${getDirectusToken()}`,
    };
}

/**
 * Fetches a Directus URL with JSON response handling.
 * Throws a descriptive Error on non-2xx responses.
 */
export async function directusFetch<T = unknown>(url: string, init?: RequestInit): Promise<T> {
    const res = await fetch(url, {
        ...init,
        headers: {
            ...directusHeaders(),
            ...(init?.headers as Record<string, string> | undefined),
        },
        cache: "no-store",
    });

    const json = await res.json().catch(() => ({}));
    if (!res.ok) {
        const jsonError = json as { errors?: { message: string }[]; error?: string };
        const msg =
            jsonError.errors?.[0]?.message ||
            jsonError.error ||
            `Directus responded ${res.status} ${res.statusText}`;
        throw new Error(msg);
    }
    return json as T;
}

...
export async function GET(req: NextRequest) {
    try {
        ...
        if (type === "customers" && planId) {
            ...
            const siRes = await fetch(`${DIRECTUS_URL}/items/sales_invoice?filter[invoice_id][_in]=${invoiceIds.join(",")}&fields=invoice_id,invoice_no,customer_code,total_amount,net_amount,is_visit`, {
                headers: { Authorization: `Bearer ${AUTH_TOKEN}` },
            });
            ...
            const customerGroup = new Map<string | number, { customer_code: string | number; customer_name: string; address: string; invoices: { invoice_id: number; no: string; amount: number; is_visit?: number }[] }>();
...
export async function PATCH(req: NextRequest) {
    try {
        ...
        if (invoices.length > 0) {
            ...
            for (const si of sis) {
                // deliveryStatuses is now keyed by invoice_id
                const statusMapping = deliveryStatuses ? deliveryStatuses[si.invoice_id] : null;

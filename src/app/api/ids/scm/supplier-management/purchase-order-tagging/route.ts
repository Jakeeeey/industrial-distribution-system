...
async function directusFetch<T = unknown>(
  ...
  if (!res.ok) {
    ...
    const msg =
      ...
      `Directus error ${res.status} ${res.statusText}`;
    throw new Error(msg);
  }
  return json as T;
}
...
function bad(error: string, status = 400, extra?: unknown) {
  return NextResponse.json(DEBUG ? { error, debug: extra } : { error }, {
    status,
  });
}
...
function normalizeRfidServer(raw: string): {
  rfid: string;
  hadMultiple: boolean;
} {
...
async function fetchAllPORowsByPOId(base: string, poId: number) {
  ...
  if (DEBUG)
    dlog(
      `fetchAllPORowsByPOId for PO ${poId} returned ${j?.data?.length ?? 0} rows.`,
    );
...
async function fetchExistingRfidRow(
  base: string,
  rfid: string,
): Promise<ReceivingItemRow | null> {
  const url =
    `${base}/items/${RECEIVING_ITEMS_COLLECTION}?limit=1` +
    `&sort=-created_at` +
...
async function ensureOpenReceivingRow(args: {
  ...
  const created = await fetchJson<{ data: Record<string, unknown> }>(
    insertUrl,
    {
      method: "POST",
...
async function buildDetail(
  ...
  const suppliersMap = await fetchSuppliersMapByIds(
    base,
  ...
  const receivingItems = await fetchReceivingItemsByLinkIds(base, [
    ...porIds,
    ...popIds,
  ]);
  ...
  if (receivingItems.length > 0) {
    dlog(
      `Sample receivingItems[0] linkId: ${receivingItems[0].purchase_order_product_id}`,
    );
  }
  ...
  for (const it of receivingItems) {
    ...
    if (!key) {
      if (DEBUG)
        console.log(`[tagging-of-po] linkId ${linkId} has no mapping key.`);
      continue;
    }
  ...
  if (DEBUG) {
    console.log(
      `[tagging-of-po] Aggregated taggedByKey:`,
      Object.fromEntries(taggedByKey),
    );
    console.log(
      `[tagging-of-po] porIdToKey size: ${porIdToKey.size}, popIdToKey size: ${popIdToKey.size}`,
    );
  }
  ...
  const items: TaggingPOItem[] = poProducts.map((line) => {
    ...
    const sku =
      scan || toStr(p?.product_code) || String(pid || line.product_id);
    ...
    if (DEBUG) {
      dlog(
        `Line [${sku}]: key=${key}, totalPool=${totalPool}, usedSoFar=${usedSoFar}, allocated=${allocated}`,
      );
    }
  ...
  const activity: TaggingActivity[] = activityFromReceivingItems(
    receivingItems,
    productsMap,
  );
...
function activityFromReceivingItems(
  ...
  productsMap: Map<
    number,
    { product_name: string; barcode: string; product_code: string }
  >,
): TaggingActivity[] {
...
function resolvePoProductLine(args: {
  ...
  const sortedLines = [...args.poProducts].sort((a, b) => {
    ...
    const isDoneA =
      (args.taggedByKey.get(keyA) ?? 0) >= toNum(a.ordered_quantity);
    const isDoneB =
      (args.taggedByKey.get(keyB) ?? 0) >= toNum(b.ordered_quantity);
...
export async function GET() {
  try {
    ...
    const poIds = rows
      .map((r: Record<string, unknown>) => toNum(r?.purchase_order_id))
      .filter(Boolean);
    ...
    for (const line of poProducts) {
      ...
      expectedByPo.set(
        poId,
        (expectedByPo.get(poId) ?? 0) +
          Math.max(0, toNum(line.ordered_quantity)),
      );
    }
    ...
    const list: TaggablePOListItem[] = rows.map((r) => {
      ...
      return {
        ...
        taggedItems,
        status:
          totalItems > 0 && taggedItems >= totalItems ? "completed" : "tagging",
      };
    });
  ...
  try {
    ...
    if (action === "tag_item") {
      ...
      if (!rfid || rfid.length !== RFID_LEN) {
        return bad(
          `Invalid RFID. RFID must be exactly ${RFID_LEN} hexadecimal characters.`,
          400,
          {
            rfidRaw,
            normalized: rfid,
      ...
      const detailWithRawTags = (await buildDetail(base, poId)) as Record<
        string,
        unknown
      >;
      ...
      if (existing) {
        return bad(
          `RFID code '${rfid}' already exists in receiving items. Cannot be duplicate.`,
          409,
          { rfid },
        );
      }
      ...
      const currentItem = currentDetail.items.find(
        (x: TaggingPOItem) => x.id === String(popId),
      );
      ...
      const isPresent = updated.activity.some(
        (a: TaggingActivity) => a.rfid === rfid,
      );
      if (!isPresent) {

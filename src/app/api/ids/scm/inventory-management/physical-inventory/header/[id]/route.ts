//src/app/api/ids/scm/physical-inventory/header/[id]/route.ts
...
export async function PATCH(
  ...
  try {
    ...
    if ("ph_no" in body)
      payload.ph_no = typeof body.ph_no === "string" ? body.ph_no.trim() : "";
    ...
    if ("price_type" in body)
      payload.price_type = Number.isFinite(Number(body.price_type))
        ? Number(body.price_type)
        : null;
    if ("stock_type" in body)
      payload.stock_type =
        typeof body.stock_type === "string" ? body.stock_type.trim() : "";
    if ("branch_id" in body)
      payload.branch_id = Number.isFinite(Number(body.branch_id))
        ? Number(body.branch_id)
        : null;
    if ("remarks" in body)
      payload.remarks = typeof body.remarks === "string" ? body.remarks : "";
    if ("supplier_id" in body)
      payload.supplier_id = Number.isFinite(Number(body.supplier_id))
        ? Number(body.supplier_id)
        : null;
    if ("category_id" in body)
      payload.category_id = Number.isFinite(Number(body.category_id))
        ? Number(body.category_id)
        : null;
    if ("isComitted" in body)
      payload.isComitted = Number(body.isComitted) === 1 ? 1 : 0;
    if ("isCancelled" in body)
      payload.isCancelled = Number(body.isCancelled) === 1 ? 1 : 0;
    if ("committed_at" in body)
      payload.committed_at =
        typeof body.committed_at === "string" ? body.committed_at : null;
    if ("cancelled_at" in body)
      payload.cancelled_at =
        typeof body.cancelled_at === "string" ? body.cancelled_at : null;
    if ("total_amount" in body)
      payload.total_amount = Number.isFinite(Number(body.total_amount))

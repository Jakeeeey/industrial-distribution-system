...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!res.ok)
      throw new Error(
        `Directus error fetching bank accounts: ${res.statusText}`,
      );
...
export async function POST(req: NextRequest) {
  ...
  if (!token) {
    return NextResponse.json(
      { error: "Server Error: DIRECTUS_STATIC_TOKEN is missing" },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Directus bank account create failed: ${res.statusText} - ${errorText}`,
      );
    }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      throw new Error(
        `Directus bank account update failed: ${res.statusText} - ${errorText}`,
      );
    }

...
export async function GET() {
  if (!DIRECTUS_BASE || !DIRECTUS_TOKEN) {
    console.error(
      "[HRM/Employee] Missing NEXT_PUBLIC_API_BASE_URL or DIRECTUS_STATIC_TOKEN",
    );
  ...
  try {
    const [usersRes, deptsRes, schedRes, oncallListRes, oncallSchedRes] =
      await Promise.all([
        fetchCollection("user", {
          fields:
            "user_id,user_fname,user_lname,user_email,user_position,user_image,user_department,is_deleted",
        }),
        ...
        fetchCollection("department_schedule", {
          fields:
            "department_id,work_start,work_end,working_days,workdays_note,grace_period",
        }),
    ...
    console.log(
      `[HRM/Employee] users=${users.length} depts=${depts.length} scheds=${scheds.length}`,
    );
    ...
    const merged = users
      .map((u) => {
        const dept = (deptMap.get(u.user_department) ?? {}) as Record<
          string,
          unknown
        >;
        const sched = (schedMap.get(u.user_department) ?? {}) as Record<
          string,
          unknown
        >;
